...
fn status_reverts_delivered_to_claimed() {
    ...
    assert!(
        stdout.contains("updated WI-001: status"),
        "stdout: {stdout}"
    );
    ...
    assert!(
        workitem_block_contains(&combined, "WI-001", "WI-000"),
...
fn status_open_clears_claimant() {
    ...
    let workitems =
        std::fs::read_to_string(proj.join("anthill-todo").join("workitems.anthill")).unwrap();
    assert!(
        workitems.contains("status: Open"),
...
fn status_name_is_case_insensitive() {
    ...
    assert!(
        out.status.success(),
        "lowercase status name rejected: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let workitems =
        std::fs::read_to_string(proj.join("anthill-todo").join("workitems.anthill")).unwrap();
...
fn status_unknown_name_errors_and_writes_nothing() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("unknown status") && stderr.contains("Frobnicate"),
        "expected unknown-status diagnostic, got: {stderr}"
    );
    let combined = read_combined(&proj.join("anthill-todo"));
    assert!(
        combined.contains("status: Open"),
...
fn description_and_status_combine() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("updated WI-001: description, status"),
        "summary should list both fields: {stdout}"
    );
    let combined = read_combined(&proj.join("anthill-todo"));
    assert!(
        combined.contains("some(value: \"reworded\")"),
        "new description not persisted: {combined}"
    );
    let workitems =
        std::fs::read_to_string(proj.join("anthill-todo").join("workitems.anthill")).unwrap();
    assert!(
        workitems.contains("status: Open") && !workitems.contains("Claimed"),
