...
fn preopened_dep_blocks_dependent_in_list() {
    ...
    let blocked_idx = stdout[open_hdr..]
        .find("-- blocked --")
        .map(|i| open_hdr + i)
    ...
    assert!(
        stdout.contains("WI-001 [PreOpened]"),
...
fn blocked_split_is_status_agnostic_across_groups() {
    ...
    let blocked_idx = tail
        .find("-- blocked --")
        .expect("PreOpened group blocked section");
