...
fn preopen_demotes_open_to_preopened() {
    ...
    assert!(
        stdout.contains("preopened: WI-001 by bob"),
        "stdout: {stdout}"
    );

    let combined = read_combined(&proj.join("anthill-todo"));
    assert!(
        combined.contains("status: PreOpened"),
...
fn promote_lifts_preopened_to_open() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("promoted: WI-001 by bob"),
        "stdout: {stdout}"
    );
...
fn preopen_on_non_open_errors() {
    ...
    assert!(
        stderr.contains("WI-001") && stderr.contains("is not Open"),
...
fn promote_on_non_preopened_errors() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("WI-001") && stderr.contains("is not PreOpened"),
