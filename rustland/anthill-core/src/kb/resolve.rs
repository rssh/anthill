...
impl KnowledgeBase {
    ...
    fn reduce_op_value(
        ...
        }) = occ.as_expr()
        {
            ...
            return self.reduce_op_value(Value::Node(call), subst, depth, dispatch_body_less, faults);
        }
            ...
            match self.walk_arg(item, subst) {
                Some(a) => {
                    let a = if reduce_args && depth < FOLD_DEPTH_CAP {
                        self.reduce_op_value(a, subst, depth + 1, dispatch_body_less, faults)
                    } else {
                        a
                    };
        ...
        if needs_dict {
            return self
                .bridge_op_to_eval(op, &params, &param_args, subst, faults)
                .unwrap_or(v);
        }
        ...
        let reduced = self.reduce_op_value(reduced, subst, depth + 1, false, faults);
        match reduced {
    ...
    fn bridge_op_to_eval(
        ...
        let Some(outcome) = self.run_in_bridge_interp(|interp| interp.call_op_bridged(op, &args))
        else {
    ...
    fn reduce_operand(
        ...
        self.reduce_op_value(v, subst, 0, false, faults)
    }
    ...
    fn reduce_dispatched_goal_call(
        ...
        let v = self.reduce_dot_value(v, subst);
        self.reduce_op_value(v, subst, 0, true, faults)
    }
