...
fn alloc_named(kb: &mut KnowledgeBase, functor_qn: &str, named: &[(&str, TermId)]) -> TermId {
    ...
    let args: SmallVec<[(_, _); 2]> = named.iter().map(|(k, v)| (kb.intern(k), *v)).collect();
    kb.alloc(Term::Fn {
        functor: f_sym,
...
"#;
    ...
    let guard = alloc_named(
        &mut kb,
        "anthill.reflect.LogicalQuery.pattern_query",
        &[("term", ancestor_pat)],
    );
...
"#;
    ...
    let ancestor_sym = kb
        .try_resolve_symbol("test.guard2.Family.ancestor")
        .unwrap();
    ...
    let pq_a = alloc_named(
        &mut kb,
        "anthill.reflect.LogicalQuery.pattern_query",
        &[("term", ancestor_pat)],
    );
    let pq_h = alloc_named(
        &mut kb,
        "anthill.reflect.LogicalQuery.pattern_query",
        &[("term", home_pat)],
    );
    let conj = alloc_named(
        &mut kb,
        "anthill.reflect.LogicalQuery.conjunction",
    ...
    assert!(
        triggers.contains(&family_sym),
        "missing Family in {:?}",
        triggers
    );
    assert!(
        triggers.contains(&place_sym),
        "missing Place in {:?}",
        triggers
    );
    assert_eq!(
        triggers.len(),
        2,
        "expected exactly two unique trigger sorts"
    );
}
