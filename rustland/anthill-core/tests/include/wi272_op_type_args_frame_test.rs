...
fn extract_sort_ref_name(interp: &Interpreter, tid: TermId) -> Option<String> {
    ...
    let sym = anthill_core::kb::typing::extract_sort_ref_sym(
        interp.kb(),
        &anthill_core::kb::term_view::TermIdView(tid),
    )?;
...
fn explicit_int_binding_installs_t_on_frame() {
    ...
    let name = extract_sort_ref_name(&interp, snap[0].1).expect("T should bind to a sort_ref(...)");
...
fn second_call_with_different_binding_is_per_call_fresh() {
    ...
    interp
        .call("test.wi272.frame.Driver.driver_string", &[])
...
fn inferred_binding_matches_explicit() {
    ...
    interp
        .call("test.wi272.frame.Driver.driver_inferred", &[])
