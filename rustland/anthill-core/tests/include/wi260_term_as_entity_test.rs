...
"#;
    ...
    let item_sym = kb
        .try_resolve_symbol("test.wi260.Inventory.Item")
    ...
    let result = interp
        .call("anthill.reflect.term_as_entity", &[Value::term(item_tid)])
...
"#;
    ...
    let result = interp
        .call("anthill.reflect.term_as_entity", &[Value::term(tid)])
        .expect("call term_as_entity");
...
"#;
    ...
    let kb = interp.kb_mut();
    let inner_sym = kb
        .try_resolve_symbol("test.wi260_nested.Tree.Inner")
        .expect("Inner present");
    let outer_sym = kb
        .try_resolve_symbol("test.wi260_nested.Tree.Outer")
    ...
    let result = interp
        .call("anthill.reflect.term_as_entity", &[Value::term(outer_term)])
        .expect("call term_as_entity");
