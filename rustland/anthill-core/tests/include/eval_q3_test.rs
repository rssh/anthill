...
"#,
    ...
    let red_sym = kb
        .try_resolve_symbol("test.q3_alloc.Color.red")
        .expect("red symbol");
...
fn q3_alloc_from_value_rejects_closures_streams_lazies() {
    ...
    for v in [
        ...
        assert!(
            matches!(err, LowerError::UnsupportedVariant(_)),
...
fn q3_lower_empty_query_yields_zero_goals() {
    ...
    let empty_q_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.empty_query")
...
fn q3_lower_rejects_non_logical_query() {
    ...
    assert!(
        matches!(err, LowerError::NotALogicalQuery { .. }),
        "got {err:?}"
    );
}
...
"#,
    ...
    assert!(
        solutions.len() >= 3,
        "expected at least 3 EntityInfo solutions, got {}",
        solutions.len()
    );

    let red_tid = kb
        .try_resolve_symbol("test.q3_exec.Color.red")
        .expect("red sym");
    let green_tid = kb
        .try_resolve_symbol("test.q3_exec.Color.green")
        .expect("green sym");
    let blue_tid = kb
        .try_resolve_symbol("test.q3_exec.Color.blue")
        .expect("blue sym");
        ...
        match sol.subst.resolve_as_value(vn) {
            Some(Value::Term { id: t, .. }) => {
                ...
                if let Some(f) = functor {
                    if f == red_tid {
                        seen_colors.insert("red");
                    }
                    if f == green_tid {
                        seen_colors.insert("green");
                    }
                    if f == blue_tid {
                        seen_colors.insert("blue");
                    }
                }
            }
    ...
    assert_eq!(
        seen_colors.len(),
        3,
        "expected all three colors to appear, saw {:?}",
        seen_colors
    );
}
...
"#,
    ...
    let pair_sym = kb
        .try_resolve_symbol("test.q3_conj.Pair.pair")
        .expect("pair sym");
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    let conj_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.conjunction")
        .unwrap();
    let term_field = kb.intern("term");
    ...
    let pat_x = Value::Entity {
        ...
        pos: Vec::new().into(),
        named: vec![
            (x_field, Value::term(var_v)),
            (y_field, Value::term(var_y1)),
        ]
        .into(),
    };
...
fn q3_quantifier_lowering_is_not_yet_implemented() {
    ...
    let forall_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.forall_q")
    ...
    assert!(
        matches!(err, LowerError::NotYetImplemented(_)),
        "got {err:?}"
    );
}
...
"#,
    ...
    let left_tag_sym = kb
        .try_resolve_symbol("test.q3_disj.LeftTag.left_tag")
        .expect("left_tag");
    let right_tag_sym = kb
        .try_resolve_symbol("test.q3_disj.RightTag.right_tag")
        .expect("right_tag");
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    let disj_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.disjunction")
        .unwrap();
    let term_field = kb.intern("term");
    let left_field = kb.intern("left");
    let right_field = kb.intern("right");
    ...
    let query = entity_named(disj_sym, vec![(left_field, left_q), (right_field, right_q)]);

    let mut stream = kb
        .execute_logical_query(&query)
        .expect("disjunction lowers");
    ...
    assert_eq!(
        seen, expected,
        "both branches must contribute distinct ?v bindings"
    );
}
...
"#,
    ...
    let left_tag_sym = kb
        .try_resolve_symbol("test.q3_disj_conj.LeftTag.left_tag")
        .unwrap();
    let right_tag_sym = kb
        .try_resolve_symbol("test.q3_disj_conj.RightTag.right_tag")
        .unwrap();
    let has_marker_sym = kb
        .try_resolve_symbol("test.q3_disj_conj.HasMarker.has_marker")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    ...
    let disj_q = entity_named(disj_sym, vec![(left_field, left_q), (right_field, right_q)]);
    let marker_q = entity_named(
        pattern_query_sym,
        vec![(
            term_field,
            Value::Entity {
                functor: has_marker_sym,
                pos: Vec::new().into(),
                named: vec![(label_field, Value::term(var_m))].into(),
            },
        )],
    );
    let query = entity_named(
        conj_sym,
        vec![(left_field, disj_q), (right_field, marker_q)],
    );
    ...
    while let Some((sol, rest)) = stream.split_first(&mut kb) {
        ...
        {
            if let (Term::Const(Literal::String(vs)), Term::Const(Literal::String(ms))) =
    ...
    assert!(
        seen.contains(&("alpha".to_string(), "M".to_string()))
...
"#,
    ...
    let a_sym = kb
        .try_resolve_symbol("test.q3_disj_nested.A.a_tag")
        .unwrap();
    let b_sym = kb
        .try_resolve_symbol("test.q3_disj_nested.B.b_tag")
        .unwrap();
    let c_sym = kb
        .try_resolve_symbol("test.q3_disj_nested.C.c_tag")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
...
"#,
    ...
    let left_sym = kb
        .try_resolve_symbol("test.q3_disj_multi.Tag.left_tag")
        .unwrap();
    let other_sym = kb
        .try_resolve_symbol("test.q3_disj_multi.Tag.other_tag")
        .unwrap();
    let right_sym = kb
        .try_resolve_symbol("test.q3_disj_multi.Tag.right_tag")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    ...
    let multi_left = entity_named(
        conj_sym,
        vec![(left_field, pq(left_sym)), (right_field, pq(other_sym))],
    );
    let query = entity_named(
        disj_sym,
        vec![(left_field, multi_left), (right_field, pq(right_sym))],
    );
...
fn q3_disjunction_empty_branch_is_not_yet_implemented() {
    ...
    let query = entity_named(
        disj_sym,
        vec![(left_field, empty_q), (right_field, some_pattern)],
    );
...
"#,
    ...
    let left_sym = kb
        .try_resolve_symbol("test.q3_neg_multi.Tag.left_tag")
        .unwrap();
    let right_sym = kb
        .try_resolve_symbol("test.q3_neg_multi.Tag.right_tag")
        .unwrap();
    let probe_sym = kb
        .try_resolve_symbol("test.q3_neg_multi.Tag.probe")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    let neg_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.negation")
        .unwrap();
    let conj_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.conjunction")
        .unwrap();
    ...
    let outer = entity_named(
        conj_sym,
        vec![(left_field, pq(probe_sym, var_p)), (right_field, negated)],
    );
...
"#,
    ...
    let left_sym = kb
        .try_resolve_symbol("test.q3_synth_memo.Tag.left_tag")
        .unwrap();
    let other_sym = kb
        .try_resolve_symbol("test.q3_synth_memo.Tag.other_tag")
        .unwrap();
    let right_sym = kb
        .try_resolve_symbol("test.q3_synth_memo.Tag.right_tag")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    ...
    assert_eq!(
        run_once(&mut kb),
        expected,
        "sanity: first run yields alpha+beta"
    );
...
"#,
    ...
    let left_sym = kb
        .try_resolve_symbol("test.q3_synth_share.Tag.left_tag")
        .unwrap();
    let other_sym = kb
        .try_resolve_symbol("test.q3_synth_share.Tag.other_tag")
        .unwrap();
    let right_sym = kb
        .try_resolve_symbol("test.q3_synth_share.Tag.right_tag")
        .unwrap();
    let pattern_query_sym = kb
        .try_resolve_symbol("anthill.reflect.LogicalQuery.pattern_query")
        .unwrap();
    ...
    let run = |kb: &mut anthill_core::kb::KnowledgeBase,
               a: TermId,
               b: TermId,
               aid: VarId|
     -> std::collections::HashSet<String> {
        ...
        let query = entity_named(
            disj_sym,
            vec![(left_field, conj), (right_field, pq(right_sym, a))],
        );
    ...
    assert!(
        shared.contains("gamma") && !shared.contains("alpha"),
        "shared ?v: conj unsatisfiable (alpha≠beta), only right branch; saw {shared:?}"
    );
    assert!(
        unshared.contains("alpha") && unshared.contains("gamma"),
