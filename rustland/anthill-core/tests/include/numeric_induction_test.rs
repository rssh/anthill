...
fn int_induction_loads_with_base_and_step() {
    ...
    assert_eq!(
        body.len(),
        2,
        "Int64.induction should have 2 body goals (base + step), got {}",
        body.len()
    );
    ...
    assert!(
        printed.contains("ho_apply"),
        "step must apply ?P: {printed}"
    );
    assert!(
        printed.contains("add"),
        "step must reach n+1 via add: {printed}"
    );
}
...
fn bigint_induction_loads_with_base_and_step() {
    ...
    assert_eq!(
        body.len(),
        2,
        "BigInt.induction should have 2 body goals, got {}",
        body.len()
    );
    ...
    assert!(
        ...
        "strong-induction step must reference sub: {printed}"
    );
}
