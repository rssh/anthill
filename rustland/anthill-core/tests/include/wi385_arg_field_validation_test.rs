...
//! only), so `f(x: Int)` called `f("hello")` and `entity Counter(n: Int)` with
...
"#;
    ...
    assert!(
        !errs.is_empty(),
        "f(\"hello\") with f(x: Int) must be rejected (was a silent hole)"
    );
    assert!(
        text.contains("Int") && text.contains("String"),
        "error should name the Int/String mismatch: {text}"
    );
}
...
"#;
    ...
    assert!(
        !errs.is_empty(),
        "Counter(n: \"hello\") with entity Counter(n: Int) must be rejected"
    );
    assert!(
        text.contains("Int") && text.contains("String"),
        "error should name the Int/String mismatch: {text}"
...
"#;
    ...
    assert!(
        errs.is_empty(),
        "correct Int arg/field must load clean: {}",
        errors_text(&errs)
    );
}
...
"#;
    ...
    assert!(
        errs.is_empty(),
        "bare Int in an Option[Int] field is accepted (Option-wrapping interim): {}",
        errors_text(&errs)
    );
}
...
// arm, so a parameterized carrier (List[Int]) against a bare spec (Stream) it
...
namespace test.wi385.provider
  ...
  operation feed(xs: List[T = Int]) -> Stream[T = Int, E = {}] = iterator(xs)
end
"#;
    let errs = try_load(src);
    eprintln!(
    ...
    assert!(
        errs.is_empty(),
        "List[Int] (provides Stream) passed to a bare Stream param must be accepted: {}",
        errors_text(&errs)
    );
}
...
"#;
    ...
    assert!(
        errs.is_empty(),
        "a List[Int] (provides Stream) in a bare Stream field must be accepted: {}",
        errors_text(&errs)
    );
}
