...
//! of the then / else types and an `Int`/`String` clash is rejected; in
...
/// branches return incompatible types — `Int` vs `String`, with no
...
namespace test.if_cheat.synth
  import anthill.prelude.{Int, String, Bool}

  sort Driver
    operation pick(c: Bool) -> Int =
...
"#;
    ...
    assert!(
        !errs.is_empty(),
        "unsound: if with incompatible branches (Int vs String) loaded clean — \
         the else branch was never type-checked (then-branch-wins)",
    );
}

/// An `if` placed directly as an operation body, where the declared
/// return type `Int` flows in as the if's expected type. Before WI-287
/// the `IfExpr` frame dropped `expected` entirely and the op-body return
/// check only saw the then-branch (`Int`), so the `String` else branch
...
"#;
    ...
    assert!(
        !errs.is_empty(),
        "unsound: if-as-op-body with a String else branch loaded clean against an Int \
...
/// over-rejecting well-typed `if`s (both arms `Int`, joining to `Int`,
/// which satisfies the `Int` return type).
#[test]
...
"#;
    ...
    assert!(
        errs.is_empty(),
        "well-typed if (both branches Int) was wrongly rejected: {errs:?}",
    );
}

/// Bare-vs-parameterized branches (`none()` : `Option` vs `some(1)` :
/// `Option[T=Int]`) must join cleanly in BOTH orders. These are mutually
...
namespace test.if_optjoin.a
  import anthill.prelude.{Int, Bool, Option}

  sort Driver
    operation pick(c: Bool) -> Option[T = Int] =
