...
pub fn emit_tactic_expr(symbols: &SymbolTable, tactic: &Tactic) -> Option<String> {
    match tactic {
        Tactic::Bare(sym) => {
            ...
            if is_default_smt(symbols, name, &[]) {
                None
            } else {
                Some(name.to_string())
            }
        ...
        Tactic::App(sym, args) => {
            let name = symbols.local_name(*sym);
            if is_default_smt(symbols, name, args) {
                return None;
            }
...
pub fn emit_tactic_from_term(kb: &KnowledgeBase, term: TermId) -> Option<String> {
    let (name, arg_pairs) = match kb.get_term(term) {
        Term::Ident(sym) | Term::Ref(sym) => (short_name(kb.qualified_name_of(*sym)), Vec::new()),
        Term::Fn {
            functor,
            pos_args,
            named_args,
    ...
    if is_default_smt_term(kb, name, &arg_pairs) {
        return None;
    }
...
fn emit_term_force(kb: &KnowledgeBase, term: TermId) -> String {
    match kb.get_term(term) {
        Term::Ident(sym) | Term::Ref(sym) => short_name(kb.qualified_name_of(*sym)).to_string(),
        Term::Fn {
            functor,
            pos_args,
            named_args,
...
fn is_default_smt_term(
    ...
    arg_pairs.iter().all(|(name_opt, _)| match name_opt {
        Some(sym) => matches!(short_name(kb.qualified_name_of(*sym)), "logic" | "timeout"),
        None => false,
    })
}
...
fn emit_app_term(kb: &KnowledgeBase, name: &str, arg_pairs: &[(Option<Symbol>, TermId)]) -> String {
    match name {
...
fn emit_repeat_term(kb: &KnowledgeBase, arg_pairs: &[(Option<Symbol>, TermId)]) -> String {
...
fn emit_using_params_term(
    ...
    let kv: Vec<(&str, String)> = arg_pairs
        .iter()
        .filter_map(|(name_opt, t)| {
    ...
    fn then_combinator_serialises_as_sexp() {
        ...
        let t = Tactic::App(
            ...
            vec![
                TacticArg {
                    name: None,
                    value: TacticArgValue::Tactic(Box::new(Tactic::Bare(simp))),
                },
                TacticArg {
                    name: None,
                    value: TacticArgValue::Tactic(Box::new(Tactic::Bare(smt))),
                },
            ],
        );
        assert_eq!(emit_tactic_expr(&s, &t), Some("(then simplify smt)".into()));
    ...
    fn or_else_combinator_uses_dashed_keyword() {
        ...
        let t = Tactic::App(
            ...
            vec![
                ...
                TacticArg {
                    name: None,
                    value: TacticArgValue::Tactic(Box::new(Tactic::Bare(qe))),
                },
            ],
        );
    ...
    fn nested_combinators() {
        ...
        let outer = Tactic::App(
            ...
            vec![
                ...
                TacticArg {
                    name: None,
                    value: TacticArgValue::Tactic(Box::new(inner)),
                },
            ],
        );
        assert_eq!(
            emit_tactic_expr(&s, &outer),
