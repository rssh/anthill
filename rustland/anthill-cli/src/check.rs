...
fn check_one_record(
    ...
    let result_tid = get_named_arg(kb, named, "result")?;
    let result_status = read_obligation_status_label(kb, result_tid);
    if result_status != "Discharged" {
        return Some(CheckOutcome {
            rule_qn,
            status: CheckStatus::Skipped(format!("status = {result_status}")),
        });
    }
...
fn read_obligation_status_label(kb: &KnowledgeBase, tid: TermId) -> String {
