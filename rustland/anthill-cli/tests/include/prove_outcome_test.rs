...
fn sat_proof_with_model_flag_emits_get_model_in_smt() {
    ...
    assert!(
        stdout.contains("(set-option :produce-models true)"),
        "model: true must wire :produce-models into preamble: {stdout}"
    );
    assert!(
        stdout.contains("(get-model)"),
...
fn unsat_proof_with_cores_flag_emits_get_unsat_core() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("(set-option :produce-unsat-cores true)"),
        "cores: true must wire :produce-unsat-cores into preamble: {stdout}"
    );
    assert!(
        stdout.contains("(get-unsat-core)"),
...
fn no_outcome_flags_keeps_legacy_smt() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        !stdout.contains("(set-option :produce-models"),
...
fn sat_verdict_with_model_populates_cli_output() {
    ...
    assert!(
        stdout.contains("COUNTEREXAMPLE") || stdout.contains("disproved"),
        "expected counterexample verdict, got:\n{stdout}"
    );
    assert!(
        stdout.contains("model:") && stdout.contains("bindings"),
...
fn cache_entry_carries_model_text() {
    ...
    let cache_dir =
        std::env::temp_dir().join(format!("anthill-outcome-cache-{}", std::process::id()));
    ...
    let assignments = json
        .get("variable_assignments")
        .and_then(|v| v.as_array())
        .cloned()
        .unwrap_or_default();
    assert!(
        model_text.contains("define-fun") || !assignments.is_empty(),
