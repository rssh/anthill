...
fn write_temp(name: &str, contents: &str) -> PathBuf {
    let dir =
        std::env::temp_dir().join(format!("anthill-tac-test-{}-{}", std::process::id(), name));
...
fn explicit_smt_with_random_seed_emits_using_params() {
    ...
    assert!(
        dump.contains("(check-sat-using (using-params smt :random_seed 42))"),
...
fn then_combinator_emits_sexp() {
    ...
    assert!(
        dump.contains("(check-sat-using (then simplify smt))"),
...
fn or_else_uses_dashed_keyword() {
    ...
    assert!(
        dump.contains("(check-sat-using (or-else simplify smt))"),
...
fn legacy_lf1_proofs_unchanged() {
    ...
    assert!(
        !stdout.contains("(check-sat-using"),
