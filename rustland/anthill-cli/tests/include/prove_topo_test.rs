...
fn write_temp(name: &str, contents: &str) -> PathBuf {
    let dir =
        std::env::temp_dir().join(format!("anthill-topo-test-{}-{}", std::process::id(), name));
...
fn cite_chain_discharges_in_dependency_order_not_alphabetical() {
    ...
    assert!(
        stdout.contains("test.topo.zzz_lemma: proved"),
        "lemma must discharge:\n{stdout}"
    );
    assert!(
        stdout.contains("test.topo.aaa_consumer: proved"),
