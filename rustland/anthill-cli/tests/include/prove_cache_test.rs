...
fn cache_hit_on_second_run() {
    ...
    let cache_dir = std::env::temp_dir().join(format!("anthill-cache-hit-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);

    let (ok1, out1) = run_prove(path.to_str().unwrap(), &cache_dir, &[]);
    assert!(ok1, "first run failed: {out1}");
    assert!(
        !out1.contains("cache hit"),
...
fn cache_invalidates_on_transitive_fact_edit() {
    ...
    let cache_dir =
        std::env::temp_dir().join(format!("anthill-cache-inval-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);
    ...
    assert!(
        out2.contains("cache hit"),
        "second run before edit must hit"
    );
...
fn no_cache_flag_bypasses_lookup_and_write() {
    ...
    let cache_dir =
        std::env::temp_dir().join(format!("anthill-cache-bypass-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);
    ...
    assert!(
        !projects_dir.exists() || projects_dir.read_dir().map(|i| i.count()).unwrap_or(0) == 0,
...
fn show_cache_lists_entries() {
    ...
    let cache_dir = std::env::temp_dir().join(format!("anthill-cache-show-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);
    ...
    let out = Command::new(ANTHILL_BIN)
        ...
        .arg(&cache_dir)
        .output()
        .expect("run anthill prove --show-cache");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(out.status.success(), "show-cache failed: {stdout}");
    assert!(
        stdout.contains("entries"),
        "expected entries summary: {stdout}"
    );
    assert!(
        stdout.contains("proved"),
...
fn gc_cache_removes_old_entries() {
    ...
    let cache_dir = std::env::temp_dir().join(format!("anthill-cache-gc-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);
    run_prove(path.to_str().unwrap(), &cache_dir, &[]);
    ...
    let out = Command::new(ANTHILL_BIN)
        ...
        .arg(&cache_dir)
        .output()
        .expect("run anthill prove --gc-cache 0");
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn refresh_cache_forces_resolver_rerun_then_writes() {
    ...
    let cache_dir =
        std::env::temp_dir().join(format!("anthill-cache-refresh-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&cache_dir);
