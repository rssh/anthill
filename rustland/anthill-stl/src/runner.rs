...
///     `Display` drops the payload, so it is rendered through
///     `anthill_core::eval::render_raised_payload`, which needs `kb` to resolve
///     functor/field symbols to names — and which a compile-time macro's raise
///     now shares, WI-757.)
...
pub fn exit_code_from_main(kb: &KnowledgeBase, result: Result<Value, EvalError>) -> i32 {
    match result {
        ...
        Err(EvalError::Raised { payload }) => {
            eprintln!("error: {}", render_raised_payload(kb, &payload));
            EXIT_RUNTIME
        }
        Err(e) => {
            eprintln!("error: {e}");
