...
### 2026-09-13T21:04:46Z — feedback — user

SHAPE OF PART (b), agreed with the user 2026-09-14. RENDER THE CANDIDATE'S VIEW OF THE KB, NOT lib/ SOURCE. (1) the spec's header exactly as the refinement check reads it (OperationInfo: params, return, ensures, effects); (2) the transitive closure of types named in the spec's and the tools' signatures, through entity fields, stopping at the prelude — for Triage that includes `mentions_all`, which a candidate must restate; (3) ONLY WHAT A CANDIDATE CAN NAME: `internal` constructors omitted, their sorts shown opaque with their public doors; (4) documentation from `{< >}` DescriptionInfo ONLY, never `--` comments — so what the generator may see is an authoring decision, and the checker's gap catalogue (C13, the gate's source, good.anthill's `risky` gate described in spec.anthill comments) stops leaking. WHERE: an anthill `describe(spec: Symbol, tools: List[Symbol]) -> String` over reflect (KB.operations / fields / descriptions, term_to_string); the host binding only assembles and VOUCHES, since `Text.trusted`'s `approved` cannot discharge a run-time string. `tools` becomes `List[Symbol]`. BLOCKERS: WI-20260913-2858G (the host calling `describe` from inside `attempt` is the re-entrant call); reflect exposes no `internal` visibility (intern.rs has `is_internal`) — to be filed. MEASUREMENT: every name good.anthill references appears; no internal name appears; an extra loaded operation named in tools appears; CONTROL — editing a `--` comment in lib/ leaves the prompt byte-identical while editing a `{< >}` block changes it (fails today); the prompt never mentions risky/GoodTriage/conceal; then rerun the live loop and compare acceptance, rounds and tokens with the source-dump runs.
