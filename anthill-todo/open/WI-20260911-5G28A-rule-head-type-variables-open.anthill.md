...
### 2026-09-24T15:24:31Z — feedback — user

ROUTE SETTLED AGAIN, AND THE FIRST STEP MEASURED (claude, session with user, 2026-09-24).

TWO CHANNELS THE REMAINING-HALF PLANS NAME ARE GONE. WI-20260921-28TAT (c82df6cc) deleted Frame::type_args and the typer's set_resolved_type_args stamp (proposal 065 §1: a rigid is read at run time only through requires TypeValue[T = X]). The 06:17 plan's T2/E1 write and read that stamp; Decision C / 060-implementation §7.3's E1 and op-to-rule-requirement-channel.md §5.1 walk Frame::type_args. None of those steps can be built as written; §5.1's spec walk must pair through the requirement channel.

A TYPE-ONLY ROUTE WAS PROPOSED AND WITHDRAWN. With TypeValue derived for every sort (HXGXF), carrying the citation's pinned TYPE into the first activation and reading a rigid through TypeValue looked like a smaller Decision C. The user's objection decides against it: a type does not carry the caller's CHOICE of provider. kernel-language.md's requires section says so — 'a named slot is never recovered from a value', 'the provider is the only thing the type was being read for' — and an anonymous requires has no type to put a choice in. The type-domains route (dictionaries travel) stands.

THE DRIVING EXAMPLE (user): a rigid A passed WITH Ord[A] into a rule. MEASURED on HEAD: viaRule[A](x: A, y: A) requires WeakOrd[T = A] = cmp(x, y).head.c, with rule cmp(?a, ?b, ?c) :- ?d = require[WeakOrd[T]], WeakOrd.compare(?a, ?b, ?c), two rival Ord[Int64] witnesses beside Int64's own. direct[A = Int64, WeakOrd = Descending](1, 5) = 4 and [.. Ascending] = -4 (op->op: the dictionary travels); viaRule[.. Descending] = -1 and viaRule[.. Ascending] = -1 — WRONG, Int64's own ordering. A SILENT WRONG ANSWER at crossing 2: the citation drops the caller's dictionary and the clause re-derives at the value's type, where 058 §3.2's default rung takes the carrier's own provision (measured alone: ?d's impl is Int64, definite). A rule body cannot select a provider itself — the loader refuses the bracket there — so a passed dictionary is the only route. PINNED in wi_5g28a_rule_dictionary_test.rs (a_rule_citation_drops_the_callers_dictionary); crossing 2 flips it to 4 / -4.

ONE DECISION THE FLIP OWES: the supplied dictionary is CHECKED against the local derivation (WI-860), and here the local derivation is that DEFAULT pick. If the check treats a default among several as unique, Descending is refused instead of answering 4. WI-20260922-ATFGH's reading — a default answers 'which provider wins', not 'which one built this value' — says it must not veto.

THE GATE (08:43's first step) — PASSED, after one carrier fix. A dictionary derived in one clause rides a GENERATIVE call as an ordinary head argument, intact and element-specific (Dictionary(Dictionary(impl: Sum), impl: Wrap) / ..Prod..), and Dictionary.sub(?d, 0) projects the element's. On HEAD the projection FAILED — 'expected Dictionary, got Node', one conditional row — because the reflect ops' expect_dictionary read only the value carrier (from_value) and a head-passed dictionary arrives on the occurrence carrier; NAR1X fixed the same carrier for dispatch (from_view at dictionary_dispatch_target) but not there. Fixed: from_value, else from_view. The projection then crosses a second head and is read and CHECKED like any other (agree = Sum's 3; disagree = nothing). Rows and the measured back-out in wi_5g28a_rule_dictionary_test.rs.

PROPOSED NEXT (not started): crossing 2 for the driving example — the citation's implicit argument (the caller's WeakOrd[A] slot, FromScope), captured in the Value::Relation at build_relation_value, bound to the clause's require read at the first activation (op->rule §7 steps 1 and 4, citation edge only); then T1 (the typer reads the bracket) so Wrap[T = X] gives the edge its σ.

### 2026-09-24T15:37:24Z — feedback — user

REPLANNED (user, 2026-09-24): the remaining half is built on the REQUIREMENT CHANNEL. docs/design/060-implementation.md §7.3 is rewritten and carries the plan — S1 the typer reads the citation bracket (fixes defect (2)'s false refusals; an unconstrained parameter is refused per WI-270); S2 crossing 2 for requirement reads (a predicate's implicit-parameter sequence, the edge check at the citation, capture in Value::Relation, bind at the first activation — flips the Ord pin to 4 / -4); S3 the domain as evidence (apply_domain, the sweep's generated read for a variable-bearing bound, the enclosing sort's parameter per activation for relational clauses); S4 WT8WG's parameterised value face; S5 two defects demo2 needs that are not 5G28A's; S6 demo2 end to end; S7 rule->rule implicit arguments. Open decisions D1-D4 are listed there; D1 (the domain's evidence: SortDomain or HXGXF's TypeValue) is recommended TypeValue.

DEMO2 PINNED (wi_5g28a_rule_dictionary_test.rs §3): a generative citation Wrap[T = X].top whose clause enumerates and calls score() through the caller's chosen witness. Today its projected spelling is refused at load (expected Wrap[T = ?X], got Wrap[T = ?T]) and its count answers 0 where the answer is 1.

TWO DEFECTS FOUND DRIVING DEMO2, NOT 5G28A's, NO OWNER: (a) a rule-body operation call whose argument is still unbound answers no solutions SILENTLY instead of waiting — ?x <=> wrap(?v), Score.score(?v, 3) gives none, the argument bound first gives 3; (b) ?d = require[Score[T]] answers no solutions SILENTLY when the only provider is a WITNESS sort — without the require the call answers 3, and a carrier providing its own spec answers. Tickets to discuss with the user.

### 2026-09-24T15:47:32Z — feedback — user

D1 SETTLED (user, 2026-09-24): the requirement a generic operation writes to let a rule list X's values is SortDomain[T = X], as 060-typedomains designs it — its promise is 'X has a domain', so a sort without one is refused where it is passed. The TypeValue[T = X] shortcut recommended in the replan entry above is WITHDRAWN: it promises only that X is known at run time, which every sort satisfies. HXGXF's TypeValue derivation stays the template for deriving SortDomain's provisions. 060-implementation §7.3 updated.

### 2026-09-24T15:50:10Z — feedback — user

CORRECTION to the D1 entry above, after rereading proposal 060 with the user (2026-09-24). D1 is not a decision: 060 §2.2's last bullet already says an abstract T enumerates only if 'domain [is] the member of a kernel finiteness spec' whose dictionary channel carries that RELATION — so the requirement's member IS the sort's own domain (the relation Colour.domain already is), not a separate member relation. 060 §5 ('the dictionary never becomes a head argument') rules out 060-typedomains §4.1's member(?x, ?d) shape: a provider's domain receives its parameters' dictionaries as IMPLICIT arguments (S2's channel). D2 is answered by 060 §4 (a supplied dictionary decides in the 058 §3.3 named-instance case); what is left is that today's local derivation reports the default pick as definite. String's domain EXISTS and is infinite (user) — nothing derives it today (measured: domain_member(?x, String) residualizes on domain_leaf), which is a gap in the derivation, like WI-20260910-5TK6B for Bool, not a sort without a domain. 060-implementation §7.3 updated for all of this.

### 2026-09-24T15:56:37Z — feedback — user

S3 RESTATED (user, 2026-09-24): the EXISTING domain scheme changes. A type variable's domain must be threaded through a requirement slot, and the type-indexed scheme (WI-743's domain_member(?x, T), whose parameterised clauses take the element's domain from a TYPE argument) cannot take one — so the domain moves onto the requirement channel for every sort, not beside the old scheme for variables only: the kernel spec whose member is domain (060 §2.2), provisions derived per sort with a domain (conditional per parameter, the element's domain from the condition's slot, never a head argument — 060 §5), the typed-head sweep reading a bound's domain through the requirement. The reason for threading is the RIGID type variable (a concrete citation's dictionary is built at the edge). Census and control (the classic-mini examples, wi743, WT8WG keep their answers) in 060-implementation §7.3 S3; §7 now points there.

### 2026-09-24T16:45:51Z — feedback — user

S1 DELIVERED (060-implementation §7.3): the typer reads a rule citation's sort parameters. Per citation, each parameter of the relation's enclosing sort that a column type mentions opens as a fresh variable (open_citation_params — the per-call instantiation an operation's parameters always had), seeded from the receiver bracket (receiver_bracket_entries, now the one reader shared with seed_receiver_type_args) and, in a member of the same sort, the enclosing instance (WI-424's sibling rule); applied arguments and the expected type reach it through σ; a bracket value and the expected type are read in the body's own terms (body_rigids), so a rigid X there is that rigid. A parameter still free is refused AT THE CITATION — TypeError::UnconstrainedCitationParam, naming the Sort[P = …].rel spelling. Two departures from the plan, both measured: the refusal is LOCAL like WI-270's for an operation call (Option.none().isEmpty() is refused the same way) rather than held to the walk's end; and a bracket that fills nothing is not refused (D4 open). A rule body builds no CitationSite and is unchanged. FLIPPED: the three false refusals load (demo2's topAt among them) and Wrap.dom.takeN(5) is a load error; nothing else in the workspace moved (7550 passed). EVAL IS UNTOUCHED: a citation still flounders even with its value bound, because the clause's bound carries the sort's shared T — S3(d), pinned in wi_5g28a_citation_bracket_test.rs. /code-review (high): 6 findings — 3 fixed (a bracket on a non-owner sort now refused, the bind invariant a hard assert, the rigid map built once), 2 measured not to occur (a supertype expected type was refused before S1 too; a constraint citation loads unchanged), 1 skipped (the row-lambda hint path keeps the old reading; it has no environment). kernel-language.md's rule-half paragraph updated with the user.

### 2026-09-24T19:11:18Z — feedback — user

S2 DELIVERED (060-implementation §7.3): a rule citation passes the caller's requirement dictionaries to the clause's require reads. THE EDGE CHECK runs after the sweep (PendingCitationRoutes, settle_citation_routes): a read becomes routable only once record_find_dictionary_grounding picks its witness, which needs the rule bodies typed — measured, at the citation cmp's read was still the one-argument form. Per read, the witness arguments are typed from the citation's columns under S1's σ, the goal built by the resolver's own witness_sort_goal / anchor_sort_goal, resolved against the caller's frame chain, and emitted onto the citation's op_dicts. CAPTURE: build_relation_value evaluates the routes in the citing frame and wraps the goal atom (__within_requirements) — no Value::Relation field (D3 decided narrow). BIND: step_init unwraps the marker, step_choice_point binds each opened clause's reads by position (bind_citation_reads); a read that arrives bound derives on the Unranked rung, so rivals leave it standing and a unique row must still agree (D2, 060 §4). FOUND, NOT PLANNED: the weave had to cover a carrier-bearing body-less call — AT ITS DICTIONARY'S CARRIER (the witness's carrier argument, or the anchored head variable); carrier-blind it answered 7, 7 for (thing(), gadget()) and refused HRFR5's typed pair. FLIPPED: Ord (-1, -1) -> (4, -4) in both spellings of the read; a returned relation keeps its dictionary (4); Rank 30 / 20 through the passed dictionary; demo2's silent 0 and empty .head -> relation_floundered. NOT BUILT: the plan's two refusals, and the route of a read only its BRACKET decides (demo2's Score[T], witness a body variable) — observable only after S3(d). FOUND, NOT S2's: the BODIED weave is carrier-blind — a defaulted describe answers 9, 9 for (thing(), gadget()) where 7, 9 is right, before and after S2 — filed as WI-20260924-GP8JC. Full workspace 7555 passed. Six back-out axes, each alone over wi_tests: routing, capture, bind and the supplied rung each redden the same 4 rows; the weave's old gate those 4 and both demo2 pins; the carrier direction 2 (its own row and HRFR5's typed pair). /code-review (high): 7 findings, 3 fixed, 4 skipped. kernel-language.md's two paragraphs (the rule half; require[X]'s covered calls) updated with the user. Commit 1901197e.

### 2026-09-24T21:08:26Z — feedback — user

S3a DELIVERED (060-implementation §7.3; S3 split into S3a / S3b / S3c): a TYPE VARIABLE's domain rides the requirement channel. anthill.reflect.SortDomain { sort T = ?; rule domain(?x) } — member declared untyped; provisions derived per sort with a domain (kb::sort_domain_derive, HXGXF's conditional shape) behind a demand gate (measured ~55 ms/load debug otherwise: a written SortDomain requirement or a type-variable clause bound opens it; the stdlib has neither). The typed-head sweep gives a type-variable bound an implicit read find_dictionary(SortDomain[T = <bound>], SortDomain, ?x, out: ?d) plus apply_domain(?d, ?x) — a new kernel builtin step_init lowers to the provider's domain goal once ?d is bound, delaying while it is not; a citation fills the read (S2), mode (in) derives it from the value. A bound naming a non-parametric sort calls S.domain(?x) statically (same answers; S.domain's own clause keeps domain_member as the bottom; a declined value face is read from the loader's record — measured, a 3-ary author domain otherwise swallowed the sort's rows). S3(d): a relational clause's enclosing-sort parameter joins its frame (equations keep the shared one); relation_clause_columns keeps the slot canonical at a citation and now builds the frame in De Bruijn order (in position order every S1 bracket refusal loaded clean). FLIPPED: countAt[X] requires SortDomain[T = X] = Wrap[T = X].el.takeN(5).length() -> 3 (Colour) / 2 (Letter); S1's pin Wrap.dom(wrap(red())) -> 1. NOT YET: parameterised bounds (S3b, which absorbs S4 — the derived List.domain reads its own dictionary and recurses through apply_domain); domain_member's fate (S3c). Full workspace 7560 passed; five back-out axes measured (header of wi_5g28a_sort_domain_test.rs); /code-review: 1 fixed, 3 skipped. kernel-language.md's domain section and rule-engine paragraph updated with the user. Commit 1877fd04.
