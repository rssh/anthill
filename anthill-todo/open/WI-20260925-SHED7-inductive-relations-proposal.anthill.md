...
### 2026-09-26T00:08:19Z — feedback — user

CODE-REVIEW ROUND 3, SHED7's side (2026-09-26, not committed). The typed head's type test is its own internal builtin, `__domain_guard` (`BuiltinTag::TypeDomainGuard`), not an `early:` label on `domain`: a written `domain(?x, Int64, early: false)` stood aside and passed a `String`, and a written `domain` with a stray operand is now a located error. The guard WAITS on an unbound or partly bound `?x` — standing aside, as round 2 did, let rotation re-ask a cut body's faulting `add(2.5, 1, ?y)` before the appended fill once a later goal bound `?x` — and passes only a GROUND value the closed reading cannot type (the fill owns it; two rows measure the pass). The typed-head fill skips the second, open read and the σ copy when the guard already pinned the bound (conformance asked of the closed type). `dealias_type` asks `alias_targets` before the pre-index alias scan: `derive_sort_domains` 14.8 → 7.5 ms per load (debug, 3 runs each). Rows for the three dealias sites that had none: the typed-head bound, the citation route, and the resolver's arm (reachable from a written `apply_domain(Hue, ?x)`). NOT CHANGED, decided by measurement: an UNWRITTEN type argument stays ONE type pinned by the first value (`Unpinned`) — the typer reads it so and refuses `duo(x: circle(…), y: square(…))` in an operation body ("expected Circle, got Square"); a row pins the agreement. FOR THE REWORK: the `Unpinned` read reads the value OPEN and so shares the typed-head pin path's per-step term-store growth (the transient pin path end to end fixes both). Back-outs re-measured over the domain suites: the guard not prepended 3 rows, its wait 1, its ground pass 2; [P] 0; axis B's three pins together 11. Rows +6 in wi_shed7.
