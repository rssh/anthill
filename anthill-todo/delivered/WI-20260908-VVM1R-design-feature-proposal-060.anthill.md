...
### 2026-09-16T20:20:44Z — feedback — user

§8.9 MEASURED AGAINST HEAD 2026-09-16, both halves — the doc now carries the table (060-implementation §8.9). This corrects the ticket's own 'STILL OPEN' list: item (i), the check tier, was settled by §8.8 and delivered inside S2; item (ii)'s dichotomy was already replaced by §8.9's question, and that question is now answered for the CALLEE half.

I1 ≡ I3 IS UNCHANGED by S5 / S6 / S7, all three of which landed after the 09-09 table and all three on this surface. An enclosing sort's requires still has no effect on a clause inside it.

THE CALLEE HALF NEEDS NO RESTATEMENT, and that is not a change to make — it already works. A rule calling an operation that declares a requirement resolves it from the VALUE at resolution time: 'operation pick(x: Box) requires Desc[T = x.E]' called from 'rule a2(p: Box, ?r) :- pick(p, ?r)' answers 7 at a Box of Red and 9 at a Box of Blue with the rule body writing nothing — and with a BARE '?p' head as readily as a typed one, so it is not the anchor doing it. The operation→operation case is refused at load for the reason that distinction implies: a static site has no value to read. So the channel doc's 'the call site drives it; require[X] is only the explicit form' is already true on the rule→op edge, and 'duplicate the callee's requirement' is the wrong frame in both regimes — below the ambiguity it is redundant, above it (two suppliers) what is needed is a CHOICE of supplier, which is information the callee never had.

A DEFECT FELL OUT, and it is I1's disease one level over. Row I4: the same call through an operation declaring 'requires Desc[T = Plain]' where Plain provides NO Desc answers 1 — the spec's default body — where the identical demand written in the rule body DontFires and answers []. The declaration is ignored and the call produces a value. 'Plain provides no Desc' is decidable at load, so this wants a refusal at the declaration or at the call, never a folded default. Three spellings of one requirement over an unsatisfiable carrier, two ignored, and the two ignored are the two an author reaches for first. Not filed — discuss before opening.

WHAT REMAINS UNDER THIS UMBRELLA IS S6 (WI-20260909-S8CBV), claimed: gate (1) plus the attribution. Its park reason is GONE — it was parked on WI-20260909-C7ANM, which is delivered, as is WI-20260910-7NBZX — so scratchpad/s8cbv-gate1-attempt.diff needs a rebase off 276ceda3 rather than a re-derivation. The one design choice still open inside it is which equality the root attribution uses: S6's description says delegate to unify_types (δ then ζ/σ), its own delivery feedback reports that views_structurally_equal over the δ-eliminated spec sufficed at the OPERATION level because receivers are already caller-keyed by δ, and closes with 'whether the rule-head case can rely on that too is the first thing to measure, not to assume'.
