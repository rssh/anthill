## Attributes

- id: WI-20260830-E0JE6-rename-guardians-harness-to
- created: 2026-08-30T20:27:49Z

- status: Delivered
- status_agent: claude
- status_at: 2026-09-08T14:25:56Z

- acceptance: cargo-test, scaland-sbt-test

- tags: guardians

## Description

RENAME `guardians.Harness` TO `AgentGenerator` AND `guardians.Checker` TO `AgentChecker`, so the names say which of the two parties each one is. The ICTERI-2026 article now uses the new names (Listing 7); the example does not, so this is a divergence like the ones in WI-20260830-THZ8R.

WHY `Harness` IS THE WRONG NAME, and it is not only taste. The sort has two operations, `render_task` and `generate`: it renders a prompt and produces a candidate. That is a GENERATOR. "Harness" names the whole generate-and-check rig, which is what `attempt` and `open_round` are -- so the current name claims the loop while describing one half of it, and a reader looking at `attempt(h: Harness, llm: Llm, chk: Checker, ...)` cannot tell from the names why there are two parameters rather than one component with a helper.

MEASURED AS A READING FAILURE, not a hypothesis: a reader of the article's listing asked, in order, 'why is Checker not in Harness', 'I can't understand attempt outside of harness', and then proposed the merge. Two differently-named parties -- generator and checker -- answer that before any prose does.

SCOPE. `sort guardians.Harness` and `sort guardians.Checker` (lib/harness.anthill); the carriers `FileHarness` and `LoadChecker`; the signatures of `attempt` and `open_round`; the fixtures under fixtures/agent/ that name either sort (checker.anthill, rejected/{steering,frontier,minting}_checker.anthill, rejected/forged_llm.anthill); any `provides Checker[C = ...]` clause.

ACCEPTANCE: no live occurrence of `Harness` or bare `Checker` outside a comment recording the old name; the guardians suite green with every fixture still accepted or refused FOR ITS OWN REASON, diagnostic substrings unchanged.

## Changes

### 2026-09-08T14:25:42Z — feedback — user

SUPERSEDED BY WI-20260908-K5HVE, in two different ways for its two renames — recorded because only one of them is actually moot.

`Checker` -> `AgentChecker` IS MOOT. K5HVE deletes `guardians.Checker` and `guardians.LoadChecker` outright: measured, `provides Checker` does no measurement work (deleting `provides Checker[C = MintingChecker]` leaves that fixture's refusal byte-identical), `guardians_check` never reads `args[0]`, and no second checker is conceivable — it loads a candidate into a discardable layer and runs the gate, with no network and no strategy to vary. There is no sort left to rename.

`Harness` -> `AgentGenerator` IS NOT MOOT — it is ENTAILED, and K5HVE has taken it. This ticket's argument is that the sort has two operations, `render_task` and `generate`, and 'that is a GENERATOR'. Today that reading is contestable, since `check` sits beside it as a second sort a reader may count as part of the same rig. K5HVE moves `check` off the spec entirely, which makes the argument unconditional — so the restructure is what makes the name right, and renaming in a separate pass would be two edits to one declaration.

THE MEASUREMENT THIS TICKET CONTRIBUTED SURVIVES INTACT and is quoted in K5HVE: a reader of the article's Listing 7 asked, in order, 'why is Checker not in Harness', 'I can't understand attempt outside of harness', and then proposed the merge. That reader study is what put the interface question on the table at all; K5HVE answers it by making one of the two parties stop being a party, which is the stronger form of the same fix.

STALE, FOR ANYONE READING THE SCOPE LIST: it names `rejected/steering_checker.anthill`, which WI-20260829-MCKTE deleted along with `Permission[Reveal]` and the `LlmOutput` wrapper. Take the fixture list from the tree.

