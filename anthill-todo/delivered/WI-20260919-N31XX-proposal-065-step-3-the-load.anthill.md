...
### 2026-09-22T15:54:57Z — feedback — user

PROPOSAL 065 OPEN QUESTION 3 FIXED INLINE (2026-09-22, user: 'fix inline'). Not a new ticket: the change is one arm plus a guard, smaller than a ticket describing it would be.

WHAT IT WAS. OQ3 asked what to do about a former (tuple, arrow) that has no derived TypeValue instance, and PROPOSED refusing at the read. Reality was neither: 'operation ask() -> Type = tyOf((1, 2))' against 'tyOf[B](x: B) requires TypeValue[T = B]' LOADED CLEAN and then died 'DeferToRequirement: __req_typevalue not bound in caller frame' -- an EvalError::Internal that bridge_op_to_eval raises as a PANIC. Eight lines, reachable from checked source. Both existing arms answered None and neither was wrong to: unprovided_provision bails at sort_functor_of_view (a former has no declaration for a repair line to name) and caller_rigid_carrier bails because the carrier is ground, not a caller parameter. With both silent the slot fell into the silent-absence rule that exists for the 29 stdlib bodies which declare a chain and never read it -- and this body reads it.

WHAT LANDED. A third arm in build_op_scoped_dicts, asked LAST so a SORT carrier keeps unprovided_provision's repair line. Refused at the CALL, not the read: a body reading B cannot see that some caller will pass a tuple. Not keyed to TypeValue -- 3G1YT's lesson applied rather than re-learned.

ONE EXEMPTION, AND IT COST 20 RED ROWS. A BODY-LESS callee is excluded. Error.reify declares 'requires ErrorTag[T = T1]', is body-less on purpose (the boundary is a frame the interpreter installs by symbol), and a TUPLE payload is a live correct corpus program: wi_9wvt7's own fixture says 'this boundary CANNOT be narrowed and must catch wide'. The host narrows when evidence is there and catches wide when not, so the absence is the wide case, not a death. The guard asks only whether an anthill body EXISTS -- which decides WHO interprets the absence -- and is NOT the body WALK 3G1YT deleted, which asked what a body CONTAINS.

THE SORT HALF NEEDED NOTHING, measured and kept as a control: build_dispatching_dict_from_chain is all-or-nothing (require_complete drops an incomplete dictionary whole) where the op half is best-effort and per-slot, which is how one unfillable slot stayed silent.

SELF-REVIEW (/code-review high) FOUND TWO, BOTH MINE, BOTH FIXED. (1) THE MESSAGE STATED SOMETHING FALSE: 'no instance of {spec} can ever name it', advising the author to wrap the former in a sort. Tested rather than trusted, and it is wrong -- a former cannot CARRY a provision, but a SORT may carry one whose spec BINDING is a former: 'sort Wrapper ... provides TT[T = (a: Int64, b: String)]' loads and satisfies 'requires TT[T = B]' at that tuple. The verdict was still right (the arm fires only after the provider search fails) but the explanation conflated two things and the repair named a different program from the one that works. Rewritten in all four places that repeated it -- the message, former_carrier's doc, kernel-language.md and 065 -- and pinned by a new control row, the working program, which must keep loading. (2) Only the CARRIER parameter was inspected, so a multi-parameter spec with a former in a non-carrier binding fell back into the silent absence; widened to any ground binding.

SIX NEW ROWS: the refusal driven with its message and located at the call, plus five controls -- the sort-level spelling (already refused, green either way), a sort carrier keeping its own message (ordering), a sort carrier with an instance still answering (positive), the body-less exemption (back-out: 20 rows of wi_9wvt7), and the former-with-a-provision program (back-out: refusing on carrier shape alone).

MEASURED: Rust 7293/0 (7287 + exactly these six), scaland 614 total/0. PERFORMANCE CHECKED because a run looked slow: wi_tests 440.57s against a 441.95s pre-change baseline, noise. The slow run was CPU contention -- scaland run concurrently put the 15-minute load average at 35 on 24 cores, slowing every compute-bound binary a uniform 3.5-4.4x while the two crates in the second cargo invocation did not move at all.

STILL OPEN IN OQ3: the ARROW half. An arrow with a rigid effect row could not be driven to the same point -- a bare operation reference is refused earlier, for unrelated reasons -- so what is settled is the FORMER CARRIER, tuples measured. OQ4 (the Type interned per dispatch) is NOT fixed: user decided 2026-09-22 that the interning will be removed outright rather than worked around.
