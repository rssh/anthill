...
**The arity+1 view above does NOT yet mirror that, and the asymmetry is measured** (WI-20260830-NX4FD). Widening its effect clause the same way makes the hook FIRE and the answer no better: `size(?ls, ?n)` on `FiniteCollection` still answers nothing (the bridge then fails one level down on `FiniteCollection requires Iterable[C, Element, E]`, of which the argument pins only `C`), and `Iterable.isEmpty(?ls, ?r)` — whose chain *is* pinnable — answers ONE **definite** solution with `?r` still free, over two rows whose true answers differ. So the arity+1 view keeps the plain `effects.is_empty()` test until the requirement resolver can complete an open element in a chain's sort half.
