...
**Every call-site binding must land, and only an operation body may carry one** (WI-839). Within one bracket, each binding must reach a **distinct** declared type parameter of the callee. Four ways it can fail, all **load errors**: a named key naming no declared parameter — including when the callee declares *none at all*, and when its parameters are its enclosing **sort's** rather than its own (a call bracket does not bind a sort-level parameter; see proposal 058 §9 phase 2); the **same key twice**; a positional with no parameter **left** to bind (those a named key already took do not count as available); and a callee that is not an operation at all — a function value, an applied rule citation, an entity constructor — which has no type-parameter list for a bracket to bind.
...
*Where the ambiguity error is raised.* Most ties are refused at **load**, before anything runs; a carrier that provides a spec *itself*, beside a second sort providing the same spec for it, is not — that pair reaches the runtime. Such a tie is refused at the point a dictionary for it is actually **built**: when dispatch resolves an impl's `requires` slot and finds two providers, it raises an ambiguity error naming the requirement and both providers, rather than proceeding without the dictionary. Two limits are worth stating, since neither is a general "ambiguity is always caught": inside SLD resolution the same tie *delays* the bridged evaluation instead of aborting the enclosing rule, and choosing which impl runs for a spec-op call on a value is a separate step that still takes the first match. A requirement that is merely *unpinnable* at the argument types is a different case and is not an error at all — the call proceeds, and only a body that actually reads the missing slot fails (WI-822/WI-855).
