...
-- It declares NO operation: it reaches them by `provides`, so one `provides Numeric[T = C]`
...
**Infix and prefix operators** desugar to an application of a named operation — `a + b` to `Additive.add(a, b)`, `!a` to `not(a)`, etc. The full operator table is in §6.6. The prelude sorts above define the operations these operators desugar to; the operators are available when the corresponding sort is required (e.g. `requires Additive[T = Money]` for `+`, or `requires Numeric[T = Money]` for the whole arithmetic bundle). Three targets are **position-directed** — `not` / `or` / `and`, so `!`, `|` and `&`. Each is a resolver primitive over reified `Term` goals in a rule-body goal position (`anthill.kernel.not` negation-as-failure, `anthill.kernel.or` disjunction, `anthill.kernel.and` conjunction) and a dispatched `Bool` operation as a value expression (`Bool.not` / `Bool.or` / `Bool.and`) — see §6.6.

**An operator names its operation ABSOLUTELY, so it is not the same thing as writing the bare name** (WI-20260825-KD9SW). Eleven operators — `+ - * / %` and `= != < <= > >=` — desugar to the *address* of the spec operation they mean (`..anthill.prelude.Additive.add` and so on), not to the short name. (`parse::pratt` carries a twelfth entry, prefix `-` → `Additive.neg`, for the same reason the other eleven are there; §6.6 explains why no surface form reaches it — a prefix `-` on a non-literal collides with negative-literal lexing and is not provided, so negation of a value is written `neg(x)`.) The three boolean connectives `| & !` do the same (WI-20260825-P9Y67), differing only in **what** they address: a resolver primitive (`..anthill.kernel.or`), not a spec operation, because a connective dispatches on no carrier and there is no spec for it to be split out of. Their **value** reading is unaffected — the position routing of §6.6 runs on the resolved symbol, so an address at the goal spelling reaches `Bool.or` in an operation body exactly as the short name did. They differ from the eleven in one further way, and it is smaller than it looks (WI-20260826-XED22): their implicit-tier entries are **gone** as well, so a written bare `or(…)` / `and(…)` needs an import like any other name. A written bare `not(…)` needs none — not because of the tier, but because `not` is a **prefix operator**, so `not(x)` is minted with the address and never runs the name ladder at all. The consequences are worth stating plainly, because this is the one place where the sugar is not a source-level rewrite:

- **An operator cannot be captured.** A declaration or import that binds `add` in the enclosing scope changes what a written `add(a, b)` means and leaves `a + b` alone. Before this, `import myapp.Money.{add}` silently retargeted every `+` in the file.
- **It costs no polymorphism.** The address names the *spec* operation, which is what dispatches, so a carrier providing `Additive[T = Money]` still answers `money(700) + money(25)` through its own `add`.
- **Refactoring `a + b` to `add(a, b)` is therefore not always meaning-preserving**, and the reverse likewise. Where the bare name is in scope as something else, the two answer differently and nothing diagnoses it. Writing the operation's own name — imported, or qualified as `Additive.add(a, b)` — is the spelling that says what is meant.
- **A written bare name still resolves by scope**, so a file that calls one of these operations by name imports it (`import anthill.prelude.PartialOrd.{gt}`). There is no implicit tier behind them any more; the operator needs none because it carries its address.
...
**Instantiation as term:** The `Name[bindings]` syntax is valid both in type position and in term position. In term position, it is a sort instantiation as a first-class value — a term like any other, so it can be asserted, queried and bound:

```
-- An ORDINARY fact whose head is a sort instantiation: it enters the rule index and a
-- goal `Eq(T: ?q)` answers from it.
fact Eq[T = Int64]
```

This follows the "types are terms" principle: sort instantiations are knowledge, expressible as facts.

*It is not a spec claim.* "Int64 satisfies Eq" is a PROVISION and is written `provides Eq[T = Int64]` — in `Int64`'s own body, or in a `namespace Int64 … end` secondary entry at its address. The two were one construct until WI-20260917-S8JYF retired the `fact` spelling of a provision (058 §4); see §5.1.
...
```
-- Sort-level: Modify parameterized with any target, written in `Modify`'s own body
provides Effect[T = Modify[?]]
...
**A DECLARED ELEMENT TYPE CHECKS THE LITERAL; IT DOES NOT REPLACE IT** (WI-20260826-7JDWY). A position declares an element type when its expected type is headed by **the literal's own collection** — `List` for a `[…]`, `Set` for a `{…}`; the `T` of that type is then the literal's element type *and* every element is checked against it with the ordinary subtype relation, reported at the offending **element's** own span and position (`type mismatch in list.element 3 (collection-element): expected Int64, got String`). The declaring positions are an operation's return, an annotated `let`, an element of an enclosing literal, and an argument or field slot whose element type either names a **constructor** or is itself **callable** (`List[T = Function[A = Int64, B = Int64]]`, which is what lets a bare operation name inside a literal find the arrow to lift against). It used to be taken on trust: `operation mk() -> List[T = Int64] = ["x"]` loaded clean and `List.head(mk())` answered `Str("x")` out of a slot the signature types `Int64`.

The declaration remains the answer rather than a join over the elements, which is what lets `-> List[T = Colour] = [red(v: 1), blue(v: 2)]` type at the parent while `-> List[T = Colour.red] = [blue(v: 1)]` is refused naming `blue`. The **head** test is what keeps the blame in the right place: another sort's `T` is not an element type, so `-> Option[T = List[T = Int64]] = [1]` is refused naming the containers (`expected Option[T = List[T = Int64]], got List[T = Int64]`) and not an element. A **rival** collection is the same case — a `[…]` is always `List`-typed at this point, so a `Set[T = X]` position declares nothing about its elements and `-> Set[T = Int64] = [1, 2]` is the container mismatch it looks like. A bare value where the declaration says `Option[T]` is **reported**, not silently wrapped — a decision rather than a limitation: inserting the `some(…)` coercion other positions insert would be a new capability, while reporting replaces a program that loaded and left the element bare at runtime. A position declaring **nothing** takes the **join** of the elements' own types (WI-20260829-WBXGX) — the same `if`/`match` arm join, so one relation answers "several expressions, one type" wherever it is asked — and the first element with no join is refused at its own span, tagged `collection-element-join` so a reader can tell a computed element type from a written one. It used to be the **first** element's, with the rest unchecked, which showed as an order-dependence: `[1, "a"]` loaded in a `List[T = Int64]` slot and `["a", 1]` did not.
...
provides Effect[T = Modify[?]]   -- Modify is an effect kind, for any target
...
| a `rule`, or a `rule { … }` block entry | **allowed iff BOTH: (1) its head INTRODUCES — the head resolves to nothing in the finished program (§8.6's ladder, asked before any head is minted) — and (2) ONE ENTRY OWNS THE PREDICATE, every clause that LANDS on that predicate being written in this same entry — a `fact` counts, since `fact H` *is* `rule H :- true` (§6.1).** A clause lands where its head's functor RESOLVES, which is not always the scope it is written *at*: a qualified head (`fact Rec.p(2)` / `rule Rec.p(2) :- true` in the main entry) introduces nothing and still lands its clause on `Rec.p`; a host `provides Rec language L … end` block's clauses resolve in the SPEC's scope (§10.2) however far from `Rec` the block itself is written; and a LABELED multi-head rule (`rule law: p(2), q(1) :- B`) introduces nothing — it names no single predicate — while each of its heads lands its own clause, since it desugars to one stored clause per head (§5.3). A clause may also reach the predicate through a SELECTIVE PREDICATE IMPORT (`import X.Rec.{p}`), which resolves like any other reference. A clause in an ENCLOSING namespace does not land inward — nothing resolves inward on its own — but an `import X.Rec.*` written there makes the name resolve inward and it lands like any other. **Written either way, it is the same clause** (WI-20260821-RDGQC): a `fact` head declares its predicate at the scope it is written in, exactly as the `rule H :- true` spelling of it does, so an enclosing `fact p(2)` beside a `Rec` that declares `p` is two scopes introducing one name and is refused by §"A rule-introduced functor is scoped where it is written". The two spellings used to differ here — a fact head declared nothing, fell to one global intern, and so became a separate predicate by accident where the `rule` spelling collided — which made one clause into two programs depending on the keyword. POSITION is not the property; RESOLVING is. **Whose entry a clause belongs to is a question about the TEXT it sits in**, not about the scope it resolves in: the type's own declaration (the main entry, nested `sort` bodies included) or a `namespace` block at its address. An ordinary `namespace Rec.Helper` is neither, though its name begins with the type's. A definition of something new displaces nothing; a clause that JOINS an existing predicate is non-monotone and can falsify a statement already proved. Condition (1) also excludes, without a clause of its own, a dot rule and an operator rule (whose functor is the desugar's `dot_apply` / `add`), a qualified head, and a multi-head rule — none of which introduces anything, so no `@[simp]` equation written here can ever rewrite a call the entry does not own. Two `namespace X` blocks in ONE file are ONE entry and compose freely |
...
| any `fact` | **refused** — a secondary entry adds MEMBERS and SPEC CLAIMS, and a `fact` is neither. Every spec claim an entry can make is written `provides Spec[…]` — including one whose carrier is some OTHER sort, which is a witness claim and so genuinely about this sort (it supplies the dictionary). The refusal predates the retirement and its *reason* has changed with it: it used to be default-deny against a `fact Spec[X]` that could not be told from an ordinary fact over a parameterized DATA sort, and there is now nothing to tell apart, a `fact` being an ordinary fact wherever it stands. What the retirement removed is the escape the row used to offer — `fact Spec[Carrier]` one level out is no longer a spec claim either, and an entry at the carrier's own address is the spelling at an address a type occupies but its declaration does not. An ordinary assertion with a fresh head is written as the rule it desugars to, `rule h :- true`, which the row above admits |
...
**`provides` IS THE ONE SPELLING OF A PROVISION, AND A `fact` IS AN ORDINARY FACT**
(WI-20260917-S8JYF, completing 058 §4's retirement at both levels). A provision names
its PROVIDER by where it is written and its CARRIER by its bindings, so it is written
in the carrier's own `sort`/`enum` body, or — where the carrier has no body to write it
in (a `sort X = ?` alias, §6.3's free-standing `entity X(…)`, a type declared in
another file) — in a `namespace X … end` SECONDARY ENTRY at its address (059 R2/R3). A
`provides` clause is admitted inside a proposal-038 `provides <Carrier> language <L> …
end` binding block too, since that block opens the carrier's scope.

*What the `fact` spelling used to be.* Inside a sort body `fact Spec[…]` recorded the
same PROVISION `provides Spec[…]` records; at an address no type occupied it derived
its carrier from a binding VALUE, which was the only way to claim a provision for a
carrier declared elsewhere. Both readings are gone, and with them the language's one
construct whose meaning depended on its container. The secondary entry is what replaced
the second one: `namespace Carrier … provides Spec[…] … end` emits byte-identically to
what `fact Spec[Carrier]` emitted, and it says the carrier by ADDRESS rather than by
derivation — so the swap is not a bare keyword rename, and text that kept a positional
carrier after the keyword would record a different provision.

**A `fact` is an ordinary fact wherever it stands, and that is what closes a silence
no diagnostic could.** A `fact Spec[…]` was a provision only when `Spec` RESOLVED to a
sort; with the spec unimported, the identical text minted an ordinary fact-only
predicate, emitting no provision and no warning. The two readings were undecidable from
the source — `sort Rec { fact helper(1) }` is a legitimate fact-only predicate, and
`fact Box` has the same shape at every level, CST included — so the repair could not be
a diagnostic. With one reading left there is nothing to tell apart: the author who
means a provision writes `provides`, which refuses an unresolved spec loudly.

**No diagnostic replaces the second reading, and none can.** A bracketed head whose
functor names no declared sort is *not* refused: §"Name Resolution" makes a clause head
a DECLARATION, so `fact myrel[T = Red]` introduces `myrel` and the bracket binds a type
argument on it — byte-identical to the missing-import shape, since §2.3 gives
capitalization no meaning. The three sibling positions that *do* report a bracketed
application over an unresolved name (a rule-body goal, a fact data slot, a type
annotation) are REFERENCE positions, where a name must resolve. A gate here was built
and removed on that measurement.

*Four readers were keyed on the `fact` spelling alone*, each found by migrating the
tree and measuring, not by reading. They are why the retirement is a change to READERS
and not only to the loader:

- `region_sorts` / `is_modifiable_sort` scanned raw `Modifiable[T = …]` facts, so
  `is_modifiable(Cell)` answered **false** on a migrated tree — a wrong answer, not an
  error. WI-862 made them read both channels; the retirement drops the raw-fact leg,
  since an ordinary fact claims nothing.
- Rust codegen rendered an in-sort claim as a supertrait bound from the `fact` arm
  only, silently dropping `trait NonMonotonicStore: Store`. It reads `provides` now,
  and reads a secondary entry's claims as the entry sort's own.
- A namespace-level `fact` became the `impl <Trait> for <Carrier>` marker, taking the
  carrier from its brackets. The marker comes from the secondary entry's address.
...
*A fact is still a rule with an empty body (§5.3), so it still enters the rule index* —
and that is the one thing the retirement genuinely costs. The goal `Spec(T: ?q)`
answered under the `fact` spelling and does not answer under `provides`. Where some
rule resolves `Spec[…]` as a GOAL, **keep the fact and write the provision beside it**:
the two were never the same statement, and only one of them was a provision.
...
**A sort with constructors is a DATA sort, and nothing is-a one** (WI-407/WI-1106).
A `provides` clause naming one is **refused at load**, naming the spec, the provider
it would have let widen, and both repairs. The rule holds of a **parametric** sort
exactly as it holds of a plain one, and it is asked of the **provided spec**, not of
the enclosing provider — a provider with constructors is the ordinary concrete carrier.

*It used to be a CLASSIFIER on the other spelling, and that is worth recording because
it was the sharpest edge the two-reading `fact` had.* While a `fact Spec[…]` could be a
provision, a data-sort functor was what told the two apart: `fact Polynom[Int64]` says
the instantiation is well-formed, not that `Int64` is a polynomial, and `fact Box(value:
Other)` constructs a box. Both filed a provider edge until the rule was applied whole —
the second changing an upcast's verdict, since a bogus edge admits the widening and the
refusal that follows is then about something else. The **written surface** could not
decide it: `[…]` applies types and `(…)` constructs, but a data `fact` is ordinarily
written with brackets, so brackets carried no claim of provision. Since WI-20260917-S8JYF
retired the `fact` spelling of a provision there is nothing to classify — every `fact` is
...
A minted operator names its target **absolutely** now (§5.5), so no tier entry remains for a declaration to shadow and the capture is *unrepresentable* rather than refused. Such a declaration therefore **loads**, and the two are simply different operations: `operation mod(a, b) = 99` beside a minted `7 % 2` is legal, and the operator still answers `1`. What the author gets instead of a refusal is the rule in §5.5 — the operator and the bare call are no longer interchangeable, and writing the operation's own name (imported, or qualified) is what says which is meant.
...
| `not` / `or` / `and` | the resolver **primitives** (§6.6) — see below |
...
**A rule body evaluates both a goal and a value slot; what it declines is an operation no runtime here can RUN.** The reading above is the design, and the limit is narrower than "the evaluator has not arrived" — two evaluators run, and both ask the same question: *is there an implementation this process can call, and is calling it effect-free?* Measured (**WI-20260825-P9Y67**, **WI-20260826-VPEWK**), and each pair is a separating row:

* **at a goal** — `:- String.contains("abc", "b")` answers **1**, `:- String.contains("abc", "z")` answers **0**. A `Bool`-returning host operation takes the relational view of §5.3, which `op_reducible_in_rule_body` admits an interpreter-mapped op into alongside a bodied one (**WI-20260822-J38JE**). `Bool.and` is *not* the witness for this row, though it answers 1/0 too: at goal position it is **position-directed** to `anthill.kernel.and` (§6.6) and resolved as a conjunction of two goals, so no host function runs and the row answers the same with the host route removed.
* **in a value slot** — `reduce_operand` (**WI-482/WI-483**) is the single pipeline shared by `eq` / `cmp` / `arith`, and it runs: with `operation bodied() -> Bool = true`, `:- bodied() = true` answers **1** and its `false` twin answers **0**. A bodied operation whose BODY calls a host op reduces too (`:- viaHost() = true` answers 1).
* **the gate** — a host-implemented operation reduces in a value slot when its implementation is one *this* runtime registers and its declared `effects` row is **empty**: `:- Bool.and(true, true) = true` answers **1** and `:- Bool.and(true, false) = true` answers **0** (**WI-20260826-VPEWK**; **WI-20260822-ZJZS7** first recorded the row, when it answered 0). The reduction and the *delay* are asked the same question, so an operation the gate admits but whose arguments are not ground **suspends** rather than being compared as data — `:- Bool.and(?b, true) = true` carries a residual, exactly as its bodied twin does.
* **what the gate still declines**, and the three reasons are different. A **body-less spec op** is declined deliberately: an operand is a term a *rule wrote*, and such an op there may be **symbolic algebra** rather than a computation — `Set.insert` / `Set.empty` are exactly the terms the membership rules resolve over, so `:- Set.insert(Set.empty(), 1) = Set.insert(Set.empty(), 1)` answers **1** *as data*, and dispatching it would destroy that. An **effectful** operation is declined because a host function raises nothing the bridge could catch — its declared row is the only guard, and a bodied op's effect is caught by the bridge instead (whether `Error` alone should be admitted is open). A third case CLOSED with **WI-880** and is recorded because it was a soundness gap rather than an incompleteness: an operation whose host function was registered by **hardcoded name** rather than by an `operation_map` clause was declined for neither reason above but because the gate could not **see** it (**WI-884**'s split). `:- String.concat("a", "b") = "ab"` answered 0, and — unlike the two cases above — it was *decided false* rather than suspended, so `not(…)` over it answered **1**: a positive definite answer out of a call that never ran. WI-880 migrated the whole remaining hardcoded surface — `String`'s eight, `Int64`'s arithmetic, conversions and division, `Float`'s division and IEEE predicates, `BigInt`'s conversions, and the **26 `anthill.reflect` accessors** — so the gate sees one registry and both rows flipped (`wi_vpewk_host_op_operand_test::a_migrated_registration_is_visible_to_the_gate`). It was not yet the WHOLE surface: 46 more stayed hardcoded — `PartialEq.eq`/`neq`, `struct_eq`, `Map`, the relational algebra, the runtime-effect operations and the dispatch-value readers — until **WI-20260922-BRT4Y** moved them too, so every operation the interpreter registers is now one this gate can see (the effect-row clause above still declines the effectful ones). The reflection family was the one whose consequence was not "a host op is awkward at one position": since the whole accessor surface sat in the invisible half, **no rule could decompose a term at all**, and `not(term_as_int(7) = some(7))` answered **1 DEFINITE** — a positive conclusion drawn from a term the rule never read (`wi880_reflect_mapping_test`). Twenty of those twenty-six are declared at NAMESPACE level and so have no `<carrier>.<operation>` key; a binding block whose target is the **namespace** registers them, which is a different question from §8.7's "a namespace-level operation is not BACKING" — that rule is about discharging a `provides`, and a mapping says what *realizes* a declared operation.

**A NESTED host call reduces too, and that took a second fix.** `reduce_op_value` used to σ-walk each argument to a `Value` without REDUCING one that was itself an op-call, so a nested call residualized — WI-20260826-VPEWK's documented remainder, sound but incomplete. Making the accessors host-mapped turned that incompleteness into a **wrong answer**: an un-reduced call IS a term, so a parameter declared `Term` passed the bridge's ground check and the host function ran ON THE CALL — measured, `:- term_as_int(as_term(7)) = none()` answered **1 DEFINITE** where the true value is `some(7)`. (The `= some(7)` row is 0 on both sides of that back-out and cannot see it; the un-fired path and the wrong-value path agree there by accident.) A host callee's arguments are reduced before the bridge now, so the nested call answers correctly and VPEWK's remainder closes with it. Two guards are measured across that change and unmoved: symbolic algebra stays DATA (`:- Set.insert(Set.empty(), 1) = Set.insert(Set.empty(), 1)` still answers 1 — those callees are in no interpreter registry, so the recursion bails on them), and a bodied op split against a host call is unchanged. **What still declines** is the BINDING case: `eq` never binds, so `term_as_int(7) = ?v` suspends — a question about the connective, owned by `WI-20260822-F0HHB`.
...
`a & b` in a goal is **not** refused — that refusal was lifted with the conjunction primitive (§6.6), and `:- true & true` / `:- true & false` answer 1 / 0.
...
**And at every goal position, not only a rule body's own atoms** (WI-20260902-VZC2C): a `|` / `&` **branch**, a **top-level query** goal, and a **constraint guard** body take the same reading, as §6.6 already says of the connectives ("at every GOAL position: the body's atoms, and the goal slots of the connectives above them"). This is worth stating because it was false for a **nullary** operation and silently so — `:- flag` answered 1 while `:- p(999) | flag`, the query `flag`, and `no ?n: Box(n: ?n) -: flag` each answered nothing, with `:- p(999) | not(flag)` then *succeeding*. The cause is that the two nullary spellings are **one term** (§8.3 / the storage canon: `flag` and `flag()` are indistinguishable once stored), so a goal that reaches the resolver *through* a term — a connective branch is bound by the head match of `or(?a, ?b) :- push_choice(?a, ?b)`, a query and a guard are built as terms outright — arrives as the bare name, which has no call to reduce. The application is restored where the relational view is taken, since that is the only point that knows the position; a bare nullary name in a **value** slot keeps §5.4's two readings (the eta lift and the zero-arg call), and neither is affected.
...
**For the `Bool` view, "effect-free" is about the row's MEMBERS, not its length** (WI-20260830-DQD5W). An operation declared over an effect-row PARAMETER — `Iterable.isEmpty(c: C) -> Bool effects E`, on a sort declaring `effects E = ?` — has a non-empty row that names no effect: `E` is instantiated by the CARRIER, and `List` writes `E = {}`. Such an operation IS granted the `Bool` relational view; only a row with a concrete member (`Modify[c]`, `Error[…]`) is refused. Measured before this: `rule empty(?b) :- Box(items: ?ls), isEmpty(?ls)` answered **NO SOLUTIONS** beside a `contains(?ls, "a")` that answered 1, and a goal with no clauses is FALSE rather than an error — so `forall ?ls: Verdict(labels: ?ls) -: nonEmpty(?ls)` loaded clean and then fired on **every** verdict, indistinguishable from a working constraint. The parametric row is admitted only for a **bodied** operation, and that is what keeps it safe rather than lenient: the SLD→eval bridge runs a body under an empty effect registry, so a carrier that instantiates the row to a real effect raises it, the bridge catches it, and the goal residualizes as before. A HOST-implemented operation raises nothing the bridge could catch — its declared row is the only guard — so a parametric row does not admit one.

**A raise that escapes the bridge is a FAULT, not "no solutions"** (WI-20260911-0V0F7). The bridge turned every evaluator error into a residualize-and-say-nothing, so a goal whose callee *ran and raised* came back indistinguishable from one the structural fold merely declined — and the empty answer set read as a relation with nothing in it. Measured on an operation whose row is EMPTY so nothing else warns: `rule answer(?r) :- guardExhaustible(0, ?r)` over a guard-exhaustible `match` answered **no solutions** with no diagnostic, beside `guardExhaustible(5, ?r)` answering `?r = 5`. The goal still **residualizes** — WI-483 substitution transparency says a callee's own failure must not break the enclosing rule — but the search now records the raise on `ResolveStats::errors` and marks itself **incomplete**, so an empty result is *undecided*, not a refutation. The repair the diagnostic names is `Error.reify` (§ the `Error` layer): declaring the label instead is strictly worse, since a concrete `Error[T]` member stops the relational view being granted at all, which is 0 solutions with nothing said. What is *not* a fault is a raise that is not a domain failure of the callee — a floundered relation drain (`relation_floundered`) and a scoped load reporting its diagnostics (`load_failed`) both ride the same channel and both mean "no answer yet" — so the partition is by the payload's **cause**, not by the raise channel.

**The arity+1 view reads the row the same way** (WI-20260830-NX4FD). `size(?ls, ?n)` — `FiniteCollection.size(c: C) -> Int64 effects E`, reached through `List`'s provision — binds `?n` to the list's length, exactly as the carrier's own `length(?ls, ?n)` does. It answered **nothing** before, and the effect clause alone did not fix it. The other half is a rule about **requirement dictionaries**, not about this view: when a call's argument types pin some of a `requires` clause's type-parameters and not others (`FiniteCollection requires Iterable[C, Element, E]`, of which `size(c: C)` pins only `C`), the slot is completed from the spec's providers where they leave exactly one possibility, and otherwise **kept as a recorded absence** — a slot that is loud at any read and costs nothing at none. Aborting the whole call instead is what made the goal silent. The absence belongs to this route (it holds argument values, which carry no type arguments), not to the program: the compile-time dictionary builder resolves the same slot from the call's types, and a slot it can't fill is refused at load (WI-20260925-4ZZKZ).

**Two further conditions on the `Bool` view, both fixed by the same ticket and each measured with `List.contains` rather than a spec op.** (1) The routing applies **at the declared arity** — the sentence above always said so, but the resolver did not check: a 3-ary goal on a 2-param operation was wrapped in `eq(…, true)`, the extra column was dropped by arg-place lookup, and the goal answered one **definite** solution with the result variable free. Gated, such a goal reaches the arity+1 view instead, which binds it. (2) The operand is rebuilt on the **occurrence** carrier before the rewrite, because the reduction folds only an occurrence and hands a hash-consed term straight back. That is what a **constraint guard** carries: `constraint c: no ?ls: Box(items: ?ls) -: contains(?ls, "z")` loaded clean over `fact Box(items: ["z"])`, so a `no` never fired and its `forall` transform (`no … not(body)`) fired on every row — the same inertness for a carrier's own operation as for a spec op.
...
The clause **determines** as well as licenses (WI-20260829-70XVH). Where a call pins the requirement's carrier, that carrier's provision answers the clause's other elements, and each element naming a type parameter *of the operation* is bound from it: `map[Sc, S, EffS](s: Sc, f: (x: S) -> …) requires Iterable[C = Sc, Element = S, E = EffS]` called over a `List[T = Int64]` reads `List provides Iterable[Element = T, E = {}]` and grounds `S = Int64`, `EffS = {}` — parameters that appear in no argument, and which would otherwise be the loud `expected a type for 'S', got unconstrained` the supply paragraph below names, forcing the author to write a bracket repeating what the source already says. Three limits, each leaving that error rather than a guess: only the **operation's own** parameters are bound (its enclosing sort's belong to the sort instance, not to this call), only ones the call has **not already pinned** (a written bracket outranks the clause, as does an argument), and only values the receiver's own type arguments **determine**. The same clause read inside the operation's **body** answers the sibling question — what type a bare carrier argument has where it flows into a constructor field typed on that spec — which is what lets a *free* operation build a combinator over any provider of the spec it requires, the third of the three declarations that can carry such a provision (the other two being the spec the operation is a member of, and the enclosing sort's own `requires`).

**A requirement's carrier may be a PATH PROJECTION** (WI-20260909-S8CBV): `operation pick(x: Box) requires Desc[T = x.E]` names the `Desc` instance of the element of *whatever `Box` this call was handed*, not a fixed sort. This is the ordinary path-dependent type of §8.1 in the one position that could not previously spell it — a parameter type (`e: x.E`) and a return type (`-> x.E`) always could, and a `requires` bracket reported `unresolved name 'x.E'`. The member is read off the receiver's **argument type at the call**, so one declaration answers a different instance per call site; a bracket naming a concrete sort does not, and that difference is the whole content of the form. A **compound** receiver (`x.f.E`) is refused: it needs an occurrence carrier the goal's term slot cannot hold.

Where the call *cannot* ground the projection — the caller passed its own abstract parameter, so the receiver is still a variable — the dictionary can only come from the caller, and the caller must **declare the same requirement**. `operation outer(b: Box) requires Desc[T = b.E] = pick(b)` supplies it; the same body without the clause is a **load error** naming the projection. Inferring the clause instead is not yet done, and the boundary is a real one rather than a convenience: matching the callee's projection against the caller's means comparing two neutrals with *different* receivers, which is the ζ identity check of `docs/design/path-dependent-types.md` §4.1 and needs the receiver re-keying that section defers.
...
**And it FORWARDS one, to a cross-sort callee, exactly as a sort-level slot does** (WI-20260921-159S9). Where a body calls an operation whose own `requires` must be supplied, the dictionary the call builds may project out of *any* slot the running frame holds — its sort's and its operation's alike. Until this, the instance-dictionary channel read the sort half alone, so `operation ins[E, OE](s: SortedSet[T = E, O = OE], x: E) requires OE: WeakOrd[E]` was refused at load ("a slot declared on the OPERATION does not reach this call") while the identical clause written on the enclosing sort ran — the two-spellings-disagreeing signature that the paragraph above names as a defect, one coordinate over.

What makes the forward sound is an invariant about ENTRY rather than about the call: **every route into an operation fills its op-scoped slots.** The channel is read strictly at evaluation — a projection that names a frame slot must resolve or the dispatch has no target — so a route that filled none would turn a load-time refusal into a run-time unbound read, which §5.3's own rule forbids. The routes are four, and, as with the five ways a body "reads the slot" above, the enumeration is the point rather than the number: a call site fills them from its own substitution; a **host entry** resolves them from the argument values, an op-scoped requirement ranging over the operation's own parameters and the arguments therefore pinning it; an **eta'd reference** carries them captured at the mint, since the function value escapes to a foreign frame; and a **deferred dispatch**, whose target is chosen at evaluation and whose call site therefore cannot have built evidence for it, fills them at entry by the same resolution the host entry uses. A slot no route can pin stays ABSENT and the body's own read reports it — "has a chain" and "needs it" being different questions, as everywhere else in this section.
...
**A requirement whose carrier is the CALLER's own type parameter, which the caller declares nothing about, is a load error** (WI-20260920-XSVCS). This is the *abstract* half of the exclusion above, and it is an exclusion only where the parameter is one a later call still pins. A parameter of the **calling** declaration is not: `mid[U](y: U) = tyOf(y)` against `tyOf[B] requires TT[T = B]` forwards `U` into an operation that demands evidence about it, and the caller's frame — which holds no such slot, because the caller declared none — is the only thing that could ever supply it. Nothing downstream fills it and no later instantiation reaches it, so the call is refused where it is written, naming the caller, the parameter, and the clause to add. The verdict is **carrier-shaped, not spec-shaped**: it holds for any spec, which is what distinguishes it from §8.4's `TypeValue` rule, whose own arm is stronger for that one spec (`type_value()` being nullary, an unfilled slot is never benign there, so the refusal needs no body evidence at all). As above, the refusal is owed only where the callee's body **reads** the slot: a requirement declared and never read costs an unfillable slot nothing, and refusing those would take the standard library with it.
...
**A subject that declares no predicate is still not a GOAL** (WI-20260902-8K4RB) — the
third member of the "is this term readable as a goal at all?" family above.
`rule reader(1) :- tauX` beside `rule tauX <=> 7 @[simp]` is a **load error**: an
equation's clauses are indexed under the `eq`/`unify` connective and never
under its subject (WI-898, below), so `tauX` owns no clause and the goal can never match
— in any program, in any branch, under any binding, and *however the equations are
written or tagged*: `@[simp]` rewrites a **value**, and a goal is **matched** rather than
rewritten. (It does fire inside a rule body — in a value slot: `rule r(?v) :- ?v =
tauX()` stores the already-inlined `eq(?_, 7)`. It is the *position* that admits no
rewrite, not the construct.) It reached neither existing gate: WI-1034's "goal names
nothing" declines because the name **resolves** (the mint stamps it an equation
functor), and the goal-reading pass tests an operation record, which an equation declares
none of. Measured before the
refusal, one file, three rules: `:- tauX` and `:- tauX()` each answered **0** and
`:- not(tauX)` answered **one** — the same negation-as-failure laundering `:- not(42)`
shows, so the silence was a wrong answer and not merely an absent one. The refusal is a
**sibling** of `UnreducedEquationFunctor` and not that error, because the repairs differ:
that one is a *value*-position citation the rewriter left standing, so it sends the
author to tag the equation `@[simp]` or to inspect the left-hand patterns, and here the
equation may be tagged **and** firing while the position still admits no rewrite. The
error names the goal and its location and prescribes the two repairs that are driven —
cite it from an **operation body**, where the equation inlines before dispatch, or give
the name a **predicate** rule with `:-` if a relation was meant. A name a scope writes in
**both** head shapes is unaffected: a predicate clause is then indexed under it, it
denotes a relation (above), and its goal answers.
...
The bare form brings the resolved dictionary into the clause for the body calls
it covers — the calls to the spec's own operations that name no carrier, and
those whose carrier argument is the one the dictionary was read from (a call to
a DEFAULTED operation is still covered whatever it carries — a known gap,
`docs/design/060-implementation.md` §7.3); the direct `?d = require[X]` form also exposes that same structural
...
A bound that is a TYPE VARIABLE — `rule el(?x: T) :- true` inside `sort Wrap[T]`
— names no sort, so its domain is an implicit parameter of the clause: a
`SortDomain[T = T]` dictionary, where `anthill.reflect.SortDomain` is the kernel
spec whose member is a sort's `domain` and whose instances the loader derives for
every sort with one. A citation fills it from its caller's evidence —
`operation countAt[X]() requires SortDomain[T = X] = Wrap[T = X].el.takeN(5).length()`
counts `Colour`'s three values under `countAt[X = Colour]()` — and with `?x` bound
the clause reads it off the value's type; with neither, the bound keeps §2's
ladder (WI-20260911-5G28A).
...
**But an equation's subject may not resolve to another scope's PREDICATE (WI-20260821-D0EXD).** "A clause of what it resolves to" is only coherent for an equation where what it lands on is a thing equations *define* — an `operation`, or another equation's subject. A rule-introduced predicate is neither: an equation's clauses index under the `eq`/`unify` connective and never under its subject (WI-898), so the equation does not become a clause of the predicate — the subject merely points at it, and the equation-defined operation the writing scope meant to name **ceases to exist there**. Measured, two arms one token apart with the sort body identical in both: `qlib { rule f(2); sort Rec { entity r(n: Int64); rule f() <=> 1 @[simp] } }` left `qlib.Rec.f` **absent** — `Rec.f()` was *"unknown functor"* while `f()` under `import qlib.*` answered `Int(1)` — and renaming the declaration to `rule other(2)` gave the exact mirror. Both loaded and nothing was reported, so a caller was right in exactly one of two programs that differ by a rename elsewhere. Such a subject is now a **load error** naming the scope losing the operation and the scope declaring the predicate, and prescribing the two repairs that are driven: an `operation` in the predicate's scope, which *is* the declaration of an equation-defined name, or a body-less `rule` in the equation's own scope, which says the two are separate things that share a spelling.

What the subject may not name is a **relation** rather than a predicate specifically: a rule's operation name is its label else its head functor ([052](proposals/052-rules-as-stream-valued-operations.md) §"Naming the relation"), so a labelled rule's own name is the same category error one spelling over — measured, `lc1 { rule f: p(?x) :- q(?x) … }` beside `lc2 { import lc1.*; rule f(true) <=> 7 @[simp] }` loaded clean with `lc2.f` absent and the citation answering `Int(7)` out of `lc1`'s label.

`<global>` is not a party to it **as a target**, and only as a target — the exclusion the visibility error below takes, on its own reason: nobody opts into the scope every file shares, so a namespace's equation must not be answerable for what a namespace-less file happens to spell. Measured before the exclusion, a top-level `rule f(?x)` beside `namespace pg { rule f(true) <=> 7 @[simp] }` was a load error naming `<global>` and prescribing a declaration there, while the predicate spelling of the same pair loaded clean. That argument is **directional**: a `<global>` head reaches a namespace's name only through an import it wrote, so it opted in exactly as any namespace does, and taking the exclusion both ways left the defect live in that half — a namespace-less file writing `import pgz.*` and an equation defined `pgz`'s predicate, visible as `Int(7)` to a third namespace that never saw the file. What the target-side exclusion costs is the silence this scope has always taken: a `<global>` predicate does absorb a namespace's equation subject, and nothing says so.

It is **refused rather than split**, because the language already refuses this pair wherever neither side is minted before the head pass reads the table: `zi { rule f(true) <=> 7 @[simp] }` beside `zj { import zi.*; rule f(1) :- true }` is the visibility error below, since both heads introduce. A 061 **declaration** is minted in pass 1, so it is the one shape that arrives already denoting — a hole in that refusal rather than a second question. And it is stated over **another** scope's predicate, on that refusal's own principle: the shadow itself is not the defect, inventing it is. `sort Rec { rule f(?y); rule f() <=> 1 @[simp] }` is one author writing both in one place, the name carries both roles, and it answers — driven, `Rec.f(false)` = `Int(2)`. Census over the whole corpus and every fixture, instrumenting the head pass with the resolved symbol's kinds: **643,866** heads that denote, of which **516,224** are an equation subject on an `operation` and **12** on another equation's subject, **63,915** a predicate head on a predicate and **63,654** on an operation — against **4** an equation subject on a predicate, every one of them in a fixture written for the visibility refusal itself. Zero in the shipped corpus. The remaining **57** land on a kind this rule does not reach and are named rather than left as a gap in the arithmetic: 30 a predicate head on a nested sort's constructor, 16 an equation subject on an **entity**, 6 a predicate head on an entity, 2 on a name that is both a predicate and a sort, 2 on one that is both a predicate and an operation, 1 on a namespace. The 16 are the class this rule admits in silence — whether an equation may define an entity constructor is a separate question from whether it may define a relation, and it is not answered here.

One consequence lands on the visibility error's own text: a body-less `rule` in the named owner does **not** collect an equation subject written in another scope, since such a subject is exactly what this rule refuses. Where one of the colliding heads is a subject written outside the named owner, that message prescribes an `operation` there instead — measured by taking its old advice, which traded one refusal for the other.

**Two scopes that can see each other may not both introduce one name (845G7).** The rule above is total and order-free, but it makes a second scope's same-named head a **shadow** rather than a clause — a scope's own name beats what it imports or inherits, because `resolve_in_scope` reads `locals` and returns before consulting any import or parent. Measured with a paired control: `mA { import mB.*; rule p(1) :- true; rule usesp(?x) :- p(?x) }` beside `mB { import mA.*; rule p(2) :- true }` answers `usesp(1)`=1 and `usesp(2)`=**0**, while the same `mA` with no `p` of its own answers 0 and **1** — so the import works, and the local is what killed it. The **shadow itself is not the defect; inventing it is.** An author who *writes* `rule p(?x)` in `mA` gets exactly this and should. So a name introduced at two scopes either of which can reach the other is a **load error**, naming them and naming both repairs: one body-less `rule p(…)` in the scope that owns it, plus a named import of that predicate in every non-enclosing contributor, makes the others' heads its clauses; one declaration in **each** scope says they are separate predicates. Enclosing contributors need no import. Where one existing scope is reachable from every head the message names it as the minimal-change owner; where none is, it says so rather than inventing one. Which place that is, and why the obvious reading of it is wrong, is below.
...
**A named owner must be reachable from every other member.** When the refusal names a scope it promises a minimal-change repair: declare there and replace each contributor's implicit non-enclosing reach with a named import, while enclosing contributors already have an explicit path. The scope it picks is therefore the one *every* other member reaches **from every file that member writes a head in** — not the one that reaches nothing, and not one merely reached from some file. The two differ wherever reach is not transitive, which a wildcard import always is: it is never re-exported. Measured on `zzA → zzB → zzC`, the old reaches-nothing reading named `zzC`; before C666A, following the declaration-only advice loaded with `zzA` silently split off, while C666A now refuses `zzB`'s wildcard-only join. In a mutual cycle every member qualifies and the first in display order is named; where no scope qualifies the message says so and offers per-scope declarations or an explicitly imported owner instead. The per-file half is the same defect one coordinate over: visibility is measured from every file, never unioned per scope.

**A PAREN-LESS nullary head is an application of arity 0** (`rule holds :- base(1)`; WI-20260821-P85Z7). It introduces its predicate and is scoped where it is written, exactly as `rule holds()` is, so the refusal above reaches it and §"A rule-introduced functor is scoped where it is written" governs it. The two spellings used to be different programs: the parser gives a bare name a `Term::Ident` rather than a zero-argument `Term::Fn`, and the loader's head walk read only the `Fn` shape — so the bare one introduced nothing anywhere and fell to the one global intern that section exists to prevent, with two scopes sharing one uncitable predicate and the loser's clause answering inside the winner's scope, on a program that loaded clean. **And the two spellings are ONE TERM** (WI-20260902-CZJ2N). Until that ticket the introduction rule above had moved and the term had not — a paren-less subject built a `Ref` leaf and a parenthesised one a zero-argument `Fn`, and the two did not unify, so `rule holds :- base(1)` was not reached by the goal `holds()` nor `rule holds() :- base(1)` by the goal `holds`, on a program that loads clean. `Fn{f}` is now stored as `Ref(f)` for every name **without a type reading**, and one view head serves both, so a predicate assembled from both spellings answers from either. **The EQUATION side is the SAME rule, where it used to be the opposite one**: `rule tau <=> …` defines exactly as `rule tau() <=> …` does, and a bare equation subject is minted exactly as a parenthesised one is. What stops a citation of a *nothing-owning* subject being silent is a loud channel that exists for it at every arity, not a spelling-dependent refusal at arity 0 — and there are **two**, one per position: `UnreducedEquationFunctor` (WI-898) where the citation is a *value*, and the goal-position refusal above (WI-20260902-8K4RB) where it is a *goal*. The second is what the guard had been covering by accident at arity 0 alone: deleting the guard made the bare goal citation as silent as the parenthesised one already was — one rule at every arity, and the right direction — and the goal-position refusal is that rule's loud half, at every arity and in both spellings.

**The exception is a name with a TYPE reading, and there the SLOT decides.** A `SymbolKind::Sort` name keeps both spellings apart, because §8.3 and WI-391/WI-387 give them different meanings in a type position: `Ref(S)` is the dispatch **wildcard** a `provides Spec[T = T]` lowers to, and a nullary `Fn{S}` is the **concrete** spec identity. Merging them there is not a widening but a wrong answer — measured: with the gate removed outright, 792 symbols change spelling and `anthill.prelude.FiniteCollection` stops covering its own `requires`, so the standard library does not load. Everything else — a predicate, an operation, an equation functor, a namespace, a sort-nested constructor — has one term. **Which paren-less spellings remain unscoped, and why each is somebody else's:** a **fact** head is SCOPED at every arity, so `fact holds` and `fact holds()` are alike in declaring the predicate where they are written — this sentence said the opposite until WI-20260821-RDGQC measured it: `fact H` is `rule H :- true` (§1234), and the two spellings of that one clause were different programs. What does remain unscoped is each functor of a **multi-head** rule (WI-20260908-NE0E4) and a head inside a `provides … language … end` block (WI-20260821-TTHRK). One spelling is neither, and it is now **closed**: a **qualified** paren-less head (`rule nsx.tgt :- b(1)`) folds into a minted `field_access` chain, so it used to neither introduce nor reference and its clause landed under `field_access` — the whole rule dropped in silence, while `rule nsx.tgt()` referenced correctly. The chain is now read as the NAME it spells wherever the term states a proposition (§6.7, *A dotted paren-less name in a LOGICAL position is the name it spells*), so the two spellings reference alike and the head introduces nothing in both, a qualified name being a reference (WI-20260901-719FJ). A **marked absolute** paren-less head (`rule ..nosuch :- b(1)`) had the same two-spelling split and is closed with this one: it is refused when it names nothing, as `rule ..nosuch()` already was (WI-1075), and a resolvable `..tgt` joins its predicate in either spelling.
...
**A guarded equation is a CONDITIONAL rewrite (WI-20260820-8RJK8).** `lhs <=> rhs :- guard` is one rule with a precondition, and the guard is the ordinary `:- body`, proved **post-match** against the substitution the left-hand side's match produced. Firing is three tiers and only the LHS is indexed ([proposal 043](proposals/043-simp-rewrite.md) §4.6, `docs/design/constrained-term-substrate.md` §"Conditional rewrite rules"): the discrimination tree narrows **structurally**, the type-side guards — the enclosing sort's `requires`, a typed pattern `?x: T` — decide on **carried types**, and the written guard decides on **values**. So a guarded and an unguarded rule with the same left-hand side index identically; the guard never enters the index. It used to be read by no firing site at all: every one of them gated on a predicate whose first clause was "the body is empty", so `map.anthill`'s `get(put(?m, ?k2, ?v), ?k) = get(?m, ?k) :- neq(?k, ?k2)` — written directly beneath its `<=>` `@[simp]` siblings — was dead beside them.

**The tag still decides, and the guard does not substitute for it.** `@[simp]` / `@[unfold]` remains the enablement (WI-881, above): an untagged guarded equation is exactly as inert as an untagged bodyless one. That is what keeps `field.anthill`'s `div(?a, ?b) = mul(?a, recip(?b)) :- neq(?b, 0)` a **law about** division rather than a rewrite that turns every division into a reciprocal multiply — the guard says *when the equation is true*, the tag says *whether to orient it*, and they are different questions. The corresponding obligation is on whoever tags one: a tagged guarded equation is a **rewrite**, so its right-hand side must be smaller than its left in whatever sense makes the normalizer stop. The stdlib tags only reducing ones and leaves `logical_stream.anthill`'s recursive `interleave` laws untagged with that reason written at them.

**Every firing site answers alike** — the resolver's rewriter, the typer's operation-body pass and the typer's dot-rule pass all share one guard evaluator — so a tagged rule means the same thing at a resolution goal, in the operation body that spells the same call, and at a method call. At the typer the redex is compile-time syntax, so most guards are simply under-determined and decline; the ones that fire are the ones already decidable from the program text (`nth(xs, -1)` under `nth(?_, ?i) = none :- lt(?i, 0)`), which is constant folding.

**What the guard BINDS is part of the answer.** A variable bound only by the guard and used in the right-hand side — `headOption(?s) = some(fst(?p)) :- splitFirst(?s) <=> some(?p)`, the standard conditional-rewrite idiom (the guard is written `<=>` because it BINDS — the dispatched `=` is a test and binds nothing) — carries the guard's **witness** into the rewrite. Because the rewrite's result then depends on that witness, the guard must **determine** it: a guard with two different witnesses for a right-hand-side variable does not decide the rewrite and the rule declines, the same answer an under-determined guard gets. A guard variable the right-hand side does not mention is an ordinary existential and constrains nothing.

**An UNDETERMINED guard suspends; it is never decided by negation-as-failure** (WI-067) — the three-valued reading a type bound already has, collapsed here to "fire" / "do not fire" because a rewrite has only two outcomes. The guard is discharged by an ordinary **definite-only** search, so an undischarged residual — a delayed goal over an unbound variable, a negation over an undecidable inner goal — yields nothing and the redex is left standing rather than rewritten. `not(p(?x))` over an undecidable `p` is the shape that makes this load-bearing: plain negation-as-failure would launder it into a fire. A **definite** witness fires regardless of truncation elsewhere in the search, truncation being able to hide only further solutions, never unmake the one already proved. And proving a guard may not itself require proving a guard: one level, past which the guard is undecided and the rule does not fire.

An **open-world operand** is undetermined in the same sense and declines before the search runs. At the typer a parameter reaches a guard as its compile-time stand-in, which a scalar builtin reads as an ordinary ground constant — so `neq(p, q)` over two parameters would succeed *structurally*, and a rule guarded on it would fire on a call whose two arguments may be equal at run time. Measured: `pick(?a, ?b) <=> ?a :- neq(?a, ?b) @[simp]` rewrote a body `pick(p, q)` to `p` while the ground `pick(1, 2)` beside it folded to `1`. The ground one still folds; the symbolic one is left standing.

**`@[simp]` is what makes an equation run (WI-881).** The attribute does not merely pick a *direction* — it is the *enablement*. An **untagged** equational rule is inert: the normalizer never fires it, so a body-less operation whose only definition is an untagged equation dies `OperationBodyMissing` at eval. That verdict is now uniform across the routes to such an operation (WI-1092), which it was not: a carrier's `eq` member reached through the eval→SLD bridge — by a requirement **dictionary**, or from the `eq` builtin's carrier dispatch — used to be handed to the resolver anyway, exhaust an EMPTY candidate set, and come back *refuted*, so the program silently computed with "not equal" wherever that carrier's equality was consulted and raised nothing. An absent definition is not a false one: a **declared** operation with no clause, no body, no host mapping and no builtin is unrunnable and says so, while an *undeclared* predicate symbol with no clauses stays the empty relation and refutes, which is ordinary closed-world SLD. Inside resolution the same target leaves the goal **undecided** — the resolver has no error channel, and an undecided goal is the one answer that is not wrong. Tag the same equation `@[simp]` and the operation runs, because the typer rewrites LHS→RHS in operation bodies before any dispatch. That is **inlining, not dispatch**, and the distinction is load-bearing: the §8.7 backing check does not count a `@[simp]` equation, so one cannot discharge a *spec* operation's obligation (a rule is not backing — WI-818); what it can do is give a sort's **own** operation a meaning — but **not** as a third way beside a body and a host mapping, and WI-885 is deciding whether it is a backing kind at all. `@[simp]` needs two things a *dispatched* call does not have: a syntactic **redex**, and a **statically known carrier** (`set.anthill` states the second — its laws fire "once the redex's carrier is known to provide `Set`"). Dispatch through a requirement **dictionary** has neither: `dispatch_via_sort_ops_table` resolves an operation *symbol* out of the dict and invokes it, and the invocation path is builtin → body → value-directed impl. There is no slot a rewrite rule could occupy, so **no dictionary entry can be built from a `@[simp]` equation** — which is why `carrier_override_op` reading such a member as absent is the correct answer, not a bug. Treat `@[simp]` as proposal 043's directional-rewrite attribute; write a definition as a body (WI-580: the equational, relational and proof views are *derived* from it — WI-669) or as a host mapping. A bare-identifier RHS immediately before the attribute is safe: `<=> none @[simp]` is the name `none` and its block, because `@[` is one token that no instantiation bracket begins with — this was a trap while the attribute was a bare `[simp]`, which read as `none[simp]` (WI-20260915-G9EA9). **A nullary HEAD no longer needs its parentheses** (WI-20260902-CZJ2N): `rule tau <=> …` and `rule tau() <=> …` are one term and one law, because a nullary application is stored bare — see *A PAREN-LESS nullary head is an application of arity 0*, above. That was the second trap here until CZJ2N, and it is deleted rather than narrowed. A `@[simp]` head still matches an **application**, so it rewrites `tau()`; a bare `tau` **call site inside an operation body** is a `var_ref` and is still not a redex, which is §5.4's type-directed reading and a different question from the head's. A nullary operation whose call sites are written both ways therefore still wants a body or a host mapping.
...
**A bare spec member in a signature is a type parameter, spelled shorter** (WI-201). `Spec.Member` in an operation's parameter, return or effect type — where `Member` is a type parameter of a constructor-less sort `Spec` — is a fresh operation type parameter `P` with `requires Spec[Member = P]`: `operation cmd(s: WorkItemStore.State)` is `operation cmd[P](s: P) requires WorkItemStore[State = P]`, and typechecks as that does. Every occurrence in one signature is the same `P`; another operation gets its own. Outside an operation signature (a field type) the bare form is not sugar, and stays the refusal it always was. **Inside a carrier, it names the carrier's binding** (WI-20260923-ZBWMC): in a block of a sort that provides `Spec` — its body, or a `namespace <Sort>` secondary entry (§5.1) — `Spec.Member` is the type that block's `provides` clauses bind `Member` to, so under `provides WorkItemStore[State = WIS]` it means `WIS`, with no parameter and no requirement. Only a `provides` clause does this; a `fact Spec[…]` is not a provision (§5.1). The narrowing reads the provisions written in the operation's own block, but a sort's provisions are the sort's wherever they are written, so if the sort's provisions give `Member` **more than one binding** — two in the block, or one in the body and another in a secondary entry or another file — `Spec.Member` names no one type there and is a **load error** listing every binding (`docs/design/path-dependent-types.md` §5.3, "two carriers in scope"): write the type meant, or take it as a parameter. A sole binding no signature in the block could spell — a family (`State = ?x`, `State = List[T = ?]`), another sort's abstract member, an operation — narrows nothing, and the sugar keeps its generic reading.
...
**The two readings of a bare operation name, and what partitions them** (WI-20260828-2TMB5). A name written without parentheses has two candidate readings: the **eta** one above, and the **zero-arg call** — `operation main() -> Int64 = total`, where `total` denotes its *result*. **The same partition governs a RULE-BODY slot** (WI-20260902-CZJ2N): `rule c(?v) :- ?v <=> seven` binds 7, exactly as `?v <=> seven()` does — and so does a **dotted** name of one, `?v <=> Box.zero` binding what `?v <=> Box.zero()` binds (the call is made where the chain is reduced as an operand, so its *term* is still the chain and `fact f(Box.zero)` is still matched by the goal `f(Box.zero)`; §6.7, *A dotted paren-less name in a LOGICAL position*) — and `rule r(1) :- flag` runs the operation's relational view where it used to fail silently — with `not(flag)` then *succeeding*, negation-as-failure reading a goal that could not run as a proof. Which reading a rule body takes is **type-directed** in exactly the same way: an arrow-typed slot lifts, everything else calls. Only a **nullary** operation has the second; for every other operation a zero-arg call is an arity error (§"A call must fill the parameter list exactly"). So a **non-nullary** bare name has exactly one reading, and it needs an **arrow to lift against**: written where the slot's declared type is not a function type — an `Int64` field or parameter, a type parameter the call site has not pinned to an arrow — it denotes **nothing**, and that is a load error naming the operation. The same error covers an operation with no function-value form at all (a body-less builtin or spec declaration, which cannot eta-lift because the typer's accepted set must stay a subset of the evaluator's), reported at the reference rather than wherever its return type first fails to conform. **The refusal is not a narrowing**: every position that could lift still lifts, through the function-typed-slot rule above, which is the only one that ever had an arrow to lift against. Lifting *without* one is what this rule forbids, and it is forbidden because such a value cannot be **minted**: the expected arrow is what pins both the requirement dictionary and the argument-spread labels (§"Applying a lambda", WI-1087), so an operation lifted against nothing yields a function value that dispatches on an empty dictionary and spreads its arguments by source order — a silently wrong answer, measured. Until WI-20260828-2TMB5 the zero-arg reading was ungated, so a non-nullary bare name silently denoted its **return type** in any slot the eta rule declined: `entity plain(v: Int64)` fed `plain(inc)`, with `inc(x: Int64) -> Int64`, loaded clean because `Int64` *is* the declared field type, while the applied spelling `plain(inc())` was an arity error. That fall-through was the same one WI-1063 found laundering an existential return and WI-1083 a ∀; both repaired the *eta* rule so fewer references reached it, and this one repairs the reading itself.
...
**A selected witness is validated at the call site** (proposal 058 §3.5). The witness is the sort a selection names, as `ByLength` in `[O = ByLength]`. It must provide the spec **at the call's own bindings** — a witness that provides it at other bindings is as loud as one that provides it not at all — and it may not be a **concrete** provider, a sort with constructors. The reason is the case the rule is for: where the call's arguments are that sort's own values, they carry their sort and already direct the dispatch, so an explicit witness could only repeat the value's choice or contradict it. It is refused rather than preferred. The rule is read off the named sort alone, deliberately, so that whether a sort may be named can be seen from its declaration. A sort with constructors is therefore refused even where none of its values takes part in the call — an ordering of `String` that declares an entity of its own is refused although the arguments are strings (WI-20260911-TX0G6 measured that case and kept the rule). To make such an ordering selectable, declare it without constructors. A selection applies to the call's **own** goal and not to the sub-goals a conditional witness resolves: those search, as they always did. To steer one, name the witness's own slot in the key's value position (`fold[Monoid = ListM[O = MyEq]]`), where the selection happens at the witness's boundary as a callee slot again.

**Both call-site spellings validate alike, and a type is held to the first rule only** (WI-20260911-TX0G6). A companion receiver's binding of a named slot, `SortedSet[T = String, O = W].empty()`, is refused exactly where the callee's `SortedSet.empty[T = String, O = W]()` is, and in the same words. That covers a value with no sort head, a sort that provides nothing, and a concrete provider. A **type** that carries a named slot's witness is different. It is the type of a parameter, a result or a `let`, or the type inference writes for a bracket-less construction. The witness must still provide the spec there, but it may be concrete: `SortedSet.empty[T = Pair[Int64, Int64]]()` itself infers `O = Pair`, the prelude's own ordering of pairs, and every later call reads that back out of the value's type. The concrete-provider rule is therefore a rule about what may be **written** at a call. It is not a rule about what a type may say. A named slot bound at a call to one of the enclosing declaration's own **parameters** selects nothing. It is a **forward**: `[O = OE]` inside a sort declaring `requires OE: WeakOrd[E]` means whatever that sort's caller chose for `OE`, and it means the same in either bracket spelling as in a type. A forward is answered by the enclosing frame's dictionary **for that parameter**, and by nothing else. That dictionary is the parameter's own named slot, on the sort or on the operation, or a slot a requirement of the enclosing declaration binds to it: `requires PersistentCollection[C = SortedSet[T = E, O = OE], …]` holds `SortedSet`'s own `O` dictionary, for `OE`. A requirement that merely covers the slot's goal is not the parameter's and is never used in its place — another parameter's slot of the same spec, or an anonymous `requires WeakOrd[E]` — and no dictionary is constructed for the goal either. The value was built with the parameter's provider, and any other answers for a different ordering. So a **plain** parameter (`[O = P]`, or a set typed `O = P`, beside `sort P = ?`) that no such requirement ties to a dictionary is refused. The refusal is the same on every route a forward takes: a direct call, a call through the spec, an operation used as a function value, and either bracket spelling (WI-20260923-WN9P8). A key reached by a spec short name (rung 2 above) binds no parameter, so it has nothing to forward through, and even a slot-naming value there is refused.
...
**A DOT call carries no bracket, and now says so** (WI-20260829-BAD3V). A method call on a *value* receiver — `?xs.map[Dst = Int64](f)`, `xs.map(f).map[Dst = Int64](g)` — desugars to `dot_apply`, which has no type-argument channel: a dot is **bracket-less by construction**, and the typer's tier-1 reasoning reads that as a premise. The bracket is nevertheless **parsed** there, so the refusal is a located error naming the applicative spelling (`Sort.map[…](…)`) instead of a syntax error on the `[`. Before this, the bracket was admitted exactly where the callee was a **name path**, which is precisely where the call is *not* a dot call, so no genuine dot call could take one at any depth, and a diagnostic advising `map[…](…)` on a chained hop sent the author to a spelling the grammar refused. **Both routes to a dot call are covered, and they are refused in different phases**: `?x.m[…](…)` is a distinct production, so conversion refuses it; `xs.m[…](…)` where `xs` names a *local binding* is written with a bare name callee and only becomes a dot call in the **loader**, when the head segment resolves to a binder — that one is refused there, with the same sentence, and the bracket alone is dropped so the rest of the call is still checked. The message names the applicative **shape** and no position, because the positions that reach it differ (a rule head has no operation body to move into); *where* the bracket is read is the separate, position-aware refusal above. A **qualified companion** receiver keeps the bracket and reads it as the call's type arguments: `Map[K = String, V = Int64].empty[T = Int64]()` means what `Map.empty[T = Int64]()` means. Off a call the bracket is not admitted at all (`?x.field[T = Int]` is a syntax error) — there is no `(` there to separate it from a trailing `[…]` meta block.

**A companion receiver's bracket BINDS the sort's type parameters for the call** (WI-20260829-W6JH0, WI-20260911-RS2G4). `Map[K = String, V = Int64].empty()` — proposal 035 form (3) — means what form (1)'s `let m: Map[K = String, V = Int64] = Map.empty()` means, and 035 says how: it "desugars to `empty()` resolved within the scope of `Map`, with `K` and `V` bound at the call". So the receiver writes the **same** channel the callee bracket writes — rule (1) above spans the operation's parameters *and its enclosing sort's* — and the two spellings `Map[K = Bool, V = Bool].size(m)` and `Map.size[K = Bool, V = Bool](m)` are one call with one verdict, reported at the same site with the same words. Previously the bindings were erased when the callee was flattened to its name segments, so the bracket constrained nothing at all; W6JH0 then read it in **one** place — as the call's result, and only where the callee's declared return was the receiver's own sort — which left it a **silent wrong answer** on every other member: `Box[T = Int64].empty()` in an `Option[T = Letter]` position loaded clean, because nothing else carried `T` and the expected type pinned it instead. Typing the result is now one *consequence* of the binding rather than the rule: a callee returning some other type (`Map[…].size(m) -> Int64`) is checked against the receiver's bindings through its arguments, exactly as the callee spelling checks it. Where both brackets are written and bind one name differently, that is a **contradiction the author wrote** — one error naming the parameter and both sources — and not a precedence question, since both now bind one variable. The **callee** bracket still does not reach a **bare self-sort return**: `Map.empty[K = Bool]()` binds `K` in the call's substitution and a constructor's bare self-sort return is deliberately left untied (§ the self-tie rule), so nothing lands there; the receiver's own claim is what types such a result.

**An unwritten slot in a bracket VALUE is a fresh variable, not an absent one** (WI-20260911-RS2G4). `[T = List]` says *a list of something*, and the something is a variable an argument can pin — so `Box.mine[T = List](box(v: [1]))` against a declared `Option[T = List[T = String]]` is refused naming both inner types, exactly as the bracket-less call is. Written bare it was an absent slot, which width-ignoring unification let the context fill, so the argument's element reached nothing. A slot that is a **named requirement slot** is the exception and stays unwritten: its value names a *provider*, chosen by the resolve hook at the goal's bindings, and a variable there would say "some provider" where the author wrote nothing.

**A sort's type parameter reaches a member's BODY** (WI-20260911-RS2G4). A member reading `T` reads the *receiver's instance*, so `operation selfType() -> Type = Box[T = T]` called as `Box[T = Letter].selfType()` evaluates to `Box[T = Letter]` — the WI-708 rule one scope up, on the same call-site channel. A **bare sibling call** inside a member body (`selfType()` written with no receiver) means "the instance I am running at", so it carries no binding of its own and inherits the enclosing frame's — unless the call site chose explicitly, which a written bracket does.

**A rigid type is a VALUE only where a requirement says so** (proposal 065, WI-20260919-N31XX; amends 055 §2's type-parameter bullet, supersedes WI-20260918-R541X's paragraph). Reading a type parameter `B` in **value** position — a bare `B`, a `B` inside a type expression (`Cell[V = B]`), a `Type`-slot argument — is well-formed only when `TypeValue[T = B]` stands among the requirements in scope, the operation's own or its enclosing sort's. Otherwise it is a **load error** naming the parameter, the read's line and column, and the clause to add. Without it a signature `f[B](x: B) -> R` promises nothing about whether `f` inspects `B`: parametricity is false, a provider's own parameter entered through a requirement slot has no call site that could supply it, and an erasing backend has nothing to read. The clause is **explicit, never inferred** — the signature is what a caller and a spec read, and a body-less spec operation has no body to infer from. It does **not** touch type positions (`x: B`, `-> List[T = B]`, a `requires` bracket), which are static and erasable; concrete sorts in value position (`Cell[V = Int64]` reads no rigid, and its `TypeValue` is the derived instance, discharged statically); or rule bodies, which unify types rather than read them. The read is judged **after `@[simp]` expansion**, so a parameter that inlining places in a type position — `dq[K]() = size(put(mkq(K), …))` — is not a value read.

**And a caller may not FORWARD a rigid into an operation that inspects it** without holding the evidence itself (065). The rule above refuses a body that *reads* a rigid; this refuses one that hands its own type parameter to an operation whose `requires TypeValue[…]` names it, while declaring no such clause of its own — `mid[U](y: U) = tyOf(y)` against `tyOf[B] requires TypeValue[T = B]`. The refusal is at the **call**, because the caller's signature is legal on its own and it is this call that needs what the caller has not got; the repair is to declare the same clause on the caller, or to stop passing the parameter. Without this half a signature could still quietly depend on a type it promises nothing about, which is what the rule exists to prevent. An unsuppliable operation-level slot is otherwise a *silent absence* — a body may declare a requirement it never reads — but `TypeValue` is never such a case, because `type_value()` is nullary and the dispatching dictionary is the only carrier of its answer. §5.2 now refuses the same **forward** for any spec, on the carrier rather than on the name; this arm survives it, and is checked first, because it is stronger where it applies: it needs no evidence from the callee's body, and it reaches a `TypeValue` requirement whose carrier is *not* the caller's parameter.

**A value read of a rigid IS a requirement dispatch** (065 §1). Reading `B` in value position is not a lookup of the caller's type argument but a call to `TypeValue[T = B].type_value()`, answered by walking the dictionary that discharged the clause — so the evidence and the answer are the same object, and a backend that erases types has something to read. A type EXPRESSION mentioning `B` (`Cell[V = B]`) changes only at the leaf: the argument pump already evaluates each argument and builds the application, so it was always the builder. This holds for a clause written on the OPERATION and for one written on the enclosing SORT alike. A consequence worth stating: a call that leaves the read type only PARTIALLY determined — a bracket value with an unwritten slot, or a spec parameter two clauses both name — can no longer supply the evidence, so it is refused at load where it used to load and fault at run time.

**Where the clause goes is decided by who supplies it** (065 §3). An **operation-level** `requires` is an extra input the caller supplies per call, and a caller dispatching through a spec knows only the spec's signature — so an implementation's operation-level requirements are a **subset** of the spec operation's, and a direct call to the member does not relax it. A clause over the provider's **own sort parameters** rides the **instance** instead, is supplied by the dictionary's builder at selection, and may therefore be added freely. So a member reading its enclosing sort's parameter carries the clause on the **sort**; a member reading its own type parameter carries it on the **operation**.

**A requirement whose carrier is a STRUCTURAL FORMER and has no provider is refused at the call** (065 open question 3). A `provides` row is written on a **sort**, and a tuple or an arrow is not one, so a former cannot CARRY a provision; the derived instances (§8.4) cover sorts only. When nothing provides the spec at that former the slot can never be filled, and the call is refused rather than left to die at run time. **A former is not unprovidable, though** — a *sort* may carry a row whose spec binding is a former (`sort Wrapper … provides TT[T = (a: Int64, b: String)]`), and that satisfies `requires TT[T = B]` at the tuple. So the refusal is about this program having no such row, and its repair names the one that would work: give some sort `provides Spec[… = <the former>]`, or pass a value whose type is a sort. It is located at the **call**, because a body reading `B` cannot see that some caller will pass a tuple. The verdict is **carrier-shaped, not spec-shaped** — nothing about `TypeValue` makes a former unprovidable, so the rule holds for a user spec identically. **One exemption:** a **body-less** callee, whose absent slot is the host's to interpret rather than an abort. `Error.reify` is the live instance — it declares `requires ErrorTag[T = T1]`, has no anthill body on purpose (the boundary is a frame the interpreter installs by symbol), and at a payload it cannot narrow it **catches wide**, as every boundary did before narrowing existed. An anthill body has no such fallback: §8.4's lowering makes a value read a dispatch through the slot, so an absent one dies `DeferToRequirement` with no handler able to catch it, which is the outcome this rule exists to prevent.

**Every `TypeValue` instance is derived** (065 §2, WI-20260919-HXGXF). A sort's instance is the loader's; a parametric sort's is conditional on its parameters (`Box provides TypeValue[T = Box[V = V]] requires TypeValue[T = V]`). A hand-written `provides TypeValue[…]` is a load error: a forgeable instance would make every type read a claim instead of a fact, and one answering the wrong type would route a reify boundary, a `facts_of` or an error tag to the wrong sort, silently.

**A body read still reaches the CALLER's type through generic callers** (WI-20260918-R541X), which is what the requirement above now licenses rather than replaces. Inside `operation g[P](x: P)`, the call `Err2.tagOf(x)` binds `Err2.T` to `P`, and what `P` stands for is decided only when `g` is called; the binding rides as a reference to `P` and the callee's frame grounds it against `g`'s at run time. This holds for a sort parameter exactly as for an operation's own, and for a `P` nested inside a type argument (`tagOp(box(x))` → `Box[V = String]`). It never applies inside an effect row, where a parameter is a row *tail*. A receiver-less member of a spec, called where the enclosing scope has **exactly one** `requires` clause over that spec and the call pins none of the spec's parameters, runs at that clause's bindings. A parameter that no route binds is **not a type**: reading one is the run-time error *type parameter `…` has no binding in the frame of `…`*, which is now a **backstop** rather than a diagnostic an author meets — the load rule above refuses the program first. **A provider's or witness's own parameters on a member reached through a requirement slot** (`Box provides TypeTerm[T = Box[V = V]]`, whose member reads `V`) used to be the one gap keeping it live, and is closed: the slot's dictionary still carries no type bindings, but the answer no longer needs one, because the evidence that selected the provision IS the type (065 §1/§4). The backstop remains for a KB built without the typer, over which no load rule has passed.
...
sort may become an effect kind — but becoming one is a declaration, and it is an ordinary
provision of `anthill.prelude.Effect`, with one spelling:

```
sort Kind
  provides Effect[T = Kind]    -- in the kind's own body
end

namespace Kind                 -- or at its address, where it has no body
  provides Effect[T = Kind]
end
```

A namespace-level `fact Effect[T = Kind]` said this until WI-20260917-S8JYF retired the
`fact` spelling of a provision (058 §4, §5.1); it is now an ordinary fact and registers
nothing. What is registered is the **kind**, not an application of it: `provides Effect[T
= Modify[?]]` and `provides Effect[T = Modify]` register the same thing, since a label is
judged on the sort it names — `Tag[T = Int64]` asks about `Tag`. The prelude writes the
`[?]` form to say "for any target", and that reads well, but the argument is inert either
way.
...
host-language escape hatches. The vehicle is the operation's trailing `MetaBlock`,
`@[...]` (§7), after its clauses and body like any declaration's:

```
operation get_values(self: GPS) -> Vec3
  @[Vec3FromConstDoublePtr3, Profile: "cpp20-stl"]

operation step(self: Robot) -> Unit
  effects Modify[self]
  @[CppName: "step", CppBody: "self->step();"]
```
...
**Why `@[`.** A block opens with the single token `@[` (WI-20260915-G9EA9). A bare
`[...]` right after the return type would be return-type application arguments
(`-> Vec3[...]`) — exactly the clauseless getter shape that most needs markers — and
WI-087 once worked around that with a leading `meta [...]` clause. `@[` cannot continue
a type, so the block trails the operation as every declaration's does, and the
`meta [...]` clause is refused with a message naming `@[...]`. An operation has one
block; a second is a syntax error.
...
**A `fact` head declares its predicate at the scope it is written in**, exactly as the
`rule p(…) :- true` spelling of the same clause does — the desugaring above is the whole
of the reason, and there is no exception to state (§5.3, *A rule-introduced functor is
scoped where it is written*).

It did not, until WI-20260821-RDGQC. This paragraph used to record a **known gap**: the
head reached the bare global intern, so two scopes writing one fact name shared one
uncitable predicate, and `fact p(…)` and `rule p(…) :- true` were **not** the same
program — only the second gave its predicate a name its own scope could cite. MEASURED,
with the `rule`-shaped control beside it: two namespaces each writing `fact pp(…)` and
reading it back answered **2 each**, each reading the *other's* fact, where the same
program written `rule pp(…) :- true` answered 1 each, its own. One clause, two spellings,
two programs — the defect class WI-20260821-P85Z7 and WI-20260902-CZJ2N each closed for
another pair of spellings. It holds at **every arity**: `fact holds` and `fact holds()`
are alike in declaring, as they were alike in not declaring.

An author who wants a fact-only predicate still writes one — `fact parent("a","b")`
introduces `parent` in the scope it is written in and needs no other declaration. What
changed is only *which* scope: its own, rather than a global one every file shares. Where
two scopes should contribute to ONE predicate, the declaration is a body-less `rule
p(…)` in the owning scope with a named import of it in each other contributor (proposal
061), and a `fact` joins it like any other clause.

**A fact head's two nullary spellings are ONE TERM** (WI-20260902-CZJ2N), which is the
half of "alike" that was not true before: they were alike in what they *introduce*
(nothing) and different in what they *assert*, so `fact holds` was invisible to the goal
`holds()`. And where the head names a registered **entity**, the bare spelling is §8.3's
all-fields-fresh pattern: `fact account` is `fact account()`, the universal claim, where
it used to assert a phantom `account/0` atom that no spelling could query.
...
- **A free-standing entity is a CONCRETE carrier, so its provisions are checked**
  ...
  `sort` spelling, so a provision naming `Vec3` carries the same obligation any other
  ...
  A free-standing entity has **no body to write the claim in**, so its provisions are
  written in a `namespace <Entity> … end` SECONDARY ENTRY at its address (059 R2/R3) —
  the one position available to it, and the spelling since WI-20260917-S8JYF retired
  `fact Spec[Carrier]` (§5.1).

  It follows that **a provision belongs in the closure where its backing exists**
  (proposal 038): if a spec's operations are host primitives, the entry carrying
  `provides Spec[…]` is written in that host's binding layer, not beside the carrier in
  the language-agnostic library, where nothing backs them. An entry may stand anywhere
  the address can be spelled, which is what makes that possible. A host implementation
  named for a **spec** op says that operation has an implementation; it never says a
  given **carrier** is realized — that is a separate declaration, and conflating the
  two lets any carrier claim the spec (WI-876, WI-931).

  **The obligation follows the claim, not which declaration came first** (WI-978). It
  reaches a provision written in `X`'s own body and one written in a `namespace X`
  entry identically. The loader files the provision by asking whether the enclosing
  ...
  *The history here is worth keeping, because the shapes it refused are what the
  retirement made unwritable.* A `fact Spec[Carrier]` at a namespace or a file's root
  scope derived its carrier from a binding, and one that named **no carrier the loader
  could read** — nothing after the spec's name, or brackets whose contents name no type
  — was about nothing and was refused (WI-933/WI-1106). Two bracket-less lines shipped
  in the stdlib and neither produced a provision while their bracketed neighbours did.
  The tempting repair, reading the carrier off the enclosing namespace's entity
  declaration, was rejected on the ground WI-978 states above: a namespace may declare
  more than one, so proximity would put declaration *order* back in charge of what a
  claim is about. A secondary entry answers the same question by ADDRESS, which is why
  the retirement could drop both the derivation and its refusals together.
...
**Infix operators** appear between terms. Since WI-20260825-KD9SW the **Functor** column
names the operation, not the token the desugar mints: an operator carries its target's
...
|----------|----------|-------|---------|--------|
| `\|` | 1 | Left | `..anthill.kernel.or` | `anthill.kernel` (goal) / `Bool` (value) |
| `or` | 1 | Left | `..anthill.kernel.or` | as `\|` (word form) |
| `&` | 2 | Left | `..anthill.kernel.and` | `anthill.kernel` (goal) / `Bool` (value) |
| `and` | 2 | Left | `..anthill.kernel.and` | as `&` (word form) |
...
constraint rather than a taste. It USED to be the implicit tier's doing — that tier mapped
one short name to exactly one qualified name — and WI-20260825-KD9SW removed the tier for
these twelve, so the surviving reason is the carrier's own table: a carrier providing two
specs that both declare `add` gets two
...
`requires`. It exists so that a minted `/` has ONE target: a `div` that were a member of
two specs would leave the operator with no single operation to name unless resolution
consulted the operand's carrier. (The reason was first written as the implicit tier's
one-short-name-one-address rule; WI-20260825-KD9SW retired that tier for the twelve and
the operator now carries `..anthill.prelude.Divisible.div` outright — which needs the
single base spec just as much, and for the same reason.) **The single base spec is
...
Higher priority binds tighter: `a + b * c` desugars to `Additive.add(a, Multiplicative.mul(b, c))`. Left-associative: `a + b + c` desugars to `Additive.add(Additive.add(a, b), c)`. Right-associative: `a ^ b ^ c` desugars to `pow(a, pow(b, c))` — `pow` short, since no spec owns it. None-associative: `a = b = c` is an error. (The eleven reachable spec-op targets are minted as absolute addresses; the shape is what these examples are about.)
...
**A dotted paren-less name in a LOGICAL position is the name it spells** (WI-20260901-719FJ). `A.B` alone lowers to the `field_access` chain above in *every* position, so what it lowers to does not say what it means — the POSITION does, and the qualification never does. Where a term states a **proposition** — a rule head, a `fact` head, a rule-body goal, a query pattern, a structured proof step's head or body goal — there is no projection to read, so the chain is the qualified name `A.B` and resolves exactly as the applied spelling `A.B(…)` resolves its functor: `rule nsx.tgt :- b(1)` joins the predicate `nsx.tgt`, `:- nsx.tgt` runs it, `fact nsx.tgt` asserts it, and `anthill query 'nsx.tgt'` answers it. Where a term denotes a **value** — an operation body, and any DATA slot inside a proposition — the chain stands: it is the dot projection of §6.7's three modes, or, when it names a rule in a statically-known scope, the `Relation[T]` value (proposal 052 §6.7), or — in a **rule body** — when it names a **nullary operation**, that operation's CALL wherever the chain is reduced as an operand: `?v <=> Box.zero` binds 0 and `0 = Box.zero` holds, as they do for `seven` (§5.4, *The two readings of a bare operation name*). The term itself still stands, which is what keeps a fact storing it matchable; an operation body is not included, and there the bare `Box.zero` names no operation. This is the same split the ONE-SEGMENT spelling already has — `person_row` is a relation value in an operation body and a nullary goal in a rule body (§8.6, *A PAREN-LESS nullary head is an application of arity 0*) — and until 719FJ the dotted spelling had only the value half: the other three lowered to a `field_access` application that heads no clause, so a head's rule was dropped in silence, a goal answered nothing (and `not(…)` of it *succeeded*), and a query pattern came back as the undischarged residual `eq(field_access(nsx, tgt), true)`. A data slot is deliberately NOT collapsed: `fact holds(nsx.tgt)` and the query `holds(nsx.tgt)` must build one term, and normalizing one side of a match is never a repair. Three gates keep the reading off everything else — the chain must be the converter's own (a HAND-WRITTEN `field_access(a, b)` is a call to whatever that name denotes at that scope — §8.6, *the short spellings remain ordinary identifiers*), it must carry no named arguments, and it must be rooted in a NAME rather than a value (`?x.f` is a projection and has no name to be).

**And once it is the name, it is the name's READING** (WI-20260902-VNWAW). Deciding *which symbol* is only half of a goal: what a symbol MEANS in a rule body is §8.3's and §5.4's question, and both of their answers apply unchanged under a dot. A dotted bare **fielded entity** goal is the all-fields-fresh pattern (`:- ns.account` searches what `:- ns.account()` searches, §8.3 *the POSITION decides whether the bare spelling is that pattern or a reference*), and a dotted bare **nullary operation** goal is that operation's call, because a predicate goal is answered by matching and an operation goal by reducing (§8.6, *A PAREN-LESS nullary head is an application of arity 0*). Neither was true when 719FJ shipped: the dotted branch returned the bare leaf, so `:- ns.flag` over `operation flag() -> Bool = true` answered nothing and `:- not(ns.flag)` **succeeded** — the same free proof, one qualification over, that 719FJ removed for a dotted predicate and WI-20260902-CZJ2N for a one-segment operation. The four other logical positions already read it this way, sharing one expansion; only the rule-body goal had forked.
...
**Surface.** A block is written `@[...]` — the single token `@[`, entries, `]` — and
trails what it annotates: a fact or rule (its clause metadata), or a declaration (§5.8,
and "Declaration metadata" below). On a clause it stands for the `Meta(...)` term the
clause carries:

```
fact parent("alice", "bob") @[trust: axiom, agent: "author"]
-- the fact parent("alice", "bob"), carrying Meta(trust: axiom, agent: "author")
```

There is no `meta:` surface form; the second line is a conceptual reading, not syntax.
The retired spellings — a bare trailing `[...]`, the operation `meta [...]` clause, and
`@ [...]` with a space — are refused with a message naming `@[...]` (WI-20260915-G9EA9).
In a term, `@` is the arrow-effect infix, and no effect is a list, so `?a -> ?b @ [simp]` is
refused on every parse rather than read as an arrow whose effect is `[simp]`.
A bare `[...]` competed with type arguments (`sort Ids = List [M]` read as `List[M]`)
and with a following rule entry's collection literal (WI-893), and lost the block
silently in both.
...
A bracket right after a type is that type's arguments, never a block:
`sort Ids = List [Int64]` is `List[Int64]`, and `sort Id = Int64 [M]` is refused as
over-application. The block is `@[...]` in every position, the four that end in a type
included — `sort Id = Int64 @[M]`, `effects E = ? @[M]`, `operation f() -> Int64 @[M]`,
`const K: Int64 @[M]` (WI-20260915-G9EA9).
...
**One principle, two engines (WI-20260911-RS2G4).** A sort's type parameter read inside a member is a **projection off the receiver's instance**, and where that instance comes from is what the two engines differ on. An **operation** READS it: Γ decides at the call — from an argument, from a companion receiver's bracket, from the callee's own bracket, or from the enclosing frame for a bare sibling call — and the body sees a rigid, input only. A **rule** UNIFIES it: an unwritten parameter of a bound variable's sort (`p(?x: List)`) is a rule-scoped type variable, de Bruijn-bound like a value variable and opened fresh per resolution, and it flows *both* ways — from a value's runtime type, from a body goal, from a citation's bracket. Within the sort's own definition the bare self reference is the parametricity tie instead, not a projection (§3 of `docs/design/type-parameter-scoping.md`). Both halves are delivered (WI-20260911-5G28A for the rule), with one piece of the rule half still open. A citation of a relation declared in a parameterised sort reads its instance **at the citation**, as an operation call does: the receiver bracket (`Wrap[T = Colour].dom`), an applied argument, the expected type, or — inside the sort's own members — the enclosing instance. A parameter none of them fixes is a load error naming the `Wrap[T = …].dom` spelling. A citation also passes the caller's **requirement dictionaries** into the clause: each `require[X]` read of the cited relation whose instance the citation's types decide is filled from the caller's own `requires` slots — or built from provisions, at a concrete instance — so `viaRule[A](x: A, y: A) requires WeakOrd[T = A] = cmp(x, y)` answers through the provider its caller selected rather than one re-derived from the value. A read that arrives filled is checked against the clause's own derivation: where that derivation is not unique, the passed dictionary decides; where it is unique, the two must agree. What is still open is the instance of the sort's parameter itself — its domain — which will travel the same way (`docs/design/060-implementation.md` §7.3).

**What the rule engine's "unifies" is, concretely (WI-20260911-5G28A).** A type variable in a rule-head bound is an ordinary clause variable. It rides the rule's own **frame** — collected at the assert beside the head's and the body's variables, de Bruijn-closed with them, and opened fresh per firing — so two citations of one rule are two instantiations, exactly as two firings are. In a relational clause the enclosing sort's parameter is such a variable too — opened per activation and pinned by the value; an equation's stays the receiver's, read at match time. Three things follow, and they are the whole of the rule half:

* **It ties columns.** `rule my_rule(?x: List[T = ?t], ?res: List[T = ?t])` makes `res`'s element type whatever `x`'s is. A citation opens the bound per citation, so an argument PINS the variable and the surviving columns narrow with it: a concrete `List[T = Int64]` argument gives `res : List[T = Int64]`, and a **rigid** one (`operation op(x: List, y: List)`, where `x`'s type is the projection `List[T = x.T]` of §"How the slot is named") gives `res : List[T = x.T]` — so `-> List[T = x.T]` is accepted and `-> List[T = y.T]` refused, by ordinary σ-equality of one neutral. No receiver re-keying is involved: in a rule the tie IS a variable, not a path.
* **It is read off the value, never guessed.** A value's type is functionally determined by the value, so a bound with a free variable is decided in mode **(in, out)** — the value is the input, the type the output. `my_rule([1, 2], ?r)` answers `[1, 2]` definitely, and `my_rule([1, 2], ["a"])` fails, because the first column pins `?t := Int64` and the second is then checked against it. With the value unbound and the type still free there is nothing to range over: the goal **delays** and residualizes, loud at the drain, rather than enumerating one choice point per sort in the KB.
* **An unwritten parameter is a variable that was not written.** `?x: List` means `?x: List[T = ?t]` with a fresh rule-scoped variable, recursively at every depth (`List[T = List]` writes the inner one too) and per occurrence, so two bare `List`s in one bound are two element types unless the author ties them. This is §"How the slot is named"'s expansion on the rule side; the projection spelling (`?res: List[T = x.T]`) is not admitted in a rule-head bound, since a rule has no receiver to project off.

A **rule-bracket** introducer (`rule g[A](?a: A) :- Spec[A]`, WI-582) is a different construct and keeps its own reading: its guard records the **spec**, so the bound is nominal and the annotation is proposal 060 §3's requirement anchor. An introducer with no bounding guard stays a load error — merging the two would let one spelling mean "same type as" in one clause and "provides this spec" in another, decided by whether a guard happens to be written elsewhere.
...
**How the opened slot is REPRESENTED (WI-1079).** The ρ an opening mints is a logical variable of the *rigid* kind, and reflect names it: `anthill.reflect.extract` reports it as `TypeExtractor.Skolem(name, id)`, beside `FlexVar(name, id)` for a still-flexible one. The two are separate forms because they answer opposite questions — a skolem unifies with **nothing but itself** (it is the opaque constant a consumer may assume nothing about, which is what makes the opening sound), while a flex variable unifies with **anything** (an instantiated `∀`, the type `empty()` has before its context pins it). `id` is the identity and `name` is only what a diagnostic prints: two skolems minted for one parameter name render alike (`?E` vs `?E`) and are different types, so a consumer comparing them must compare `id`. Distinguish all three from `TypeVar(name)`, which is neither — it is the *placeholder* for a type the extractor could not name, carries no identity, and is not a variable at all. **An un-annotated lambda binder is NOT one of these (WI-20260904-50B2K).** Its type is *to be inferred*, and the two want opposite behaviour — a placeholder must not commit, inference must — so it is a real flexible variable and reports as `FlexVar`. What stays a `TypeVar` is a type nothing will *ever* supply: the carried type of a runtime value whose typing was never decided, and a sub-pattern of a constructor that declares no field types (`case SetLiteral(a, _, _)`), where the missing thing is the declaration and no enclosing type can supply it. The distinction is by *reason for the absence*, not by position: a component of a binder list whose own type is still an inference variable is itself to be inferred, while the same component under an undeclared constructor field is not. The **bound** variable of a `∀` arrived with WI-1083's binder: it is an element of a `TypeExtractor.PolyType`'s `binders` list (§4.4, §5.4), which is a list of variables precisely so that `id` says which occurrences of the quantified body it binds. Instantiating a `PolyType` turns each binder into a fresh `FlexVar`; opening an existential turns a slot into a `Skolem`. The three forms are one lifecycle.
...
A **list or set literal** carries the variant to its **elements**, in an argument slot and at a declared return alike (and an element type that is *callable* rather than a variant is pushed the same way, which is what makes `head_apply([inc], 41)` agree with its `cons`-spelled twin) — `takeReds([red(v: 1)])` and `-> List[T = Colour.red] = [red(v: 1)]` both admit it, and a sibling element is refused naming its own constructor (`expected red, got blue`). That position was excluded when this section was first written, and the exclusion was about the *element check*, not about variants: a literal took its element type from the declaration without the elements being read, so pushing the variant in would have replaced a correct refusal with a silent accept. §4.6 now checks every element against the declared type, which is what makes the push sound (WI-20260826-7JDWY).

Two positions do **not** yet admit a variant, and each has its own reason rather than being an omission:
...
**Partial entity patterns:** When an entity term appears with fewer named arguments than the entity declares, the missing fields are automatically generalized to fresh anonymous variables. This means `account(owner: "Alice")` is equivalent to `account(id: ?, owner: "Alice", balance: ?)`, and `account()` is equivalent to `account(id: ?, owner: ?, balance: ?)`. The expansion applies whenever the functor is a registered entity — including the zero-argument case.

**And the POSITION decides whether the bare spelling is that pattern or a reference** (WI-20260902-CZJ2N). This sentence used to read "bare `account` without parens remains a reference to the entity/sort", full stop, and that was true of every position — so `fact account` asserted a phantom `account/0` atom that `:- account()` could not see and no other spelling reached either. Now:

- In a **logical position** — a rule head, a fact head, a rule-body goal, a query pattern, a proof step — a bare entity name **IS** the expansion. `fact account` is `fact account()`, the universal claim; `:- account` searches what `:- account()` searches. This is what the type level already did one rung up: `fact Monoid` is `fact Monoid[?]`, a bare spec name auto-bound.
- In a **data slot** or a **type slot** it remains the reference to the entity/sort. A term's spelling is its identity in a data slot, and `Ref(WorkItem)` is the sort-as-value that `facts_of(kb(), WorkItem)` reads.

This convention avoids requiring the user to explicitly list unneeded fields with `?`. (Its type-level counterpart — fresh variables for the unbound *parameters* of a parametric sort used as a type — is **expansion during unification**, §8.1.)
...
refused for a `fact Spec[X]` beside a `provides Spec[X]`, back when the two were
two spellings of one claim. Migration was **zero**,
...
identical fact and rule count. The sites had to be found by reading, not by running.
WI-20260904-B8ESG closed the position: a head argument that names nothing is now refused
too (§5.3 "Naming one from elsewhere").
...
**Spec satisfaction:** To declare that a concrete type satisfies a parametric spec, write a `provides` clause — in the carrier's own body, or in a `namespace <Carrier> … end` secondary entry at its address when the carrier is declared elsewhere:
...
- **A non-ground pair**, which fails open here as everywhere else. Ground is a property of the type, not of how it is stored: a type carrying a literal argument (`Foo[T = Int64, N = 3]`) is ground and compared, with the provision's bindings substituted beneath it too — `-> Foo[T = T, N = 3]` under `provides Sp[T = Carrier]` is compared as `Foo[T = Carrier, N = 3]` (WI-20260923-Z1Q8B; until then every such pair failed open).
...
A sort-level `requires` keeps its meaning (it conditions **every** provision, and supplies every body's evidence) and the two compose. **Each provision has a dictionary layout of its own** (066 §7): its chain is the carrier's sort-level `requires` followed by that provision's conditions, and it is the frame of the provision's members and the provider half of every dispatch through the provision; an operation outside every block has the sort-level chain alone. The sort-level slots carry one name in every chain of a carrier, so a body of the carrier calling another reads the caller's frame by name where the callee's chain is the caller's or the sort-level one; a direct call to a member of a provision the caller is not in builds that member's dictionary at the call, its conditions resolved in the caller's scope (the caller's own `requires` included). A member backs **only its own provision**: another spec of the carrier that declares a member of the same name has no such member from it, and is refused at load unless backed otherwise. Two or more clauses of one spec are **alternatives** — the provision holds where one clause's conditions resolve — and contribute no slot; such a spec may not carry a `where` block yet (a member is still named `Carrier.op`).
...
**A host-backed operation says so in its declaration (WI-20260922-BRT4Y).** `operation_map` answers "in this host, WHICH function", and only once that host's binding layer is loaded. The question the declaration itself must answer is a different one — *is this operation body-less on purpose?* — and it is answered by the attribute `@[host_implemented]` (§5.8):

```
sort Int64
  operation compare(a: Int64, b: Int64) -> Int64 @[host_implemented]
end
```

Without it a body-less operation is three things wearing one face — a spec member awaiting dispatch, a host-backed operation, and an unfinished mistake — and the loader could tell them apart only by whether some binding block happened to map it, which is why the stdlib loaded without its binding layer LOADED CLEAN and died `OperationBodyMissing` at the first call. The attribute is the CLAIM and a binding block's `operation_map` is the EVIDENCE, and the load holds them to each other, both ways:
...
Each entry reaches the KB as an `anthill.realization.OperationMapping` fact (`carrier`, `operation`, `host_fn`, `lang`) — the flat, queryable form the realization tables use, so the runtime's registry reads facts rather than a hardcoded list. **The SLD engine's registry reads them too (WI-879).** A carrier member is a goal in its own right — a bare `gt(?a, 0)` written in a rule inside `sort Int64` resolves to `Int64.gt`, not to the spec op — so it needs a resolver primitive of its own or it stops computing, silently. What that primitive is follows from the clause and is not a separate choice: the member overrides a spec op that already has one, so the tag is **mirrored** from the spec op, and adding an `operation_map` entry to a new carrier's binding is the whole of what a carrier does to get the resolver behaviour. Mirrored only where the spec op is reached **through a spec the carrier provides** and only where the member is **host-mapped**: a member with an Anthill body keeps it (a primitive would shadow it), and a same-short-name kernel builtin is not a spec op (`Bool`'s mapped `not` is not negation-as-failure). A spec op with no primitive behind it mirrors nothing — `Ordered.compare` has none, so a mapped `compare` gets none.
...
-- =================================================================
--
-- A trailing `@[...]` block is the declaration's or clause's metadata: on a
-- fact or rule it is the clause's meta term, conceptually Meta(...):
--   fact X @[trust: axiom, agent: "author"]
--     ≈ the fact X carrying Meta(trust: axiom, agent: "author")
-- (a conceptual expansion — there is no `meta:` surface form).
--
-- The tested-N surface form desugars to the tested(N) constructor:
--   tested-47  →  tested(47)

MetaBlock   ::= '@[' MetaEntry (',' MetaEntry)* ']'   -- `@[` is ONE token (WI-20260915-G9EA9)
