...
**A rule-introduced functor is scoped where it is written (WI-894).** A rule head that names something no enclosing scope defines *introduces* that name, and the name belongs to the scope the rule is **written in** — the sort when written inside a sort, the namespace when written at namespace level. This is the rule `operation`, `entity` and `const` already follow; a rule functor is not an exception to it. Consequently two sorts may each define a `pick` by their own laws, and each sort's calls reach its own; there is no global rule namespace for them to collapse into. The name a rule introduces is the **conclusion's own** functor, except for an equation (`lhs = rhs` / `lhs <=> rhs`), where the head functor is the *connective* and the introduced name is the **LHS head** — `rule ite(true, ?t, ?_) = ?t` introduces `ite`, never `eq`. An **equation is bodyless** (§8.3), so `f(?x) = ?y :- guard` is a predicate rule whose head names the connective, not a definition of `f`. Three shapes introduce nothing. A conclusion whose outermost form the parser *desugared* — `?x.m(?y)`, `?x.f`, `?a + ?b`, in either an equational or a predicate head — carries the desugar's functor, not the rule's. A **qualified** subject (`rule String.isEmpty(?s) <=> …`) *references* an existing name rather than introducing one. And a name that **already resolves globally** through the implicit prelude or the reserved kernel vocab is never captured: `rule cons(?h, ?t) <=> ?h` states a law about `List.cons`, and minting a scope-local `cons` would silently repoint every bare `cons` in that scope at a different function (the same reason an `eq`/`unify` connective head never mints `<ns>.eq` — WI-530).

**The label rule is a frozen boundary, not a semantics.** A label never suppresses an *equation*'s introduction — `bool.anthill`'s `ite_true:` / `ite_false:` are two labeled clauses of one `ite`, so the label names the clause while the LHS names the function. For a *predicate* head a label does suppress it. The stated reason — a labeled predicate rule already has an identity, so its head reads as a conclusion about an existing predicate (`rule bound: gte(?x, 3.0) :- gte(?x, 5.0)` is a lemma about `gte`) — applies word for word to the *unlabeled* twin, which introduces `<ns>.gte` anyway. So the operative rule is narrower than the stated one: the label is where the change stopped, measured (removing the carve-out broke 9 targets, every smt-gen policy render among them). A labeled predicate head that introduces a genuinely new name is therefore still globally scoped. Unifying the two needs its own decision (WI-896) — the obvious discriminator, "does the name resolve via the implicit prelude", re-triggers the WI-523 regression that a user's own `or` / `not` predicate head must keep its own Goal symbol.
