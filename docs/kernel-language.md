...
**The exception is a name with a TYPE reading, and there the SLOT decides.** A `SymbolKind::Sort` name keeps both spellings apart, because §8.3 and WI-391/WI-387 give them different meanings in a type position: `Ref(S)` is the dispatch **wildcard** a `provides Spec[T = T]` lowers to, and a nullary `Fn{S}` is the **concrete** spec identity. Merging them there is not a widening but a wrong answer — measured: with the gate removed outright, 792 symbols change spelling and `anthill.prelude.FiniteCollection` stops covering its own `requires`, so the standard library does not load. Everything else — a predicate, an operation, an equation functor, a namespace, a sort-nested constructor — has one term. **Which paren-less spellings remain unscoped, and why each is somebody else's:** a **fact** head is unscoped at EVERY arity (§6.1), so `fact holds` and `fact holds()` are alike and WI-20260821-RDGQC owns the fact rule whole; each functor of a **multi-head** rule, and a head inside a `provides … language … end` block, are likewise RDGQC's and WI-20260821-TTHRK's. One spelling is neither, and it is now **closed**: a **qualified** paren-less head (`rule nsx.tgt :- b(1)`) folds into a minted `field_access` chain, so it used to neither introduce nor reference and its clause landed under `field_access` — the whole rule dropped in silence, while `rule nsx.tgt()` referenced correctly. The chain is now read as the NAME it spells wherever the term states a proposition (§6.7, *A dotted paren-less name in a LOGICAL position is the name it spells*), so the two spellings reference alike and the head introduces nothing in both, a qualified name being a reference (WI-20260901-719FJ). A **marked absolute** paren-less head (`rule ..nosuch :- b(1)`) had the same two-spelling split and is closed with this one: it is refused when it names nothing, as `rule ..nosuch()` already was (WI-1075), and a resolvable `..tgt` joins its predicate in either spelling.
...
can cite. It holds at **every arity**: `fact holds` and `fact holds()` are alike here,
which is what makes the paren-less fact spelling the same gap rather than a second one —
unlike the RULE head, where the two spellings were opposite programs until
WI-20260821-P85Z7 (§5.3, *A PAREN-LESS nullary head is an application of arity 0*).
