...
-- `Ord provides WeakOrd[T = T]`, so a carrier writes ONE provision and the lower
-- floor is DERIVED; it also `requires WeakOrd[T]`, which is what puts a WeakOrd
-- dictionary inside an Ord one so an `Ord`-constrained body can reach `compare`.
