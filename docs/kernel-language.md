...
A **named** logical variable in a return (`-> Stream[T = Int64, E = ?E]`) is a fifth spelling this list does not cover, and the implementation does **not** open it — so the laundering chain above loads when the row is written `?E` and is refused when it is written `?` or omitted (measured). That is a divergence, not a decision: §"Sort composition" states that a sort binding written `?` *or* `?name` expresses existential quantification, the name buying only that it **binds across the term**. Sharing is what distinguishes the two spellings; it is not an exemption from opening, and a named variable occurring *only* in a return has nothing to share with. **TICKET WI-1078**, which must also keep the signature-bound uses working — `mplus(a: LogicalStream[?A], b: LogicalStream[?A]) -> LogicalStream[?A]` ties its return to its parameters, so the caller instantiates it and nothing should open it.
