...
A type argument may also be a **value** rather than a sort — `Modify[store]` indexes the `Modify` effect type by the value `store` (proposal 027.1: `Modify[c]` on a parameter, `Modify[result]` on a return, `Modify[result.a]` per projection). This is *value-dependent* typing: the value is carried as `Type.denoted(value: <occurrence>)` so it is recognised as a value indexing the type, not a sort reference (WI-302). The grammar already admits the surface forms (names and field projections in `[...]`); `denoted` is the type-term representation that keeps the value distinct from a `sort_ref`.

This is distinct from *constructing* a `Type` by a computation. An operation call that returns a `Type` (e.g. building `parameterized(base: sort_ref(List), bindings: [TypeBinding(T, apply_subst(env, t))])`) is ordinary expression code that applies the `Type` constructors directly in term position — it is not type-annotation syntax and does not use, or need, the `[...]` instantiation sugar.
