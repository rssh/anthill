...
The `lambda` keyword is **required**. A keyword-less `(x, acc) -> body` (or `x -> body`, or the effectful `x -> body @ E` form) in an operation/const-body expression position is not a lambda — the infix `->` there builds an arrow-*type* term, whose left-hand names would be read as value references, not binders. The loader rejects this shape with a targeted error suggesting the `lambda` keyword (WI-605). The diagnostic fires only when the term can't be anything else: it has exactly the desugared arrow's shape, and the name (`arrow` / `arrow_effect`) does not resolve to a local binding or to a callable (operation, constructor, or const) in scope — a genuine call to something the user named `arrow` keeps its meaning.
