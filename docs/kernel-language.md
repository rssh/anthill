...
**And the ladder is asked of the FINISHED program (WI-980, [proposal 059](proposals/059-secondary-entries.md) R6).** "Only when the ladder finds nothing" needs a *when*, because this is the one position whose own answers change the table it reads: every other name is defined before any name is resolved (the WI-321 cross-file invariant), while a rule head is *introduced* during the same pass that decides it. Asked against the scanned prefix, textual order decided the program — measured, `rule p(1)` beside `sort Rec { rule p(2) }` loaded as **one** predicate with two clauses when the namespace-level rule was written first and as **two** predicates, `p` and `Rec.p`, when it was written second, with `p(2)` answering `1` and `0` respectively; the same pair split across two files at one address split the same way, on whichever file the loader reached first.
...
**And a cycle ACROSS FILES is refused, because the split is one nobody wrote** (WI-20260821-E85J5). Splitting is the only order-free answer, but it is still an answer the program did not give — and what it costs a *use* is silent. A scope's own name beats an import: `resolve_in_scope` reads a scope's `locals` and returns before it consults any import or parent, so the predicate the tie-break just minted at `mA` **shadows** the `import mB.*` that made the cycle, and no ambiguity is raised because none exists. Measured with a paired control: `mA { import mB.*; rule p(1) :- true; rule usesp(?x) :- p(?x) }` beside `mB { import mA.*; rule p(2) :- true }` answers `usesp(1)`=1 and `usesp(2)`=**0**, while the same `mA` with no `p` of its own answers 0 and **1** — so the import works, and the local is what killed it.

The **shadow is not the defect; inventing it is**. A local beating an import is what every other name in the language does, and an author who *writes* `rule p(?x)` in `mA` gets exactly this and should. What the tie-break did was make that choice out of two files neither of which shows the cycle. So this is 061's own rule, reaching the one shape its file check could not see: that check is keyed on the **predicate**, and the tie-break splits the cycle into two single-file predicates *before* it counts files, so it reported nothing. The load is now refused, naming the scopes in the cycle and their files, and it names both repairs — one body-less `rule p(…)` in the scope that owns it makes the other scopes' heads its clauses, or one in **each** scope says they are separate predicates. Both are measured; the second reproduces the shadow, now as written text.

Two neighbours bound it, and each is a deliberate non-case. A **nested** cycle across two files is the paragraph above — the enclosing member owns, the heads become one predicate, and the ordinary "heads in more than one file" error reports it; same two files, same two heads, only the nesting differs and it decides which message the author gets. And a cycle **inside one file** is untouched, by 061's own file unit and its open question 3: both scopes are in front of the one author who wrote them, so there is no second party for a declaration to be an agreement with, and the split — shadow and all — stands there.
