...
**A body read of a type parameter reaches the CALLER's type through generic callers, or is refused** (WI-20260918-R541X). Inside `operation g[P](x: P)`, the call `Err2.tagOf(x)` binds `Err2.T` to `P`, and what `P` stands for is decided only when `g` is called. The binding therefore rides the channel as a reference to `P`, and the callee's frame grounds it against `g`'s frame at run time. This holds for a sort parameter exactly as for an operation's own, and for a `P` nested inside a type argument (`tagOp(box(x))` → `Box[V = String]`). It never applies inside an effect row, where a parameter is a row *tail*. A receiver-less member of a spec, called where the enclosing scope has **exactly one** `requires` clause over that spec and the call pins none of the spec's parameters, runs at that clause's bindings. So `operation tagOf[P](x: P) -> Type requires TypeTerm[T = P] = TypeTerm.valueOf()` gives the spec's default `valueOf() -> Type = T` the caller's type. A parameter that no route binds is **not a type**. Reading one is the run-time error *type parameter `…` has no binding in the frame of `…`*; the silent `Ref(T)` it used to deliver is gone. One gap is still open: a **provider's or witness's own** parameters on a member reached through a requirement slot (`Box provides TypeTerm[T = Box[V = V]]`, whose member reads `V`). The slot's dictionary carries no type bindings, so today that read is the error above.
