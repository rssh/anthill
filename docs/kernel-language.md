...
**Both call-site spellings validate alike, and a type is held to the first rule only** (WI-20260911-TX0G6). A companion receiver's binding of a named slot, `SortedSet[T = String, O = W].empty()`, is refused exactly where the callee's `SortedSet.empty[T = String, O = W]()` is, and in the same words. That covers a value with no sort head, a sort that provides nothing, and a concrete provider. A **type** that carries a named slot's witness is different. It is the type of a parameter, a result or a `let`, or the type inference writes for a bracket-less construction. The witness must still provide the spec there, but it may be concrete: `SortedSet.empty[T = Pair[Int64, Int64]]()` itself infers `O = Pair`, the prelude's own ordering of pairs, and every later call reads that back out of the value's type. The concrete-provider rule is therefore a rule about what may be **written** at a call. It is not a rule about what a type may say. A named slot bound at a call to one of the enclosing declaration's **own named slots** selects nothing in either spelling: `[O = OE]` inside a sort declaring `requires OE: WeakOrd[E]` forwards whatever that sort's caller chose, as the same binding written in a type does. In a bracket, only a slot forwards. A forward is answered by the slot's *goal*, which is keyed by spec and bindings, not by the name the type writes. So a **plain** parameter (`[O = P]` beside `sort P = ?`) could silently take a slot of the same spec that has nothing to do with `P`, and it is refused instead. The refusal is conservative: a plain parameter that a requirement of the enclosing declaration mentions (`requires PersistentCollection[C = SortedSet[T = E, O = P], …]`) is answered soundly out of that requirement, and the same call written without the bracket runs. A key reached by a spec short name (rung 2 above) binds no parameter, so it has nothing to forward through, and even a slot-naming value there is refused.
