...
An op-scoped requirement **does** occupy frame slots of its own, placed after its parent sort's (WI-822). A call site fills them from the call's own substitution, exactly as it fills the sort's; the operation's slots are the *operation's*, so a sibling member of the same sort neither inherits them nor is inherited from. What differs from a sort-level `requires` is not the channel but **when a body reads it**: an op-scoped requirement is served by value-directed dispatch wherever a runtime argument names the carrier — which is the ordinary case, and the reason the clause exists — and the body reads its dictionary only where value-direction cannot answer at all. That is one shape: a spec operation with **no** parameter (`operation zero() -> T`), which no argument can direct, dispatched where two or more providers answer. There a bracket cannot decide either (§4.4 check 3 of proposal 058 refuses an explicit witness over concrete providers, on the grounds that the value decides), so the operation's own dictionary is the only answer, and the same program written with a sort-level `requires` behaves identically. A slot the call site could not fill is simply absent, and a body that reads it fails at the read naming the running operation — never on a dictionary that could not be justified.
