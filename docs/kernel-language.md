...
**What the guard BINDS is part of the answer.** A variable bound only by the guard and used in the right-hand side — `headOption(?s) = some(fst(?p)) :- splitFirst(?s) = some(?p)`, the standard conditional-rewrite idiom — carries the guard's **witness** into the rewrite. Because the rewrite's result then depends on that witness, the guard must **determine** it: a guard with two different witnesses for a right-hand-side variable does not decide the rewrite and the rule declines, the same answer an under-determined guard gets. A guard variable the right-hand side does not mention is an ordinary existential and constrains nothing.
