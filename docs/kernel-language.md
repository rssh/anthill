...
**A companion receiver's own bracket types the call's result** (WI-20260829-W6JH0). `Map[K = String, V = Int64].empty()` — proposal 035 form (3) — means what form (1)'s `let m: Map[K = String, V = Int64] = Map.empty()` means: the receiver names the sort *with* bindings, and the call's result **is** that type, whenever the callee's declared return is the receiver's own sort. Previously the bindings were erased when the callee was flattened to its name segments, so the bracket constrained nothing at all — it did not even reject a parameter the sort does not declare. A callee returning some *other* type (`Map[…].size(m) -> Int64`) is untouched, since the receiver then describes nothing that comes back; its bracket is still checked against the sort's declared parameters, as every written type is. Where both brackets are written and bind one name differently, they are answering different questions — the receiver names the **result**, the callee bracket binds the callee's type parameters for the call — and it is the receiver that reaches the value. The **callee** bracket does not reach the result type: `Map.empty[K = Bool]()` binds `K` in the call's substitution and a constructor's bare self-sort return is deliberately left untied (§ the self-tie rule), so nothing lands.
