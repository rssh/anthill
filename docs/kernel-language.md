...
**This gate is also where the rule still launders, and "the call's arguments pin it" is too strong.** A member whose return leaves a slot of its *own* sort unwritten writes no binding at all, and a missing binding is width-ignored at the consumer — so `operation widen(s: MyStream[T = Int64, E = {Error}]) -> MyStream[T = Int64] = s` declared *inside* `sort MyStream` loads, and its result satisfies a parameter declaring `E = {}`. That is §8.1's headline exploit surviving on the self side, in **both** spellings alike (so it is not a residue of the named-variable exemption above). Minting a fresh skolem there was measured and rejected — it stops the prelude loading and fails 1590 tests — and the filler the *parameter* side uses for exactly this case, the receiver's own projection (`s.E`), has not been tried at a call. **TICKET WI-1082.**
