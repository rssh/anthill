...
**A rule body evaluates both a goal and a value slot; what it declines is a BODY-LESS operation.** The reading above is the design, and the limit is narrower than "the evaluator has not arrived" — two evaluators run, and one gate excludes the same class from both. Measured (**WI-20260825-P9Y67**), and each pair is a separating row:

* **at a goal** — `:- Bool.and(true, true)` answers **1**, `:- Bool.and(true, false)` answers **0**. `op_reducible_in_rule_body` admits an interpreter-mapped op alongside a bodied one (**WI-20260822-J38JE**); before that it read `op_body_node(f).is_some()` alone, so a host-implemented operation was callable from an operation body and inert in a rule body for no stated reason.
...
* **the gate** — `:- Bool.and(true, true) = true` answers **0**, because `reduce_operand` passes `dispatch_body_less: false` and `Bool.and` / `or` / `not` are declared body-less. That is deliberate rather than missing: an operand is a term a *rule wrote*, and a body-less spec op there may be **symbolic algebra** rather than a computation — `Set.insert` / `Set.empty` are exactly the terms the membership rules resolve over, and dispatching them would reduce data. (**WI-20260822-ZJZS7** recorded the `Bool.and` example and attributed it to host-backing; the attribution is the body-less gate.)
...
So the open work is to make the gate ask about the OPERANDS rather than about the declaration: a body-less host-backed call whose arguments are ground and concrete may reduce, and one whose arguments are unground or abstract must **delay** — the residual discipline the goal side already follows — instead of being declined wholesale.
