...
sort anthill.prelude.MapReadable
  ...
  operation multiget(m: MapReadable, keys: List[K]) -> List[Pair[K, V]]   effects E
...
### Batched lookup — `multiget`

`multiget(m, keys) -> List[Pair[K, V]]` looks up many keys at once: the result holds a `(k, v)` pair for each input key that is present, in input order; absent keys are omitted (so `length(result) ≤ length(keys)`). Its semantics are pinned to `get` — `(?k, ?v)` is in the result iff `?k` is in `keys` and `get(m, ?k) = some(?v)` — so a pure carrier's implementation is just `get` folded over the list.

The reason it lives on `MapReadable` rather than as sugar is the **effectful carrier**. With the access effect `E` non-empty, calling `get` N times discharges `E` N times — N database round-trips. `multiget` lets such a carrier override the default and satisfy the whole batch in one `SELECT … WHERE key IN (…)`, paying `E` once. That is the batching win the access-effect framing above buys: it is only expressible because the read carries an effect worth amortising. For a pure carrier (`E = {}`) `multiget` is a convenience with no effect difference.
