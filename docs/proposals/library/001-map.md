...
`deleteMany` returns `Int` — the count of input keys that were actually present and removed (keys absent from the map don't count), the affected-rows convention of SQL `DELETE` and Redis `DEL`. `setMany` returns `Unit`: every entry is written, so a count would be redundant with `length(entries)`. (This raises a singular-side question — whether `delete(m, key) -> Unit` should mirror as `-> Bool` "was present"; see Open Q 9.)
