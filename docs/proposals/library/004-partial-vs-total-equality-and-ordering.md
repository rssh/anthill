...
The split gives three distinct symbols/obligations — `===` (structural), `PartialEq.eq` (partial), and the `Eq` requirement (lawful) — and a module selects which by what it `import`s and what it `requires`; no context silently gets IEEE where it wanted lawful, or vice-versa. Each is a **single, globally-coherent** instance per `(spec, carrier)`. Anthill enforces this for the `Eq` family specifically, at load, via `AmbiguousEqDispatch` (WI-837): the semantic-equality dispatch index collects all three supply routes and refuses a second distinct `eq` target, because equality dispatches from UNIFICATION and there is no call site at which to select one. (The older, spec-GENERIC "ambiguous witness" refusal this paragraph used to cite was deleted by **WI-843** / proposal 058 phase 3b, which lets two nameable providers of one `(spec, carrier)` coexist and refuses only an unselected use site. The `Eq` family is exempt from that by mechanism, not by accident — proposal 058 §6.) That is all this proposal needs for the `Float` problem.
...
4. **Codegen** — `Eq`/`Ord`/`TotalFloat` emit a total `operator==`/`<=>` (and the `= default` for entity structs); `PartialEq`/`PartialOrd` keep `==`/`<`. *Half shipped:* the `PartialEq`/`PartialOrd` mapping is live (`anthill-cpp-gen/src/lib.rs:4279-4297`); the total comparator is **WI-1107**, which must also decide what a `NonEq` composite emits — a silently defaulted `operator==` there would compare `double`s by IEEE `==` and undo step 2.
