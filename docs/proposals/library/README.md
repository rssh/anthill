...
| [`008-term-view-and-operations.md`](008-term-view-and-operations.md) | Draft 2026-08-26 | Reaching a type's structure **from a rule**. `Type` already has a complete view — `TypeExtractor` (`SortRef` / `Parameterized(base, bindings)` / `Arrow` / …), reified by the implemented `anthill.reflect.extract`. But `extract` is an EVAL builtin, so a rule cannot reach it; and the resolver's bridge `extract_sort_ref(?inst, ?result)` **succeeds without binding `?result`** (measured; owned by WI-20260822-ZJZS7, *a host-backed operation does not reduce in a rule body* — this proposal DEPENDS on it and supplies one more row). Also: `TermRepr` / `KB.reify` are unimplemented and lossy — delete rather than replace; reflection rows declare `Term` where they mean `Type`. First consumer: `guardians/lib/safety.anthill`, whose tier 1 needs no new typer seam — a provision in a loaded KB IS the verdict. No driver WI yet. |
