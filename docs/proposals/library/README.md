...
| [`008-term-view-and-operations.md`](008-term-view-and-operations.md) | Draft 2026-08-26, updated 2026-08-27 | Reaching a type's structure **from a rule**. Its headline finding — 29 `anthill.reflect.*` host fns registered by hardcoded name, so **no rule could read a term** (and §5.2's *decided-false* soundness gap) — was **closed by WI-880** on 2026-08-27. What survives, now filed as WI-20260827-2YHZ3 and WI-20260827-1ZG70: a host op's arity+1 relational view does not bind (and neither does `<=>` — only a test against a KNOWN value works); a META-PREDICATE is suspended on a non-ground argument, though the functor of `some(?x)` is `some` whatever `?x` is; the resolver's `extract_sort_ref` **succeeds without binding** (unowned); `TermRepr`/`KB.reify` unimplemented and lossy — delete, don't replace; reflection rows say `Term` where they mean `Type`. First consumer: `guardians/lib/safety.anthill`. |
