...
| [`004-partial-vs-total-equality-and-ordering.md`](004-partial-vs-total-equality-and-ordering.md) | Draft 2026-07-06 | Split the conflated `Eq` into `PartialEq` (partial — `eq`/`neq`; `Float` provides, IEEE) + `Eq` (requires `PartialEq` + a **checked** reflexivity law; `Float` does not provide), and `Ord` into `PartialOrd`/`Ord`. Makes `requires Eq[T]` non-vacuous and closes the interpreter↔codegen Float-`NaN` soundness gap. Driver WI-644. |
| [`005-stored-references.md`](005-stored-references.md) | Draft 2026-07-24 | `FactRef`: a source-neutral opaque stored-row reference, paired with visible content by `StoredRef[T]`. Replaces resident-only `FactId` at the persistence boundary. Driver WI-780 / proposal 057 R3. |
| [`006-reflection-record-schemas.md`](006-reflection-record-schemas.md) | Draft 2026-07-27 | Replace the schema-less legacy `member` and `Description` metadata rows with declared `MemberInfo` / `DescriptionInfo` reflection records, making full-arity resolved reads possible without schema inference. Driver WI-834. |
| [`007-weak-vs-strong-ordering.md`](007-weak-vs-strong-ordering.md) | Implemented 2026-08-15 | Split the conflated `Ord` again: `WeakOrd` (total, and congruent w.r.t. `Eq`, but its kernel may be strictly COARSER — where a key-based comparator lives) under `Ord` (adds only the converse law, so the kernel IS `Eq`). One-to-one with C++20 `weak_ordering`/`strong_ordering`. `Ord provides WeakOrd`, so a carrier writes one provision and the loader derives the floor below; `SortedSet`'s comparator slot is `WeakOrd`, making its equivalence-class semantics declared contract. Driver WI-1109. |
| [`008-term-view-and-operations.md`](008-term-view-and-operations.md) | Draft 2026-08-26, updated 2026-08-27 | Reaching a type's structure **from a rule**. Its headline finding — 29 `anthill.reflect.*` host fns registered by hardcoded name, so **no rule could read a term** (and §5.2's *decided-false* soundness gap) — was **closed by WI-880** on 2026-08-27. **Corrected again 2026-08-27**: "a rule body can test but not bind" and "`extract_sort_ref` succeeds without binding" were BOTH the same reading artifact — `anthill query`'s answer line truncates a variable chain at one hop, so a head var bound by a rule-body BUILTIN renders `?_`. `WI-20260827-2YHZ3` is re-aimed at that reader; a rule body DOES bind a result, `<=>` binds the reduced VALUE, and the first consumer is writable today. What survives: a HOST op's arity+1 relational view answers 0 (already WI-938's follow-up); a META-PREDICATE is suspended on a non-ground argument, though the functor of `some(?x)` is `some` whatever `?x` is (WI-20260827-1ZG70); `TermRepr`/`KB.reify` unimplemented and lossy — delete, don't replace; reflection rows say `Term` where they mean `Type`. First consumer: `guardians/lib/safety.anthill`. |
