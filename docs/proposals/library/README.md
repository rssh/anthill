...
| [`008-term-view-and-operations.md`](008-term-view-and-operations.md) | Draft 2026-08-26, updated 2026-08-27 | Reaching a type's structure **from a rule**. Its headline finding — 29 `anthill.reflect.*` host fns registered by hardcoded name, so **no rule could read a term** (and §5.2's *decided-false* soundness gap) — was **closed by WI-880** on 2026-08-27. What survives: a host op's arity+1 relational view does not bind (write `f(x) <=> ?r`); a `SortProvidesInfo.spec` carrying open effect-row tails is not ground so the bridge correctly suspends — the consumer must accommodate that; the resolver's `extract_sort_ref` **succeeds without binding** (unowned); `TermRepr`/`KB.reify` unimplemented and lossy — delete, don't replace; reflection rows say `Term` where they mean `Type`. First consumer: `guardians/lib/safety.anthill`. |
