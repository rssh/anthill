...
| [`008-term-view-and-operations.md`](008-term-view-and-operations.md) | Draft 2026-08-26 | Reaching a type's structure **from a rule**. `Type` already has a complete view — `TypeExtractor` (`SortRef` / `Parameterized(base, bindings)` / …), reified by the implemented `anthill.reflect.extract`. But **no rule can read a term at all**: 29 `anthill.reflect.*` host functions are registered by hardcoded name rather than an `operation_map` clause, which §5.2 lists as a surviving decline (and a *soundness* gap — decided-false, so `not(…)` answers 1). WI-20260826-VPEWK closed WI-ZJZS7 but covers `operation_map`-mapped ops only; **WI-880** owns the rest. Separately and unowned: the resolver's `extract_sort_ref` **succeeds without binding**. Also: `TermRepr`/`KB.reify` unimplemented and lossy — delete, don't replace; reflection rows say `Term` where they mean `Type`. First consumer: `guardians/lib/safety.anthill`, whose tier 1 needs no new typer seam — a provision in a loaded KB IS the verdict. |
