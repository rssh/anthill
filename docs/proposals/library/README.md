...
| `iteration-collection.md` | Formalise the read / persistent-build / mutable-build split for collections, mirroring `001-map.md`: read layer = `Iteration` (the "CollectionRead" layer), persistent builder = `PersistentCollection` (rename of today's `Collection`, whose `insert -> Collection` is functional), and a **new `MutableCollection`** (the in-place builder `MutableMap` already needs and nothing satisfies yet). Settle the read-layer name (`Iteration` vs a `…Readable` suffix for symmetry with `MapReadable`), document the laws and the relationship to `Stream`, and the contract for adding new carriers. | `iteration.anthill` and `collection.anthill` exist as code; the three-way split was surfaced by `001-map.md` (Map's MapReadable/PersistentMap/MutableMap is the keyed instance). Design rationale lives only in stdlib comments. Promote so future iterator shapes (parallel, chunked, paged) and the mutable builder have a clear home. |
