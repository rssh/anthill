...
- **Workspace layout** (mirrors rustus's `rustus-macros` / `rustus-core` / `rustus-prelude` split):
  ...
  - `rustland/anthill-rust-prelude/` — pre-registered mappings for primitives and stdlib traits: `i64` → `Int64`, `String` → `String`, `Vec<T>` → `List[T = ...]`, `PartialEq` → `anthill.prelude.Eq`, `PartialOrd` → `anthill.prelude.Ordered`, etc. Users do not re-annotate these.
