...
- **Dependencies**: `anthill-core` (KB types, term store, symbol table) and `anthill-stl` (for the stdlib sorts it must recognize: `Implementation`, `NamespaceMapping`, `CarrierBinding`, `Eq`, `Ord`, persistence and reflect sorts). `anthill-core` stays free of stdlib-specific knowledge; that knowledge lives here.
...
- **Workspace layout** (mirrors rustus's `rustus-macros` / `rustus-core` / `rustus-prelude` split):
  ...
  - `rustland/anthill-rust-prelude/` — pre-registered mappings for primitives and stdlib traits: `i64` → `Int64`, `String` → `String`, `Vec<T>` → `List[T = ...]`, `PartialEq` → `anthill.prelude.Eq`, `PartialOrd` → `anthill.prelude.Ord`, etc. Users do not re-annotate these.
