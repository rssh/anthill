...
```anthill
sort ScoredBranch
  import anthill.prelude.{List, Ord}
  sort R = ?
  constraint ordered_score :- Ord[T = R]

  -- N-ary scored choice. Each alternative carries its priority at branch time.
  -- Runtime resumes the highest-scored alternative first (per Ord.compare,
...
- Custom: pairs `(primary, tiebreak)`, vectors with lexicographic compare,
  arbitrary `Ord` algebras (e.g. tropical semiring for path cost).

The constraint `Ord[T = R]` is the only requirement. The paper's
...
here — `Ord` alone is enough for correct best-first ordering.
...
- `ScoreValue` is the host representation of an `R` value (typically `f64`
  in Rust, plus an `Ord`-callable trait for non-numeric `R`).
...
and `Some(s1)` vs `Some(s2)` ordered by the registered `Ord[T = R]`
...
// ScoreValue: f64 by default, with an Ord-callable trait for non-numeric R.
pub enum ScoreValue {
    F64(f64),
...
1. Add the `ScoredBranch` sort declaration to
   ...
   the `Ord[T = R]` constraint, the `fact Effect[T = ScoredBranch[?]]`
