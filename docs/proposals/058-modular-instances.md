...
**Status:** **Accepted / landed.** §3.1–§3.9 are delivered *and stated in the spec* — WI-845 landed §8, which rewrote `kernel-language.md` §8.7 *Instance coherence*, retired its per-`import` promise, and carried the named-vs-anonymous rule (§3.4) into §5.2 and the value-vs-type precondition split into §5.4. That closes the drift §3 indicted: every sentence of the amendment is either driven by a named test or cites the check that enforces it, and the one sentence nothing pinned — *an import governs visibility, not selection* — is now driven by `wi845_import_is_visibility_test`. Delivery detail per section: §3.3's composition WI-870; §3.6's relations and load checks WI-860, their consumer (§3.2's rung 2a) WI-861 and its bound WI-1094, its sugar WI-862; §4's retirement of the `fact` spelling of a provision WI-862 (the in-sort half) and WI-20260917-S8JYF (the namespace-level half, which is where every corpus site was); §3.8's per-provision conditions WI-869, driven end to end over `Pair` (`wi858_pair_orderings_test`); §3.10's derivation WI-1098, its parametric leg WI-20260918-CKD4J, and use-site discharge WI-1102. **Still proposed, and marked as such at each site:** §3.8's *derived* `PartialOrd` floor beneath an `Ord` witness. **Deferred by name** (§3.6 and §4): `Coherent` rows with the `coherent sort` sugar, a `within:` field for sort-scoped defaults, a per-carrier `NoDefault`, an authored name slot on the instance-claim form (§3.1). This document states the language **rules and surface** only. Implementation mapping, phase status, measurements, and build order: [`../design/058-implementation.md`](../design/058-implementation.md). Exploration record: `docs/brainstorms/prelude-multiple-orderings.md` and git history.
...
Supply and demand are two ends of one relation, and the language already has it. Every `provides Spec[…]` declaration is recorded by the loader as a `SortProvidesInfo` fact, and the reflect layer exposes those facts as a queryable relation: `provides(?A, ?S_inst)` — "sort `?A` provides the spec instance `?S_inst`" (`stdlib/anthill/reflect/typing.anthill`, whose own comment calls `requires X` and the provision "the two ends of one relation"). A `fact Spec[…]` was a second spelling of the same declaration until §4's retirement; it is an ordinary fact now.
...
Two or more providers of one spec for one carrier **may coexist** — if and only if every ambiguous call they could cause has a repair the author can write. There are two repair currencies. Concrete providers pay in **values**: their values carry their own sort, dispatch answers by the value, and they coexist as before with no names involved. Abstract providers pay in **names**: an unselected call's repair is `[Spec = W]`, so every candidate needs a spelling. A witness sort has one — its declared sort name. An instance claim does not — its identity *is* the bindings it asserts — so a group containing one keeps the load-time refusal: coexistence one cannot select out of is a trap, not a feature. ("Instance fact" was its name while §4's retired spelling was how one was written; what the gate reads is `provision_binds_any_op`, a property of the bindings, so the classification survived the retirement unchanged.)

The missing names are **not generated**, deliberately. A bracket value is a *declared identity* — stable under edits, resolved through the ordinary name system. A generated name would be either derived from the claim's content (a fingerprint: editing it breaks every site that wrote the name, or silently re-binds them if another claim matches the old fingerprint) or ordinal (reordering two declarations silently swaps what every written selection means — last-wins in a new costume). Wanting a name is served by declaring one: wrap the claim in a witness sort, or — the deferred increment — an *authored* name slot on the claim form (`provides AddM: Monoid[…]`).
...
**What the binder resolves to at a call is what tells the two apart** (WI-1094). A still-FLEX binder is silence — nobody wrote it anywhere — so the ladder answers and the answer is written INTO the binder, and every later bracket-less call reads it back as an ordinary tier-1 selection. A binder the enclosing signature QUANTIFIED (`size(s: SortedSet[T = String])`) is not silence, and §3.9 leaves only two answers for it. Forwarding is the value's own only where the parameter's **type names the slot as one of that signature's own declared parameters** — §7.1's `report[T, O]` form — **and the frame holds a dictionary for that parameter**: its own named slot, or a slot a requirement of the signature binds to it (`requires PersistentCollection[C = SortedSet[T = E, O = O], …]` holds `SortedSet`'s own `O` dictionary). The forward is answered from that entry and no other (WI-20260923-WN9P8). Anything else the frame happens to hold is evidence for the signature and not for the value: an *anonymous* `requires Ord[T]` covers the goal and forwards, and **measured**, that reads a `Descending` set back in ascending order exactly as a construction would. So does another declared parameter's slot of the same spec, which is why a declared parameter with no dictionary of its own is refused rather than forwarded. So the erased slot is refused, at that call, **whatever the provider count** — the pre-WI-1094 refusal of this shape came from a tier-3 tie and disappeared as soon as one provider was left standing.
...
**A SPEC's `provides` is a CONVERSION, and it is a chain entry rather than a provision row** *(delivered — WI-1110)*. The `provides` relation records what has which type; `B provides A[T]` written on a **spec**, binding `A`'s carrier parameter to `B`'s own type parameter, records no such thing — nothing has type `B` — and says instead "hold a `B[T]` and you can obtain an `A[T]`", which is what `B <: A` means operationally. So it takes a slot in `B`'s chain, exactly where `requires A[T]` would, and differs only in **who fills it**: a `requires` slot is filled by the caller, a conversion slot by the provider's own derived row. Filing it as a provision row instead makes the spec a candidate answer to every goal for the floor below — a candidate that can never resolve, eliminated only by cycle detection, and one that closes a cycle outright if the same edge is also written `requires`. The distinguishing test needs the operations as well as the bindings: a **parametric witness** (`sort AnyM { sort E = ?; provides Monoid[T = E]; operation combine(a: E, b: E) }`) has the same shape and *is* a dictionary. See `docs/proposals/library/007-weak-vs-strong-ordering.md` §"WI-1110".
...
### 3.10 Lawfulness derives positively — the missing use-site discharge *(delivered: the derivation by WI-1098, its parametric leg by WI-20260918-CKD4J, the use-site discharge by WI-1102)*
...
**What landed.** WI-1098 asserts the lawful side (`Eq` + `PartialEq` for every Total composite, `eq_derive::derive_total_eq`), so the "gap in the positive channel" is closed for a concrete-field composite. WI-20260918-CKD4J derives the parametric case in the clause form (`eq_derive::derive_conditional_eq`): a parametric sort gets `provides PartialEq[S] :- PartialEq[P…]` and `provides Eq[S] :- Eq[P…]` over the parameters its fields mention, and a composite's field typed as a parametric sort at concrete arguments (`Option[T = Int64]`) is decided through that sort's conditions translated to the arguments — so `Option[Float]` stays unlawful. The `NonEq` mirror is taken under the same ticket: a composite reaching `Float` through a field type's arguments classifies Partial, and a parametric constructor's value is compared field-wise when its arguments reach a `Float`. Not derived: a conditional `NonEq` for a parametric sort itself, so a use-site key whose unlawfulness is in its argument (`Map[K = List[T = Float]]`) is still the known gap. WI-1102 reads the discharge positively at the **call**, which is where both the carrier and the spec are in hand: a requirement whose type parameters the call site pins concretely, and whose goal no provision answers, is a load error naming the carrier and the missing provision (`kernel-language.md` §5.2, "A requirement whose carrier the call names…"). The refusal is withheld for an abstract element, a rule-body goal, a callee that never reads the slot, and a spec op that resolves structurally — each stated at `typing::OpSlotParkSite` with the program it was measured against. The `Set[T = X]` / entity-field reading of the same rule is WI-644's `check_use_site_requires_eq`, which is the *declaration*-side reader and unchanged here.
...
Two notes on the surface this rides on. Inside a sort, `provides X[…]` and `fact X[…]` were **one construct** — both recorded the same provision (measured end to end). *(Delivered — WI-862 deprecated the in-sort spelling; **WI-20260917-S8JYF completed the retirement at BOTH levels**)* **the `fact` spelling of provisions is retired**: `provides` is the one spelling, and `fact` means only a plain data assertion (`WorkItem`, `Covariant`, `DefaultProvider`) — removing the language's one construct whose meaning depended on its container. The retirement converges with §3.8's per-clause surface (`provides X[…] :- goals` is the conditional leg of the same consolidation) and halves the sugar: `default provides X[…]` — one leading modifier, the `internal` pattern, marking the enclosing sort as that instance's default (sugar for the §3.6 row) — needs no `default fact` twin.

**The namespace-level population went with it**, and this paragraph used to say the opposite ("Namespace-level op-binding instance facts (§3.1) sit outside sorts and are untouched"). That scoping was measured wrong on delivery: the in-sort spelling had **zero** corpus sites, so the deprecation warned nobody, while the namespace-level one had **twenty** across nine files including the stdlib. What supplies the missing address is a **059 secondary entry** — `namespace <Carrier> … provides Spec[…] … end` — which emits byte-identically to what `fact Spec[Carrier]` emitted and needs no new surface. §3.1's model is unchanged by the move: an instance claim's identity is still its BINDINGS, and the coexistence gate reads `provision_binds_any_op` — a property of the bindings, not of the keyword — so a migrated op-binding claim is still the unnameable candidate the gate refuses to let coexist.

Migration is **not** mechanical, which is the one claim here that measurement corrected — twice. First, a `fact Spec[Carrier]`'s carrier is POSITIONAL and derived while a provision takes its provider from the ADDRESS, so a keyword swap in place records a different provision; the migration target is the entry, not the keyword. Second, a `fact` is a rule with an empty body, so it also enters the **rule index**, and four readers keyed on that alone: region analysis, in its two functions (`is_modifiable(Cell)` answered *false*), the Rust supertrait bound, the namespace-level `impl` marker, and a `requires` clause mixing a value precondition with a spec requirement, which was proved from Γ wholesale and only ever passed because the raw `Ord[T = Int64]` row made its spec conjunct resolvable as a goal. Each was found by migrating the tree and running it. The rule that survives: where some rule resolves `Spec[…]` as a **goal**, keep the fact and write the `provides` clause beside it — what is retired is the *spelling of a provision*, not the fact. What closes the silent drop the retirement was filed for is the retirement itself: with one reading left, a missing import demotes nothing. A gate at the head — refusing a bracketed head whose functor names no declared sort — was built and removed, because a clause head is a DECLARATION and `fact myrel[T = Red]` is a rule-introduced predicate carrying a type argument. A `provides` clause is admitted inside a proposal-038 `provides <Carrier> language <L> … end` block too, since the block opens the carrier's scope; without that the retirement would have deprecated the only text that worked there.
