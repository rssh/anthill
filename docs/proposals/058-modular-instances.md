...
**Status:** **Accepted / landed.** §3.1–§3.9 are delivered *and stated in the spec* — WI-845 landed §8, which rewrote `kernel-language.md` §8.7 *Instance coherence*, retired its per-`import` promise, and carried the named-vs-anonymous rule (§3.4) into §5.2 and the value-vs-type precondition split into §5.4. That closes the drift §3 indicted: every sentence of the amendment is either driven by a named test or cites the check that enforces it, and the one sentence nothing pinned — *an import governs visibility, not selection* — is now driven by `wi845_import_is_visibility_test`. Delivery detail per section: §3.3's composition WI-870; §3.6's relations and load checks WI-860, their consumer (§3.2's rung 2a) WI-861 and its bound WI-1094, its sugar WI-862; §3.8's per-provision conditions WI-869, driven end to end over `Pair` (`wi858_pair_orderings_test`); §3.10's derivation WI-1098 and use-site discharge WI-1102. **Still proposed, and marked as such at each site:** §3.8's *derived* `PartialOrd` floor beneath an `Ord` witness; §3.10's parametric leg (the clause form for `Eq[Pair[A, B]] :- Eq[A], Eq[B]`, a concrete-field composite being all WI-1098 derives). **Deferred by name** (§3.6 and §4): `Coherent` rows with the `coherent sort` sugar, a `within:` field for sort-scoped defaults, a per-carrier `NoDefault`, an authored name slot on the `fact` form (§3.1). This document states the language **rules and surface** only. Implementation mapping, phase status, measurements, and build order: [`../design/058-implementation.md`](../design/058-implementation.md). Exploration record: `docs/brainstorms/prelude-multiple-orderings.md` and git history.
