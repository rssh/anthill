...
- **The gap that remains is "accepts" vs "receives on"** (WI-1077): a self-representing spec that takes its own element somewhere — `Set.insert(s: Set, x: T)`, `Map.put(m: Map, …)`, `LogicalStream.pure(x: T)` — still records that parameter, so `Relation provides LogicalStream` infers no default. Anything built on this substrate should assume the inferred leg covers specs whose operations touch no type parameter, and not yet `Set`/`Map`.
