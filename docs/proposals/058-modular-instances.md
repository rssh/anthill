...
**The MIXED pair must be refused too** — one witness sort *and* one instance fact for the same `(spec, carrier)` — and **measured (2026-07-28), it is not refused today: it loads clean.** The two existing checks are each grouped by provider KIND (`AmbiguousInstanceFact`, `load.rs:283`; `AmbiguousWitness`, `typing.rs:15995`, which explicitly skips a provision whose provider *is* the carrier), so neither sees the cross-kind pair; the same program with the instance fact replaced by a second witness sort is refused, which is the control. Two dictionaries for one `(spec, carrier)` therefore already coexist with dispatch silently picking one — filed as **WI-838**, pre-existing and independent of this proposal.
...
|---|---|---|
| `witness_provision` / `witness_op_for_carrier` (`typing.rs:16116`, `:16164`) | first witness wins | its own doc: *"Coherence (two witnesses for one application) is rejected at load … so first-match is the sole instance"* (`typing.rs:16114`) — tier 3 deletes that premise |
