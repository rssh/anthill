...
## Status: DELIVERED (2026-09-19, WI-20260919-1Z3E7) — the scoping rule, the syntax, the `Pair` migration. Decided in discussion on WI-20260918-CKD4J, including the keyword (`where`, §3). The layout per provision (§7) is delivered under the same ticket.
...
## 7. A layout per provision (decided and delivered 2026-09-19)

§1–§6 scope a body's *reading* of a condition; the frame is still laid out per carrier, and WI-869's strictness mask leaves a sibling provision's slots `Unavailable` at a dispatch that did not earn them. §7 makes the layout match the scope, so that a block is what it reads as — a dictionary-holder whose `requires` are its clause's `:- goals`.

1. **The layout is per provision, with conditions decided per clause.** A provision's chain is the carrier's sort-level `requires` followed by its conditions. An operation outside every block has the sort-level `requires` only. An operation's frame is its owner chain followed by its own `requires`, as today. For a carrier with no conditional provision every chain is today's chain, so nothing outside the conditional carriers moves. Two clauses of one spec are two ALTERNATIVES (WI-1033): the provision holds where one clause's conditions resolve, decided at the dispatch; since no body reads them (point 5), they lay out no slot, and the layout is keyed by (carrier, spec) — which is also what lets eval predict a dictionary's shape from its spec and provider, as it always has. (WI-869 laid every clause's conditions into one chain and demanded all of them at every dispatch; measured, a `Box[OnlyA]` satisfying one of two `Show` clauses was refused — `wi_1z3e7_…::alternative_clauses_hold_when_one_does`.)
2. **The sort-level slots keep one name in every chain of the carrier**, so a frame handed from one body of the carrier to another is read by name as today. A collision is resolved on the condition side, as the op half already resolves one.
3. **A member backs only its own provision.** When a spec's member is looked up on a carrier, an operation in ANOTHER provision's block is not a candidate. `sort Box { provides Weak[T = Box] :- Weak[A] where operation weak … end; provides Twin[T = Box] }` with a `Twin` that also declares `weak`: `Box.weak` exists only where `Weak[A]` does, so `Twin[Box]` has no `weak` — a load error on the `Twin` provision (or `Twin`'s own default, if it has one). Consequently a dispatch never reaches a member through a foreign provision, and the dictionary it builds is always laid out as the member's frame.
4. **A direct call to a member from another body of the carrier** (`Box.eq(a, b)` from a helper) builds the member's clause dictionary at the call site — its conditions resolved in the caller's scope, exactly as a call from another carrier does. Without evidence in scope it is the ordinary load error.
5. **A `where` block on a spec the carrier provides through more than one clause** is a load error for now: a member backs only its own clause (point 3), so the other clauses would have no member, and a block on each would declare `Carrier.op` twice. Clauses are identified by content (carrier, spec, conditions), so re-loading a file does not double them. Blocks as anonymous instances — members identified by (carrier, clause, name), dispatch resolving to a clause — is the recorded direction beyond this. Its motivating case is not only the conditional one: an UNCONDITIONAL block is a morphism from the carrier to the spec, and a carrier with two morphisms into one spec (`Int64` a `Monoid` by `add` and by `mul`) is two unconditional blocks of one spec — refused by this rule today, and exactly what instance identity would admit.
6. **Retired**: WI-869's strictness mask and its `Unavailable` provider slots, the `NotThisDispatch` refusal (unreachable after 3), and §6's hidden-slot mask (each body's chain is exactly its scope).

### 7.1 Implementation

- `provider_dict_chain(sort, key)` / `provider_dict_entries(sort, provision)` — the chain per (carrier, provision), normalized through `provision_layout_key` (`None` = sort-level; a carrier without conditions answers its sort-level `Rc` for every provision). `DictChain` carries the key, and names it: the sort-level half is named alone, a condition's collision is resolved on the condition side (`_c`).
- An operation's frame: `op_owner_dict_entries` (its sort's chain under `op_owner_provision`), then its own `requires`. The typer installs it per body; `op_reads_requirement_slots` is the per-operation "needs a dictionary" gate.
- Dispatch: `candidate_provider_sub_goals` lays the provider half out for the dispatched spec; `alternative_condition_goals` decides a multi-clause spec in `resolve_inner`. `dict_layout(spec, provider, self_provision)` — the parent bundle (spec == provider) is laid out for the called operation's provision, which eval reads off the target.
- Same-carrier calls: `frame_serves_callee` decides the WI-418 inherit; otherwise the member's dictionary is built at the call from the caller's whole frame, or the call is refused (`ProvisionConditionOutOfScope`).
- Removed: WI-869's `strict_mask` and its `Unavailable` provider slots, `UnavailableWhy::NotThisDispatch`, and §6's hidden-slot mask (`ResolutionScope::hidden`, `DictChain::hidden`).
