...
**Status:** Delivered — Phase 1 (`===`, WI-615) and Phase 2 concrete tier (dispatched `=`/`eq` + `Set`/`Map` membership instances, WI-616). The Phase 2 delivery did NOT need the SLD→eval op-body bridge: carrier `Eq` impls are relational rules on the carrier's own `eq` op, so ordinary SLD is the evaluator — the semantic-`eq` builtin (`BuiltinTag::SemEq`) succeeds by reflexivity on structurally identical operands and otherwise proves `<carrier>.eq(a, b)` (the load-time eq-dispatch index over `sort_ops` own-op overrides, the WI-350/WI-444 short-name convention) in a **closed, bounded sub-resolution**: bindings never escape (`=` stays a test), the first definite proof settles the verdict (`eq` stays semi-deterministic), and non-ground / buried-override / truncated compares suspend as *undecided* rather than deciding. Still gated on the bridge (WI-625): host-BODIED `eq` impls, retroactive instance-fact dispatch at SLD (`fact Eq[T = X, eq = …]`), the abstract-argument requirement-dictionary tier (WI-300 Tier B), and the interpreter's raw structural `eq` fallback.
