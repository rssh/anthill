...
**R2 — A `namespace` at a sort's address is a second entry to that sort's scope.** Legal before, beside, or after the definition — the *before* order is a load-order accident today, not a language distinction (WI-979). The operations and consts it declares are **members of `X`** — dot-dispatchable, backing provisions, scoped by `X`'s type parameters. Rules are not members and R3 refuses them there. One symbol, as everywhere else (WI-926). This is the only route to a member of a type whose declaration one does not own, and it is 001's deferred `companion` spelled by address rather than keyword.

**R3 — A second entry may add members and provisions, never identity.**

**Allowed — stated explicitly, because it is the point of the mechanism:**

- **operations and consts** — the members;
...
- a **rule** — of any kind: a Horn clause, a `[simp]`/`[unfold]` equation, a sort-scoped `dot_apply` law.
...
One honest limit on this refusal: it does **not** close the underlying hazard. A rule extending a predicate declared elsewhere is non-monotone wherever it is written — an ordinary nested namespace does the same, and so does the sort's own body. R3 closes only the case this proposal creates, where the writer is not the type's author.
...
**R4 — A second entry may FILL, never DISPLACE.** Both halves are 058's rules applied to members rather than new ones:
