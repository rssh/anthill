...
## Status: PARTLY IMPLEMENTED (2026-09-21). Proposed 2026-09-19, decided in discussion after WI-20260918-R541X; supersedes the two open directions recorded on WI-20260919-891QP.

**§7 steps 1–4 are DONE** (BQHGD, HXGXF, N31XX, 891QP). **WHAT REMAINS:** §2's structural FORMERS still have no derived instance (a named tuple or arrow has no sort to carry a provision — step 2), §7 step 5 (WI-20260919-H20YY), and open questions 1 and 4. **2 is answered**, by 28TAT removing the frame channel outright; **3 is answered** (2026-09-22) — a requirement at a former carrier is refused at the call, which is what the missing instance MEANS for a program until step 2 lands. 1 is deferred by decision; 4 is superseded by a planned removal of the interning rather than fixed here.

**BUILT:** §2's derivation (WI-20260919-HXGXF, sorts only — structural FORMERS deferred), §3's subset rule including the type-parameter alignment an implementation needs to restate a clause (WI-20260919-N31XX part 1), "The rule" with §6's migration in both halves — the READ and the FORWARD — and **§1's lowering, for a read backed by an OPERATION-level clause and by a SORT-level one alike** (N31XX part 3).

**§1 IS COMPLETE, BOTH HALVES.** A value read of a rigid is a dispatch through its slot whether the clause is written on the OPERATION or on the enclosing SORT. The sort half needed one thing elsewhere: `build_concrete_dispatch_dict` was being handed the caller's SORT-only chain, so a FREE operation's own `requires` was invisible to it and `require_complete` dropped the whole dictionary — the callee then ran with an empty requirements frame. It is given the whole frame chain when the caller's sort chain is EMPTY **and the callee's chain names `TypeValue`**, which is as narrow as it is because two pinned invariants sit either side: `serves` also gates proposal 066 §7.4's refusal (inverting it turned 59–76 unit tests red on a diagnostic aimed at calls it was never about), and `wi822_op_scoped_supply_test` pins that the instance-dictionary channel reads the sort half ON PURPOSE, since host entry seeds no op slot. `TypeValue` may widen where another spec may not because **this proposal's own forward rule guarantees the caller declared it**, so every anthill route fills the slot before the forward is read.

**§4 / WI-20260919-891QP IS CLOSED BY IT.** R541X's two (C) rows — a PROVIDER's and a WITNESS's own parameter on a member entered through a requirement slot — now ANSWER `Box(V: Boom)` and `Crate(W: Boom)` where they were a located fault. A `Dictionary` still carries no type BINDINGS; what changed is that the answer no longer needs one, because the evidence that selected the provision IS the type. Step 4 needs no separate work.

**TWO PROGRAMS THAT USED TO LOAD ARE NOW REFUSED, both correctly.** `twoReq[P, Q] requires TypeTerm[T = P], TypeTerm[T = Q]` calling `TypeTerm.valueOf()` picks no clause, so the evidence cannot be built — the ambiguity is now reported at the call instead of at run time. And a bracket value with an unwritten slot (`[U = List]`, which RS2G4 expands to `List[T = ?T]`) leaves the element undetermined, so a body that READS it is refused; the written form loads and answers. Both were `EvalError` before. §7 records each step's state.
...
The frame type-argument channel stops being consulted for value reads once every such read is backed by a slot. **It is now GONE ENTIRELY** (WI-20260921-28TAT, 2026-09-21): its one other consumer, `Error.reify`'s `T1` (027.4), was migrated to the very route this section describes — the boundary reads its payload sort out of the dictionary its own `requires ErrorTag[T = T1]` supplies — and with no reader left, `Frame.type_args`, `FrameTypeArgs`, `find_type_arg`, `collect_closed_type_args`, `collect_resolved_type_args` and `ground_type_params` were deleted. Open question 2 was framed as 3MV2C's decision to make; it was answered by removal instead.
...
**The census does not cover:** bodies loaded with `run_typer: false` (never checked); rule bodies (excluded by the rule); the scaland port; and `Error.reify`'s `T1`, which read the frame channel rather than a body (open question 2 — since answered: 28TAT migrated it to its own `requires ErrorTag[T = T1]` and deleted the channel, so the census's one exclusion no longer exists).
...
3. **The load rule + lowering (§1)** (WI-20260919-N31XX), with the migration of step 1's census in the same change, so the suite never passes through a state where the rule is on and a read is unmigrated. **ALL THREE PARTS ARE BUILT: the rule and the migration (2026-09-20), the lowering for an OPERATION-level and a SORT-level clause alike (2026-09-21).** See the status block above for §1's two halves.
   ...
   **The leg WAS deliberately NARROW, and the control that said so has since flipped.** When this landed, the same forward shape over a plain user spec — `requires TT[T = B]`, nothing to do with 065 — still loaded: operation-level requirement propagation was not checked in general, and that wider gap was real, pre-existing and not this proposal's to close. **WI-20260920-XSVCS closed it** (2026-09-20), so the plain-spec forward is now refused too, by the OTHER arm — carrier-shaped rather than spec-shaped (`kernel-language.md` §5.2). This arm survives it and is checked first, because where it applies it is stronger: it needs no evidence from the callee's body, and it reaches a `TypeValue` requirement whose carrier is *not* the caller's parameter. The acceptance row still asserts BOTH halves, now asserting each arm's own message, so the two cannot be confused later.
4. **WI-20260919-891QP** through instance contexts (§4); its two R541X rows flip to positive. **CLOSED BY STEP 3, with no separate work** (2026-09-21): the lowering's sort half is what carries a provider's own parameter, so both rows flipped as part of it. See §4 and the status block.
5. **WI-20260919-H20YY**.
...
2. **`Error.reify`'s `T1`** — **ANSWERED (2026-09-21), by WI-20260921-28TAT rather than by 3MV2C.** It was the second consumer of a rigid-as-value, reading the frame channel rather than the body, and this question held the channel open until someone decided. 28TAT decided it the other way round: the boundary now reads its payload sort out of the dictionary that `operation reify[Rho, X, T1](…) requires ErrorTag[T = T1]` supplies (`stdlib/anthill/prelude/effects.anthill`), so `Error.reify` carries a clause exactly as this proposal's rule demands of any other consumer — and with its last reader gone, `Frame.type_args` and the channel with it were deleted. 3MV2C still owns the *error-tag* design; it no longer owns a decision this proposal is waiting on.
3. **Formers without a value reading** — **ANSWERED (2026-09-22): refused, as proposed, but at the CALL and not at the read.** §2's derivation is sorts-only, so a former gets no derived instance. The verdict rests on the dep being GROUND — no later instantiation can change what it names — plus a failed search, NOT on formers being unprovidable. **They are not**, and review measured it: a SORT may carry a row whose spec binding is a former (`sort Wrapper … provides TT[T = (a: Int64, b: String)]`), and that satisfies `requires TT[T = B]` at the tuple. Such a program never reaches the arm, because the provider search finds that row; the refusal means "nothing provides it", and its repair names the row an author could write.

   **It was not merely undecided; it was a PANIC.** Measured before the fix: `tyOf((1, 2))` against `tyOf[B](x: B) requires TypeValue[T = B]` **loaded clean** and then died `DeferToRequirement: __req_typevalue not bound in caller frame`, an `EvalError::Internal` that `bridge_op_to_eval` raises as an abort. Both existing arms answered `None` and neither was wrong to — `unprovided_provision` bails at `sort_functor_of_view` (a former has no declaration a repair line could name) and `caller_rigid_carrier` bails because the carrier is ground, not a caller parameter — so the slot fell through to the silent-absence rule that exists for the 29 stdlib bodies which declare a chain and never read it. This body reads it.

   **AT THE CALL, because that is where the carrier is known.** A body reading `B` cannot see that some caller will pass a tuple. The former is named either way. Not keyed to `TypeValue`: the reason a former cannot be provided is about the CARRIER, so the arm is too — WI-20260921-3G1YT's lesson applied rather than re-learned.

   **ONE EXEMPTION, and it cost 20 red rows to find.** A **body-less** callee is excluded, because who interprets an absent slot is the host's business there. `Error.reify` declares `requires ErrorTag[T = T1]` and is body-less on purpose — the boundary is a frame the interpreter installs by symbol — and a TUPLE payload is a live, correct corpus program: the host narrows the caught payload when the evidence is there and **catches wide** when it is not, as every boundary did before narrowing existed. Refusing it took the whole of `wi_9wvt7_error_reify_test` red. The guard asks only whether an anthill body EXISTS, which is not the body WALK 3G1YT deleted: that one asked what a body *contains*.

   **The arrow/effect-row half of this question is still open** — an arrow with a rigid effect row could not be driven to the same point (a bare operation reference is refused earlier, for unrelated reasons), so what is settled is the *former carrier*, tuples measured.
4. **Transients.** A `Type` value built per dispatch is a transient, and interned terms live for the KB's lifetime (CLAUDE.md's representation note; 3MV2C's 2026-09-18 feedback on `ground_type_params`). The lowering must build its result on the transient carrier, not intern one per call. **Still the case** — `type_of_dictionary` ends in `kb.alloc` and the arity-0 branch goes through `make_sort_ref` → `alloc(Term::Ref(…))`. One nuance that narrows it: `alloc` hash-conses, so repeated calls *at the same type* cost nothing after the first and the growth is in DISTINCT types reached, unbounded only where a generic family deepens structurally. **Decided not to fix here (user, 2026-09-22): the interning is to be removed outright rather than worked around.**
