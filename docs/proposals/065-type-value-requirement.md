...
**HALF OF §1 REMAINS.** A read backed by a SORT-level clause is still served by the frame type-argument channel, because a spec op with a DEFAULT BODY is dispatched statically and its frame carries no dictionary at all (HXGXF measured this; it is the same fact that forced `TypeValue.type_value` to be body-less). Lowering that half takes five R541X rows to `Internal("… `__req_typevalue` not bound in caller frame")` — working programs dying — so it waits on **WI-20260919-H20YY**, which is what makes such a member carry its evidence. Measured from the other side too: with the channel backed out, `wi708_body_type_arg_read_test` — the file whose whole subject IS that channel — passes in full, and the 16 rows that go red are every one a sort-half read. So "the channel stops being consulted" is now true of op-level reads and of nothing else.

**WHAT THE LOWERING ALREADY CLOSES, held back only by that gate.** Lowering the sort half as well made R541X's two (C) rows — a PROVIDER's and a WITNESS's own parameter entered through a requirement slot — answer `Box(V: Boom)` and `Crate(W: Boom)` instead of their located fault. That is §4 / **WI-20260919-891QP** closing for free, measured, and waiting on the same gate. §7 records each step's state.
