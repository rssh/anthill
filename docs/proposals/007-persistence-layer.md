...
- The Stage B unification skips answer dedup when σ has any non-`Value::Term` binding. `kb::reify` walks bindings via `resolve_with_term`, which only sees `Value::Term` entries — so external-row substitutions would all reify to the same TermId (the goal with unbound vars) and the dedup would collapse genuinely distinct rows. Hash-consing a Value-tree to make `reify` Value-aware would defeat Q4's no-`TermStore`-growth guarantee, so the dedup is structurally skipped instead. *(Superseded: WI-348 made the key a carrier-agnostic structural fingerprint that reads any carrier through `TermView`, so the blindness this compensated for is gone — the by-carrier skip was replaced in WI-1023 by two opaque guards, and an external row on a `Value` carrier now keys as faithfully as its term twin. Since WI-FFPGD the predicate is `SearchStream::is_duplicate_answer` and projects onto the QUERY's goals; a relation drain, which is a bag of proofs, turns it off outright.)*
