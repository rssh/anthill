...
3. **`Value::Map(MapHandle)` runtime.** Arena-refcounted, like `Substitution` / `Stream`. `MapKey` covers Int64 / Bool / Str / Symbol / Term; non-scalar keys deferred. A key is canonicalized before it is stored, so one logical key never occupies two slots: a reflect `Symbol` keys as `MapKey::Ref(sym)` from either of its carriers (`Value::SymbolRef` or a `Value::Term` wrapping `Term::Ref`). Canonicalization is why `try_from_value` reads the KB (WI-1016). The canon is read off `ViewHead`, so it reaches every spelling of one name — including the non-canonicalized nullary constructor `Fn{c,[],[]}` that `resolve_qualified_name_term` mints by bypassing `alloc`'s WI-511 rewrite (WI-1023). `Term::Ident(s)` is a DIFFERENT term and keeps its own key: widening there would MERGE two keys and let one `put` overwrite the other's entry.
