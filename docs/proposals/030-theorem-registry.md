...
- δ.4 partial (WI-781) — the policy now drives **cite-hypothesis** emission. The prove driver's `render_cited_lemmas` routes every cite through `render_cited_lemma_under_policy` instead of lifting unconditionally, so a `TranslationPolicy` fact observably changes the emitted document and the WI-772 bodied-policy refusal reaches the CLI error channel — both were unreachable before, `policy_for` having had no production caller at all. Arms wired: `LiftedAxiom` → `lift_rule_to_implication_clause` (the default for anything cited, so emission is unchanged absent a fact); `Inline` → no spliced hypothesis, since the consumer's own emission inlines the body where it references the predicate. `DefineFun` / `DeclareFun` are **refused loudly** rather than falling back — the emitters do not exist, and a silent fallback would discharge under a policy the author did not choose. So is `Inline` on a consumer emitted with `abstract_body` (structured proofs' `conclude` record), where no rule call is descended into and the citation would therefore contribute nothing at all while appearing honoured.
  - **Still open, and the larger half:** the other call-site channel. `Emitter::process_body_goal` — which lowers a reference to a predicate *inside a rule body* — never consults `policy_for`. A `TranslationPolicy` on a predicate that is used in a body but never cited is still silently ignored, which is the same inert state WI-781 fixed for cites. Routing that, plus writing the `DefineFun`/`DeclareFun` emitters, is what remains; the emitters stay gated on the concrete consumer this item always named (multiple proofs citing one predicate, where a dedup'd `define-fun` + `assert (forall …)` shortens the document).
  - Note for whoever writes them: the current `LiftedAxiom` arm emits the *implication* form with the predicate symbol **erased** (`(assert (forall (…) (=> premises conclusion)))`), not the `declare-fun NAME` + `assert (forall (args) (= (NAME args) body))` that this proposal and the schema describe. It is the right hypothesis for a single consumer and the wrong shape for symbol-level reuse — so the symbol-preserving form arrives with `DefineFun`/`DeclareFun`, and `LiftedAxiom`'s own emission likely changes then too.
- δ.5 ✗ deferred — logic-fragment selection from policy mix is meaningful only once `DefineFun`/`DeclareFun` emit (above).
