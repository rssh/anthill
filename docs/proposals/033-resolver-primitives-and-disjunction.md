...
5. **007 ↔ 026.1 reconciliation (out of scope for 033, but tracked here for cross-reference).** Proposal 007 (persistence layer) and proposal 026.1 (Value-integrated KB queries) describe overlapping concerns — external data flowing into the resolver — in unreconciled vocabularies. 007 predates `Value` and reads as TermId-only. 026.1 Q4 (deferred) describes a Value-based external-stream path that would naturally back queryable `SqlStore` retrieval at scale. Until 007 is revised post-026.1, the §`step_init` forward-compat note above ("if Q4 lands, the walk will need `alloc_from_value`") is anchored on a Q4 framing that may turn out to be 007-driven instead. The proposal-base action — update 007 and/or 026.1, or write a unifying proposal — is independent of 033's implementation milestones.
