...
- **Workspace layout** (parallel to `rustland/{anthill-rust-derive, anthill-rust-export, anthill-rust-prelude, rust-anthill-gen}`):
  ...
  - `scaland/anthill-scala-prelude/` — pre-registered mappings for primitives and Scala stdlib types: `Int64` → `Int64`, `String` → `String`, `List[T]` → `List[T = ...]`, `cats.Eq` → `anthill.prelude.Eq` (if cats is available; otherwise just `Object.equals`), `Ordering` → `anthill.prelude.Ord`, etc. Users do not re-annotate these.
