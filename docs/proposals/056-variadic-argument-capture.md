...
**Status:** Delivered (2026-07-17; WI-727 — grammar `...args` marker, loader capture + `Without[T, Drop]` type constructor, `fix` reimplemented as an ordinary op with no op-identity-keyed typer code). Draft written 2026-07-16 (outcome of the WI-714 `fix` design thread). **First-increment scope / deferred:** named-argument capture only (a positional leftover binds the capture parameter directly; symmetric positional capture is a follow-up); the rule-head rest-pattern face (§2.3, optional for `fix`); the polymorphic-column type-check in `Without` is best-effort (concrete schemas only).
