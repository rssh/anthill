...
**Status:** Delivered (2026-07-17; WI-727 — grammar `...args` marker, loader capture + `Without[T, Drop]` type constructor, `fix` reimplemented as an ordinary op with no op-identity-keyed typer code). The **rule-head rest-pattern face (§2.3)** followed on 2026-08-18 (WI-1129 — grammar `rest_arg`, the converter's position rule, the `[simp]` engine's capture fold, and `reflect.sub_occurrence_labels`, the "component names" reader §2.3 names). Draft written 2026-07-16 (outcome of the WI-714 `fix` design thread). **POSITIONAL capture** landed on 2026-08-18 (WI-1130) and **not as §3 OQ #2 first wrote it**: a positional leftover **binds the capture parameter directly** (`R` is that argument's own type), the two channels are **exclusive**, and the `_N`-collection reading was withdrawn — see OQ #2 and §5, both corrected. Nothing was deferred there; what WI-1130 fixed was a **double-bind**: the direct binding already happened, and the capture fold then appended its own synthesized record for the same parameter, so every positional arrangement died on a load error that named a parameter the author never wrote. The polymorphic-column type-check in `Without` stays best-effort (concrete schemas only).
...
**Affects:** grammar (the capture marker on a parameter / a rule-head rest-pattern), loader + typer (arg-matching collects the leftover named args into a named tuple rather than erroring; the `Without` reduction), `docs/kernel-language.md` (§5.4 parameters; §5.3 rule heads + §11 `CallArg`/`RestArg` — the two faces landed in §5.4 and §5.3 respectively, not §4/§8.3 as first written). Runtime: none new — the capture is an ordinary named-tuple value.
...
| 2 | **Named + positional** — RESOLVED, and NOT as first written (WI-1130) | The capture slot **is a position too**, so a **positional** leftover binds the capture parameter **directly** — `R` is that argument's own type, not a record wrapping it. `cap(5)` and `cap(rest: 5)` therefore denote the same thing. The **two channels are exclusive**: one positional leftover *or* any number of named ones, never both (`cap(1, 2, a: 3)` is a located refusal naming `a`), and a *second* positional leftover is the ordinary WI-1100 arity error. ~~a positional leftover lands in `R` as `_N` (§4.5 named-tuple positional sugar), so one mechanism covers named and positional~~ — **withdrawn**. The `_N` reading was never implemented and was rejected on measurement: it would have made `cap(5)` mean `(_1: 5)` while `cap(rest: 5)` kept meaning `5` — two spellings of one argument, two values, silently — and it would have built a record half named and half synthetic that the sole consumer, `Without` (which drops columns **by name**), rejects in its own reduction. So `...args: R` covers the two spellings by **binding**, not by collection. |
...
- **A *separate* positional-variadic feature** — not needed, and not built. **Not because positional capture is subsumed** (WI-1130 withdrew that claim, §3 OQ #2): a positional leftover **binds the capture parameter directly** rather than being collected, so what `...args: R` admits positionally is **exactly one** argument, and admitting it *excludes* the named channel. A genuinely variadic **positional** residue — two or more leftovers gathered into one value — is neither built nor subsumed, and is refused by the WI-1100 arity check. It has **no client**: every capture site in the tree is name-keyed (`fix`, `where_run`, `join_run` — the latter two *mint* their labels), and the driving consumer `Without` drops columns by name, so a positional residue would have nothing to consume it. Should one ever appear, it is an additive change from here.
