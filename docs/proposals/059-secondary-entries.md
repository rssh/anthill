...
- **any other fact, and any constraint.** They need no policy of their own: *facts are rules*, so a fact is a rule with no body and a constraint a rule with no head, and the rule ban above already reaches both.

  **Written as an exception to the ban, not as a list of bad facts.** The default is refuse — being a fact is exactly what makes it refusable — and `fact Spec[X]` / `provides` is carved out of it. Spelled the other way round ("facts are fine, except…") the list would have to enumerate every harmful shape, and a new one would arrive allowed.

  **TODAY THE CARVE-OUT CANNOT BE MADE EXACTLY, and that is a gap this proposal does not close.** The loader already has the discriminator — `maybe_emit_fact_provides_info` — but it recognises a spec claim by SHAPE: the fact's functor is a sort with at least one type parameter. Measured, that cannot tell a spec from a parameterized DATA sort: `sort Box { sort T = ?; entity box(v: T) }` with `fact Box[T = Int64]` is recorded as "Int64 provides Box", against a real spec claim recorded correctly and an ordinary `fact Point(x: 1, y: 2)` correctly recorded as neither. So this clause would admit a data fact over any parameterized sort. The root is that **nothing declares a sort to BE a spec** — there is no `spec` keyword and no `SymbolKind::Spec`, so every reader infers it from shape.
...
The things one wants to price about a secondary entry — the `requires` R3 refuses, the orphan `fact Spec[X]` it allows, and why its members are visible without an import — are **one question**, and it is not about secondary entries.
