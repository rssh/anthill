...
**Why not narrow the ban to rules with a NEW head?** It is the obvious refinement — a predicate nothing referred to cannot invalidate anything — and R4's capture clause looks like it already draws the line, since "new head" is exactly "the head resolves to nothing". Measured, it does not hold, for two reasons.

*A fresh head still flips a statement.* A main entry carrying `rule q(0) :- not freshp(1)` answers `q(0)` once; add `namespace Rec { rule freshp(1) }` and it answers zero times. The head was genuinely new — the symbol lands at `X.freshp`, the same place the main entry would have put it — yet an existing statement went from true to false. The mechanism is WI-895: a rule body's *unresolved* name interns bare and silently fails to match, so the main entry could already mention `freshp` while `freshp` resolved to nothing. "New" is a claim about what resolved before, and today a body may reference what does not resolve.

*And R4 would not police the result.* Two secondary entries that each introduce `freshp` do not collide: both clauses join one predicate (`freshp(1)` and `freshp(2)` both answer). Neither captured a pre-existing name, so the capture clause is silent, and a predicate ends up composed by two parties that never agreed on it.

So the narrowing is **deferred, not rejected**: it needs WI-895 (a rule body's unresolved name refused rather than bare-interned, which makes the first program a load error instead of a true statement) and WI-980/R6 (so that "new" is decided against the finished program rather than by text order). With both, "a rule whose head introduces" becomes a defensible line, and R4 would need a clause for jointly-introduced predicates.
