...
| `Wrap.dom(wrap(red())).takeN(5).length()` | ~~refused: "argument binding column `x` has an incompatible type"~~ loads since S1; flounders at run time — the value is bound, but the clause's bound still carries `Wrap`'s shared `T` (S3(d)) | `1` |
...
| demo2 `topAt[X]()`'s body `Wrap[T = X].top.head.x` | ~~refused: `expected Wrap[T = ?X], got Wrap[T = ?T]`~~ loads since S1; raises `empty_stream` at run time | `wrap(red())` / `wrap(blue())` by provider |
