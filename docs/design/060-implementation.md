...
| the typed-head sweep: the fill after the body and, in front of it, the type test `__domain_guard(?x, B)` (`BuiltinTag::TypeDomainGuard`: waits on an unbound or partly bound `?x`, so rotation re-asks it before a body goal that delayed after it; passes a GROUND value the closed reading cannot type, which the fill owns); the waiting conformance check alone for a type with no domain | `typing::install_typed_head_domain_goals` |
