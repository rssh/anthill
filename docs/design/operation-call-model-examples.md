...
- **Sub-instance indices below assume the SPECS AS WRITTEN HERE**, whose own `requires` chains are empty (`Eq`, `Monad`). Under the layout rule (`operation-call-model.md` §"Dispatch rule", WI-857) a dictionary bundles the **spec's** own chain first and the **provider's** second, so a provider-half index is offset by the spec chain's length. Mapping these examples onto today's stdlib therefore shifts them: `Eq requires PartialEq[T]` since WI-644, so an `EqList` dictionary's inner `Eq[X]` sits at `sub_requires[1]`, not `[0]`. The examples are left in their own model — the shapes are what they illustrate — but do not read an index off this page as a claim about the stdlib.
