...
operation residual(v: Vec, k: Int) -> Vec    -- `requires Numeric[T]` implicitly;
...
- **`find_operation_on_sort(sort, name) -> Option<qualified_op>`**, subsort-aware:
  ...
  - **Tier 1b** — ops of specs the sort *satisfies* (`Int.min` → `Ordered.min` via `fact Ordered[Int]`); the headline `requires`-typeclass case. The WI-240 `sort_ops` table covers user `fact Spec[ImplSort]` but **not** builtin satisfaction (`Int → Ordered`) — gap to close (WI-281).
...
- **D5 — tier-2 import semantics/timing** (043 open-Q10a). Plus **Tier-1b spec-satisfaction** (`Int.min`→`Ordered.min`) — currently a `sort_ops`-table gap (WI-281).
