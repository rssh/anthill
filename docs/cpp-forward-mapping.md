...
Body-LESS is the line, and it is the same one the load-time backing check draws: an operation with a body has an anthill definition the backend can in principle emit; one without gets its meaning from the host. Unlike the two refusals above, this one is not a capability gap in the generator — the fix is in a `.anthill` file — so it is *not* degraded into a `// TODO:` comment in the emitted body.

`String`'s operations were the leading example of this refusal; **WI-890 filled them**. `rustland/anthill-cpp-gen/anthill/string.anthill` now realizes the whole surface except `toUpper`/`toLower`, whose full Unicode case mapping has no C++17 standard-library form and which therefore stay a deliberate loud refusal here (an ASCII-only fallback would silently diverge on non-ASCII input, the WI-884 defect). `String`'s index unit is the Unicode scalar, not the byte, so `length`/`substring`/`indexOf` — and the structure-building `replace`/`split`/`trim`/`repeat` and the double-reading `endsWith` — lower to scalar-indexed helpers in `anthill_runtime.hpp` (`anthill::runtime::str_*`) rather than to `std::string`'s byte-indexed members.

`BigInt` still lands here, and by a **decision recorded in `stdlib/anthill/realization/cpp_std.anthill`**: C++17's standard library has no arbitrary-precision integer, so `BigInt` has no `TypeMapping` for the base `cpp` profile at all (its values do not even lower), and it is out of scope until a profile vendors a big-integer library (e.g. Boost.Multiprecision) through a `carrier` clause. The rust host realizes it via `num_bigint`; the two hosts differ here by design.
