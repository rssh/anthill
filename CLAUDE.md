...
- **`[simp]` is the enablement, `<=>` is the admission** (WI-881/884/888) — an untagged equational rule is INERT; only the `[simp]` tag makes a body-less operation's defining equation run (inlining LHS→RHS before dispatch — NOT spec-op backing, WI-818). A `[simp]` head is an APPLICATION, so a nullary head needs its parentheses (`tau()`, not `tau`). The CONNECTIVE decides only whether the rule LOADS: `<=>` is the sole spelling admitted at a bodyless head and an `=` one is a load error (a GUARDED `lhs = rhs :- g` keeps `=`), while the tag alone decides whether it FIRES. Rule + traps: `docs/kernel-language.md` §5.3.
