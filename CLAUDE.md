...
- **`[simp]` is the enablement, not the direction** (WI-881/884) — an untagged equational rule is INERT; only the `[simp]` tag makes a body-less operation's defining equation run (inlining LHS→RHS before dispatch — NOT spec-op backing, WI-818). A `[simp]` head is an APPLICATION, so a nullary head needs its parentheses (`tau()`, not `tau`), and firing tracks the tag ALONE (`=` and `<=>` both load as equations) — don't diagnose an inert rule from its connective. Rule + traps: `docs/kernel-language.md` §5.3.
