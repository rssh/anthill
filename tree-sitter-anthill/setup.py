...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_anthill", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_anthill": ["*.pyi", "py.typed"],
        "tree_sitter_anthill.queries": ["*.scm"],
    },
    ext_package="tree_sitter_anthill",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_anthill/binding.c",
                "src/parser.c",
            ],
            define_macros=[
